...
void CMHbook::Clean()
{
  ...
  for(i=0; i<NHIST; i++)
  {
    if(hist1[i]!=NULL) {delete hist1[i]; hist1[i]=NULL;}
    if(hist2[i]!=NULL) {delete hist2[i]; hist2[i]=NULL;}
    //prompt[i] = 0;
  }
...
void CMHbook::Reset()
{
  ...
  for(i=0; i<NHIST; i++)
  {
    if(hist1[i]!=NULL) hist1[i]->Reset();
    if(hist2[i]!=NULL) hist2[i]->Reset();
    //prompt[i] = 0;
  }

  //for(i=0; i<NHIST; i++) tline[i]->Reset();
  printf("CMHbook::Clean reached\n");
...
Int_t CMHbook::CMhisti2root(UThisti *hist, Int_t id)
{
  ...
  if(hist[id].nbinx > 0)
  {
    //printf("CMHisti2root: [%d] nbinx=%d nbiny=%d\n",id,hist[id].nbinx,hist[id].nbiny);
    if(hist[id].nbiny == 0)
    {
      if(hist1[id] == NULL)
      {
		/*NOTE: title cannot contains [] !!!*/
        sprintf(title,"hist1_%04d",id);
        //printf("=======> title >%s<\n",title);
        //printf("=======> hist title >%s<\n",hist[id].title);
        ...
        if(pads[id] != NULL)
        {
          //printf("01[%3d] -> 0x%08x 0x%08x\n",id,hist1[id],pads[id]);fflush(stdout);
          pads[id]->cd();
          //printf("CMHbook::CMhisti2root: 111\n");fflush(stdout);

//hist1[id]->Reset(""); does not help


          hist1[id]->Draw(); // *** Break *** segmentation violation !!!!!!!!!!!!!!!!!!
          //printf("CMHbook::CMhisti2root: 222\n");fflush(stdout);
        }
      }

      if(prompt[id]) /* do not accumulate */
      {
/*printf("do not accumulate ..\n");*/
      ...
      {
		/*
printf("accumulate .. 0x%08x 0x%08x\n",hist[id],hist1[id]);
printf("accumulate: %d\n",hist[id].entries);
printf("accumulate: %d\n",hist1[id]->GetEntries());
		*/
    ...
    //printf("CMHbook::CMhisti2root: ret 1\n");
    return(1);
  }

  //printf("CMHbook::CMhisti2root: ret 0\n");
  return(0);
}
...
Int_t CMHbook::CMhistf2root(UThistf *hist, Int_t id)
{
  ...
  if(hist[id].nbinx > 0)
  {
    //printf("CMhistf2root: [%d] nbinx=%d nbiny=%d\n",id,hist[id].nbinx,hist[id].nbiny);
    if(hist[id].nbiny == 0)
    {
      if(hist1[id] == NULL)
      {
...
/* swap title back, until right format implemented */
//printf("befor %4.4s\n",(char *)fbuf);
#ifdef Linux
  {
    unsigned char *c1, *c2, *c3, *c4, ctmp;
    c1 = (unsigned char *)fbuf;
    c2 = c1 + 1;
    c3 = c2 + 1;
    c4 = c3 + 1;
    for(i=0; i<histf[id].nwtitle; i++)
    {
      ctmp = *c1;
      *c1 = *c4;
      *c4 = ctmp;
...
Int_t CMHbook::CMgetHISS(int *jw, int ind)
{
  ...
  id = etNnum(jw,ind);
  printf("CMgetHISS: bank 'HISS' id=%d found\n",id);
  if(id <=0 || id >=NHIST)
  {
    printf("CMgetHISS: ERROR id=%d, must be from 1 to %d\n",id,NHIST);
    return(0);
  }

  if(histi[id].nbinx > 0 || histi[id].nbiny > 0)
  {
    printf("CMgetHISS: WARN id=%d already exist - replace it\n",id);
    free(histi[id].title);
    if(histi[id].nbiny == 0)
    {
      free(histi[id].buf);
...
#ifdef Linux
  {
    unsigned char *c1, *c2, ctmp;
    c1 = (unsigned char *)buf2;
    c2 = c1 + 1;
    for(i=0; i<histi[id].nwtitle; i++)
    {
      ctmp = *c1;
      *c1 = *c2;
      *c2 = ctmp;
