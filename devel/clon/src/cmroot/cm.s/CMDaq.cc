...
CMDaq::CMDaq(char *fname, CMDisplay *mDisplay)
{
// Create one CMDaq object
  ...
  /* graphic part */
  m_Display = (CMDisplay *) mDisplay;
  /*printf("CMDaq: gCMDisplay=0x%08x, m_Display=0x%08x\n",gCMDisplay,m_Display);*/
  m_Pad = m_Display->Pad();
  m_Display->SetDaq(this);
  m_Display->SetTitle(fname);
  /***************/

  strcpy(filename,fname);
  strcpy(et_station_name,"ROOT");
  nclient = 0;
  downloaded = 0;
  prestarted = 0;
  active = 0;
...
//_____________________________________________________________________________
CMDaq::CMDaq(char *fname, char *etstationname)
{
// Create one CMDaq object
  ...
  strcpy(filename,fname);
  strcpy(et_station_name,etstationname);
...
// Display histograms and visual scalers
  ...
  {
    et_opened = 0;
    if(m_Display) m_Display->StopLoop();
  }
