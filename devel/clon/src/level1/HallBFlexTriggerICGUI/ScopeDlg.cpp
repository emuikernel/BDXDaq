	...
	case SB_PAGERIGHT:      // Scroll one page right
		{
			...
			if (CurPos < 95)
				CurPos = min(95, CurPos + (int) info.nPage);
		}
		break;
...
void ScopeDlg::OnTimer(UINT nIDEvent) 
{
	...
	if(pFlexRegs->VMERead16(TS_BOARD_ADDRESS + TS_TRIG_STATUS, &status, FALSE))
	{
		if(status & 0x2)
		{
			ReadoutScope();
			...
			{
				pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_STATUS, 0x0000, FALSE);
				pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_STATUS, 0x0001, FALSE);
...
BOOL ScopeDlg::OnInitDialog() 
{
	...
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("E_ECtotE%d", i);
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("E_ECinE%d", i);
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("E_CC%d", i);
	for(i = 0; i < 3; i++) m_sSignalNames[sig++].Format("IC_TOT%d", i);
	for(i = 0; i < 3; i++) m_sSignalNames[sig++].Format("IC_ELE%d", i);
	for(i = 0; i < 3; i++) m_sSignalNames[sig++].Format("IC_VET%d", i);
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("ECE%d", i);
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("ECP%d", i);
	for(i = 1; i <= 6; i++) m_sSignalNames[sig++].Format("ECC%d", i);
	for(i = 1; i <= 11; i++) m_sSignalNames[sig++].Format("LUT_ECC%d", i);
	for(i = 1; i <= 11; i++) m_sSignalNames[sig++].Format("LUT_ECE%d", i);
	for(i = 1; i <= 11; i++) m_sSignalNames[sig++].Format("LUT_ECP%d", i);
	for(i = 0; i < 128; i++) m_iTriggerPattern[i] = TRIGPATTERN_X;
	
	m_scrollScope.SetScrollRange(0, 95);
...
void ScopeDlg::OnPaint() 
{
	...
	for(i = m_scrollScope.GetScrollPos(); i < m_scrollScope.GetScrollPos()+32; i++)
		dc.TextOut(0, 51+(p++)*(TEXT_HEIGHT+1), m_sSignalNames[i]);
...
{
	unsigned short bit_masks[8], ignore_masks[8];
	...
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE3_H, bit_masks[7])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE3_L, bit_masks[6])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE2_H, bit_masks[5])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE2_L, bit_masks[4])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE1_H, bit_masks[3])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE1_L, bit_masks[2])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE0_H, bit_masks[1])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_VALUE0_L, bit_masks[0])) return FALSE;

	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE3_H, ignore_masks[7])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE3_L, ignore_masks[6])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE2_H, ignore_masks[5])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE2_L, ignore_masks[4])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE1_H, ignore_masks[3])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE1_L, ignore_masks[2])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE0_H, ignore_masks[1])) return FALSE;
	if(!pFlexRegs->VMEWrite16(TS_BOARD_ADDRESS + TS_TRIG_INGORE0_L, ignore_masks[0])) return FALSE;
...
{
	unsigned short bits[8], lastbits[8];
	HallBFlexRegisters *pFlexRegs = &(((CHallBFlexTriggerGUIDlg *)GetParent())->m_HallBFlexRegisters);

	unsigned short ScopeTraces[1024];
