...
#define VMESERVER_PORT			6001
#define REQUEST_MSG_SIZE		2048
#define BLK_MAX_LEN				512
...
/*
	for(int i = 0; i < 128; i++)
	{
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER2_H, &ScopeTraces[i*6], FALSE);
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER2_L, &ScopeTraces[i*6+1], FALSE);
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER1_H, &ScopeTraces[i*6+2], FALSE);
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER1_L, &ScopeTraces[i*6+3], FALSE);
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER0_H, &ScopeTraces[i*6+4], FALSE);
		pFlexRegs->VMERead16(V1495_VMEADDRESS + TS_TRIG_BUFFER0_L, &ScopeTraces[i*6+5], FALSE);
	}
*/
typedef struct
{
...
#define TS_ENABLE_SCALERS								0x0800	/* 0->1 resets all scalers. 1 to enable scalers. 0 to disable and allow readout */
...
/*      TS_CFG_LUT_BASE_TOP                             0x6FFF*/

/* address bits 13:10 define trigger bit                      */
/* address bit 9 = 1 => ECC/STOF table, 0 => ECP/STOF table   */
