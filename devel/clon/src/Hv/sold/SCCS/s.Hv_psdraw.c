...
c Name history : 1 0 Hv/sold/Hv_psdraw.c
e
s 00718/00000/00000
    ...
    
{
    ...
    xrad = xc - psl;
    yrad = yc - psb;
    fprintf(stderr, "do oval (l, t, r, b) = (%f, %f, %f, %f)\n(xc, yc) = (%f, %f)\n(xrad, yrad) = (%f, %f)\n",
...
/* if shadow, draw the shadow text first */
    ...
    if (shadow) {
	Hv_PSDrawCenteredString(xl-SHADOW_SHIFT,
				xr-SHADOW_SHIFT,
				yb+SHADOW_SHIFT,
				yt+SHADOW_SHIFT,
				fontsize,
...
/* if shadow, draw the shadow text first */
    ...
    if (shadow) {
	Hv_PSDrawString(x-SHADOW_SHIFT,
			y+SHADOW_SHIFT,
			fontsize,
			fontname,
			string,
...
void Hv_PSDrawSegments(XSegment  *segs,
		       short    n,
		       short    color)
