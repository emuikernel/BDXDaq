h16036
s 00000/00000/00000
d R 1.2 02/08/25 23:21:28 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_xyplotio.c
e
s 03180/00000/00000
d D 1.1 02/08/25 23:21:27 boiarino 1 0
c date and time created 02/08/25 23:21:27 by boiarino
e
u
U
f e 0
...
/* if there are not at least 2 non-comment
   lines then ignore this record */
    ...
    if (numnclines < 2) {
      fprintf(stderr,
	      "Only %d non-comment lines. Ignoring this record.\n",
	      numnclines);
      break;
    }
...
/* read all the lines of the next plot. This includes style lines at the top
   ...
   ADDITIONAL STYLE LINES */
	...
	if (Hv_Contained(line, "style")) {
	  numstylelines++;
	  if (starteddata)
	    fprintf(stderr, "Ignored style line: [%s]\n", line);
	  else
...
/* check wrong number of data points. Pad columns that are missing data*/
	  ...
	  if (i != PlotNumSets) {
	    fprintf(stderr, "WARNING: found line with %d entries, expecting %d\n", i, PlotNumSets);
...
/* read the theissen triangulation records */
  ...
  while (Hv_FindRecord(fp, "THEISSEN", False, line)) {
	  ...
	  fprintf(stderr, "Found Theissen record with %d data points and %d edges \n", numpts, numedge);
...
/* read data */
	  ...
	  for (i = 0; i < numpts; i++) {
	      ...
	      fprintf(stderr, "[%d] %f %f %f\n", i+1, curve->x[i], curve->y[i], curve->z[i]);
	  }
...
/* read edges, store as floats */
	  ...
	  for (i = 0; i < numedge; i++) {
	      ...
	      fprintf(stderr, "[%d] %d %d\n", i+1, (int)(curve->xerr[i]), (int)(curve->yerr[i]));
	  }
...
/* dump the legend position -- no need to if it is attached*/
  ...
  if (plotdata->legenditem != NULL) 
    if (!plotdata->attachlegend) {
      ...
      DumpFloat(fp, True, "legendleft", fx, -1.0e30);
      DumpFloat(fp, True, "legendtop",  fy, -1.0e30);
    }
...
/* first segment should be the "style" word.
  ...
  if (Hv_Contained(key, "order")) {
	...
	PlotNumCurves++;
	fprintf(stderr, "found histo data\n");
      }
...
/* continuation plot? */
    ...
    if (lastplot != NULL) {
      ...
      PlotXlabelLeft = 2.0e30;
      PlotXlabelTop  = 2.0e30;
      PlotYlabelLeft = 2.0e30;
      PlotYlabelTop  = 2.0e30;
    }
  }
...
*****************************************************/
    ...
    RestOfString(&temp);
    Hv_Free(PlotTitle);
    Hv_InitCharStr(&PlotTitle, temp);
  }

  else if (Hv_Contained(key, "xlabel")) {
    RestOfString(&temp);
    Hv_Free(PlotXlab);
    Hv_InitCharStr(&PlotXlab, temp);
  }
  else if (Hv_Contained(key, "ylabel")) {
    Hv_Free(PlotYlab);
    RestOfString(&temp);
    Hv_InitCharStr(&PlotYlab, temp);
  }
...
/* see if it was really a vertical label */
    ...
    if (Hv_Contained(key, "vlabel"))
      MallocPlotLabel(vx, vy, font, color, temp, False);
    else
      MallocPlotLabel(vx, vy, font, color, temp, True);
...
/* default ordering is X Y Y Y ... */
  ...
  PlotLegendLeft = 2.0e30;
  PlotLegendTop =  2.0e30;
...
/* now work on relative position -- assume
  ...
  Hv_SetFRect(&pworld, col*fsize + fgap, (col+1)*fsize - 0.4*fgap,
	      1.0 - (row+1)*fsize + 1.8*fgap, 1.0 - row*fsize - 0.3*fgap);
...
/* find the right Y */
  ...
  if (ycol < 0) {
    fprintf(stderr, "Error in finding y data for curve %d\n", curveID);
    return;
  }
...
/* find the last c before this y */
  ...
  if ((xcol < 0) && (ccol < 0)) {
    fprintf(stderr, "Error in finding x data for curve %d (ycol = %d)\n", curveID, ycol+1);
    return;
  }
...
/* find the right Y */
  ...
  if (n1col < 0) {
    fprintf(stderr, "Error in finding histo data for curve %d\n", curveID);
    return;
  }
  ...
  if (n1col >= 0) {
    ...
    fprintf(stderr, "malloced 1D histo array with %d points\n", npts);
  }
  else {
    fprintf(stderr, "Serious problems in reading histo data. Trust nothing.\n");
    for (i = 0; i < npts; i++)
      curve->n1[i] = 0.0;
  }
...
/* now stuff the styles */
  ...
  fprintf(stderr, "Exiting Stuff1DHisto\n");
}
			 ...
			 short  units)
{
  ...
  case CMUNITS:
    dummy = dummy*scres/2.54;
    break;
    
  case MMUNITS:
    dummy = dummy*scres/25.4;
    break;
  }
			  ...
			  char    *line)
{
  ...
  if (PlotNumSets < 1)
    fprintf(stderr, "No STYLE ORDER line read. Will assume X Y Y Y ... \n");
...
/* dont read a solitary column */
  ...
  if (PlotPlotType == Hv_1DHISTO) {
    fprintf(stderr, "Will malloc %d  1D histo data sets\n", PlotNumSets);
    if (PlotNumSets < 1)
      return 1;
  }
	...
	((PlotNumXSets < 1) && (PlotNumCSets < 1))) {
      fprintf(stderr, "No point to read in %d data sets and %d curves and %d X sets.\n",
...

{
  ...
  curve->polyorder  = Hv_sMax(2, Hv_sMin(10, CurvePolyOrder[cindex]));
  curve->polyorder  = Hv_sMin(curve->npts-1, curve->polyorder);

  curve->legendreorder  = Hv_sMax(2, Hv_sMin(10, CurveLegendreOrder[cindex]));
  curve->legendreorder  = Hv_sMin(curve->npts-1, curve->legendreorder);

  curve->numgauss   = Hv_sMax(1, Hv_sMin(10, CurveNumGauss[cindex]));
