h44580
s 00000/00000/00000
d R 1.2 02/08/25 23:21:13 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_items.c
e
s 02696/00000/00000
d D 1.1 02/08/25 23:21:12 boiarino 1 0
c date and time created 02/08/25 23:21:12 by boiarino
e
u
U
f e 0
...
static void    Hv_TimerDrawItem(XtPointer      xtptr,
				XtIntervalId  *id);
...
/*-----------------------------------------------------------
 * Hv_LockItem
 * locks an item so that it cannot be deleted  
 *-----------------------------------------------------------*/
...
/*-----------------------------------------------------------
 * Hv_UnlockItem
 * unlocks an item so that it can be deleted  
 *-----------------------------------------------------------*/
    ...
    
{
    ...
    XShrinkRegion((Region)newrgn, -2, -2);
...
/* if drawinterval was never created, do so now */
    ...
    Item->drawinterval->intervalid = XtAppAddTimeOut(Hv_appContext,
						     interval,
						     (XtTimerCallbackProc)Hv_TimerDrawItem,
						     (XtPointer)Item); 
    Hv_Flush();
}
    ...
    
{
    ...
    XtRemoveTimeOut(Item->drawinterval->intervalid);
    Hv_Flush();
    Item->drawinterval->interval = 0;
    Item->drawinterval->intervalid = 0;
    Item->drawinterval->timedredrawcount = 0;
    ...
    
{
    ...
    irgn = Hv_CopyItemRegion(Item, True);
    if ((dh != 0) || (dv != 0))
	...
	if (Item != View->containeritem) {
	    ...
	    if ((vrgn != NULL) && (Item != View->feedbackitem) && (View->feedbackitem != NULL))
    ...
    
{
    ...
    hr = View->hotrect;
    fw = HRBPtr->scheme->framewidth + 1;
    fw2 = 2*fw;
    
    Hv_SetRect(HRBPtr->area, hr->left - fw, hr->top - fw, hr->width + fw2, hr->height + fw2);
    return HRBPtr;
}
...
/* does NO redrawing */
/* PRESENTLY ASSUMES mydata field deleted ELSEWHERE */
...
/* offset region, if present */
    ...
    if (Item->region != NULL)
	XOffsetRegion((Region)Item->region, dh, dv);
...
/* offset the height and resize the width */
	...
	Hv_OffsetItem (Item, cx1 - cx2, dv, False);
    }
...
/* specialized region fix? */
		...
		if (temp->fixregion != NULL)
		    ...
		    fxc = (temp->worldarea->xmin + temp->worldarea->xmax)/2.0;
		    fyc = (temp->worldarea->ymin + temp->worldarea->ymax)/2.0;
...
/* now do some Item specific allocation */
	...
	Item->scheme->framewidth = 3;
	Item->scheme->fillcolor -= 1;
	if (View->hotrectborderitem != NULL)
	    Hv_Warning("Presently on one hotrect border  per view is supported.");
	else
	...
	Item->scheme->framewidth  = 5;       /* width of the 3d frame */ 
...
{
    short Top_Color       = Hv_gray14;
    short Bottom_Color    = Hv_gray7;
    short Fill_Color      = Hv_gray11;
    short LightTrim_Color = Hv_gray15;
    short DarkTrim_Color  = Hv_gray4; 
    ...
    *scheme = (Hv_ColorScheme *)Hv_Malloc(sizeof(Hv_ColorScheme));
    ...
    (*scheme)->framewidth  = 3;          /* in this case it is the width of the 3d frame */
...
/* if no placement item was provided, assume previous */
    ...
    if ((Item->type == Hv_TEXTITEM) && !(Hv_CheckBit(Item->drawcontrol, Hv_DONTAUTOSIZE)))
	Hv_SizeTextItem(Item, left+Hv_STRINGEXTRA, top+2*Hv_STRINGEXTRA);
    else
