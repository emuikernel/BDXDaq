h02555
s 00000/00000/00000
d R 1.2 02/08/25 23:21:21 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_rainbowscale.c
e
s 00551/00000/00000
d D 1.1 02/08/25 23:21:20 boiarino 1 0
c date and time created 02/08/25 23:21:20 by boiarino
e
u
U
f e 0
...
/* This file contains functions dealing with rainbow items */
...
static void Hv_EditRBColor(Widget  w);


/*------ variables global for this file --------*/

static   short      newmincolor,   newmaxcolor;
static   Widget     mincolorlabel, maxcolorlabel;
...

{
  ...
  if (data->orientation == Hv_HORIZONTAL)
    ...
    Hv_SetRect(&rect, l, b - cwide, area->width, cwide);
...
/* see if this rainbow is the child of a BOX parent */
  ...
  data->scalefactor = attributes[Hv_SCALEFACTOR].f;
...

{
  ...
  static Widget        dx, dy, min, max, precision, colorwidth;
...
/* the dialog has been created */
  ...
  while ((answer = Hv_DoDialog(dialog, NULL)) > 0) {
    ...
    data->dx  = Hv_sMax(1, Hv_sMin(10, Hv_GetIntText(dx)));
    data->dy  = Hv_sMax(1, Hv_sMin(10, Hv_GetIntText(dy)));
    data->precision  = Hv_sMax(0, Hv_sMin(5, Hv_GetIntText(precision)));
    data->colorwidth = Hv_sMax(1, Hv_sMin(10, Hv_GetIntText(colorwidth)));
...

{
  ...
  static Widget        dialog = NULL;
  static Widget        min, max, precision;
  Widget               dummy, rowcol, rc;
  Hv_RainbowData      *data;
  Hv_Region            oldregion, newregion, totalregion;
  int                  answer;
...
/* only write if at least two labels */
  ...
  for (i = 0; i < data->numlab; i++) {
	...
	Hv_NumberToString(tstr, val, data->precision, 1.0e-6);

    xmst = Hv_CreateAndSizeMotifString(tstr, Hv_fixed2, &w, &h);
    XmStringFree(xmst);
    
    if (data->orientation == Hv_HORIZONTAL) {
      yt = Item->area->bottom + 4 + Hv_STRINGEXTRA;
      xt = Item->area->left + (short)Hv_nint(((val-minf)*(Item->area->width))/(maxf-minf));
      xt = xt - (w/2);
    }
    else {
      xt = Item->area->right + 4;
      yt = Item->area->bottom - (short)Hv_nint(((val-minf)*(Item->area->height))/(maxf-minf));
      yt = yt - (h/2);
    }
