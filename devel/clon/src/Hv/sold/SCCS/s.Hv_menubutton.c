h16501
s 00000/00000/00000
d R 1.2 02/08/25 23:21:17 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_menubutton.c
e
s 00955/00000/00000
d D 1.1 02/08/25 23:21:16 boiarino 1 0
c date and time created 02/08/25 23:21:16 by boiarino
e
u
U
f e 0
...
/******* add all the entries ***********/
    ...
    if (mbd->hasactivearea)
      Item->area->width  = Hv_sMax(20, attributes[Hv_MBICONWIDTH].s +
				   Hv_MBACTIVEWIDTH + Hv_MBBUTACTRECTGAP + 3);
    else
      Item->area->width  = Hv_sMax(20, attributes[Hv_MBICONWIDTH].s + 3);
    
    Item->area->height = Hv_sMax(20, attributes[Hv_MBICONHEIGHT].s + 4);
    break;
...
/***********************************************
  Called when an Hv_MENUBUTTOM is deleted. As
...
/* always the right part of area is marked to
  ...
  if (Item->state == Hv_OFF) {
    ...
    if (mbd->hasactivearea) {
      ...
      Hv_SetRect(&rect2, xc-1, yc-5, 3, 10);
...
/* fill the background */
    ...
    if (mbd->hasactivearea) {
      ...
      Hv_SetRect(&rect, xc-2, yc-5, 4, 10);
  ...
  case Hv_TEXTBASED:
    if (mbd->estyles[mbd->current -1] != Hv_SEPARATORTAG) {
      ...
      Hv_DrawCompoundString(xc - sw/2, yc - sh/2, str);
    }
...
/* final check: shouldn't be needed but it
  ...
  if (!Hv_PointInRect(pp, &(mbd->display))) {
    Hv_OffsetRect(&(mbd->display),
		  pp.x - mbd->display.left - Hv_MBPAD,
		  pp.y - mbd->display.top - Hv_MBPAD);
    fprintf(stderr, "Benign warning: seemingly unnecessary offset in Hv_SetMenuButtonDisplayRec.\n");
  }
}
...
/* save what is under the display of choices */
  ...
  pmap = Hv_SaveUnder(mbd->display.left, mbd->display.top,
		      mbd->display.width, mbd->display.height);
...
/* restore */
  ...
  Hv_RestoreSaveUnder(pmap, mbd->display.left, mbd->display.top,
		      mbd->display.width, mbd->display.height);
  Hv_FreePixmap(pmap);
...

{
  ...
  case Hv_TEXTBASED:
    ...
    for (i = 0; i < mbd->numrow; i++) {
      ...
      if (mbd->estyles[i] == Hv_SEPARATORTAG) {
	Hv_DrawLine(rect.left+3, rect.top+3, rect.right-3, rect.top+3, Hv_black);
	Hv_DrawLine(rect.left+3, rect.top+4, rect.right-3, rect.top+4, Hv_white);
      }
      else {
	str = mbd->entries[i];
	Hv_CompoundStringDimension(str, &sw, &sh);
	Hv_DrawCompoundString(rect.left+Hv_MBMARGIN, rect.top+Hv_MBMARGIN, str);
      }
    }
  ...
  case Hv_ICONBASED:
	...
	Hv_IconDraw(Item, NULL, choice, &rect);
	Hv_FullClip(); 
      }
    break;
