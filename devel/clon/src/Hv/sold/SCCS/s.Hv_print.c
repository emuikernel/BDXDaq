...
c Name history : 1 0 Hv/sold/Hv_print.c
e
s 00341/00000/00000
...
static void Hv_PrintToFileCB(Widget w);

static void Hv_UseFooterCB(Widget w);
...
/* dialog has ben created */
  ...
  Hv_PosSetActiveRadioButton(prcmd, lplist);

  Hv_PosSetActiveRadioButton(Hv_psOrientation, orientlist);
  if (Hv_psEntireView)
...
/* a delay so that we can run to the printer? */
    ...
    if (delaytime > 0) {
      ...
      fprintf(stderr, "paused for %d seconds before issuing print command...\n", delaytime);
