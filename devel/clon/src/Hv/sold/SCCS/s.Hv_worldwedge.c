...
c Name history : 1 0 Hv/sold/Hv_worldwedge.c
e
s 01495/00000/00000
...
/* This file contains functions dealing with world wedges */
...
static Widget     linecolorlabel, fillcolorlabel;
     ...
     
{
  ...
  if (rotate) {
    Hv_LatLongToXY(View, &xc, &yc, latitude, longitude);
    sa = sin(azimuth);
    ca = cos(azimuth);
  }
  ...
  for (i = 0; i < np; i++) {
    ...
    rinv = x*x/xrad/xrad + y*y/yrad/yrad;
    radius = sqrt(1.0/rinv);

/*    radius = sqrt(x*x + y*y); */

    Hv_OffsetLatLong(latitude, longitude, radius, azim,
		     &(map->latitude[i]), &(map->longitude[i]));

    if (rotate) {
      Hv_LatLongToXY(View, &xo, &yo, map->latitude[i], map->longitude[i]);
      xo -= xc;
      yo -= yc;
      x = xo*ca + yo*sa;
      y = yo*ca - xo*sa;
      x += xc;
      y += yc;
      Hv_XYToLatLong(View, x, y, &(map->latitude[i]), &(map->longitude[i]));
    }


/* check the args to the log function */
    
    latarg = tan(0.5*(Hv_PIOVER2 + map->latitude[i]));
    if (latarg < 1.0e-20)
      latarg = 1.0e-20;
    
    map->latitude[i] = log(latarg);  /* convert the latitude to a y value */
...
/* these are used in "menu history" */
  ...
  float  convfact[]  = {1.0, 1.0, 1.854, 1.609, 1.0};
...
/* if map view, set extra fields */
  ...
  while ((answer = Hv_DoDialog(dialog, NULL)) > 0) {
	...
	ww->radius = fabs(Hv_GetFloatText(outerradw)*convfact[units]);
	ww->innerrad = fabs(Hv_GetFloatText(innerradw)*convfact[units]);
	ww->innerrad = Hv_fMin(ww->innerrad, 0.95*(ww->radius));
      }
      else {
	Item->azimuth = Hv_DEGTORAD*Hv_GetFloatText(azimuthw);
	ww->xrad = fabs(Hv_GetFloatText(xradw)*convfact[units]);
	ww->yrad = fabs(Hv_GetFloatText(yradw)*convfact[units]);
      }
    }
...
void  Hv_DrawWorldWedgeItem(Hv_Item    Item,
	...
	Hv_SetRect(Item->rotaterect, rrp.x-3, rrp.y-3, 7, 7);
...
void Hv_DrawWorldWedgeButton(Hv_Item     Item,
  ...
  Hv_DrawLine(xc, area.bottom-5, xc-7, area.top+6, Hv_white);
  Hv_DrawLine(xc+1, area.bottom-5, xc+8, area.top+6, Hv_black);
...

{
  ...
  if (Hv_offsetDueToDrag) { /* basepoint stores lat,long in h,v */
      Hv_LatLongToLocal(Item->view, &x, &y, ww->basepoint.h, ww->basepoint.v);
      Hv_LocalToLatLong(Item->view, x+dh, y+dv, &(ww->basepoint.h), &(ww->basepoint.v));
  }
...
/* what we do here depends on whether we
   are in a map view */
	...
	dx = (ww->radius)*cos(ang);
	dy = (ww->radius)*sin(ang);
	fp[i].h = fp[0].h + dx;
	fp[i].v = fp[0].v + dy;
      }
...
/* get the rotate rect point */
    ...
    if (View->mapdata != NULL) {
      ...
      rrx = fp[0].h + (ww->radius)*cos(ang);
      rry = fp[0].v + (ww->radius)*sin(ang);
...
static void Hv_EditWorldWedgeColor (Widget w)
...
static void Hv_EditWorldWedgePattern(Widget w)
