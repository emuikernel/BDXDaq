...
c Name history : 1 0 Hv/sold/Hv_xyplot.c
e
s 03529/00000/00000
...
#define  Hv_BIGBUTTONWIDTH       48
#define  Hv_BIGBUTTONHEIGHT      28
...
/* do we need to create a plot data record? */
	...
	xmin = attributes[Hv_XMIN].f;
	xmax = attributes[Hv_XMAX].f;
	ymin = attributes[Hv_YMIN].f;
	ymax = attributes[Hv_YMAX].f;
	
	if (xmax < xmin) {
	    if (fabs(xmin) < 1.0e-20)
		xmax = 1.0;
	    else
		xmax = xmin + (0.1*fabs(xmin));
	}
...
/* supply a title if none was provided */
    ...
    Hv_VaCreateView(&View,
		    Hv_TAG,            Hv_XYPLOTVIEW,
		    Hv_DRAWCONTROL,    Hv_ZOOMABLE + Hv_ENLARGEABLE +
		    Hv_SCROLLABLE,
		    Hv_POPUPCONTROL,   Hv_CUSTOMIZEVIEW + Hv_ZOOMRESIZETOGGLE +
		    Hv_DELETEVIEW + Hv_PRINTVIEW +
		    Hv_UNDOVIEW + Hv_SENDBACKVIEW +
		    Hv_REFRESHVIEW,
		    Hv_TITLE,          tchr,
    ...
    
{
	...
	Hv_PlotDataMinMax(tcurve->x, tcurve->npts, tcurve->xscale, &(tcurve->xmin), &(tcurve->xmax));
	Hv_PlotDataMinMax(tcurve->y, tcurve->npts, tcurve->yscale, &(tcurve->ymin), &(tcurve->ymax));
	...
	if (plotdata->ylog)
	    ...
	    ymin = 0.0;
	ymax = Hv_ConvertToANiceValue(Hv_fMin(1.0e-20, ymax));
    }
...
/* see if y/x can change according to the
   value of the y/x nozoom field */
	...
	Hv_SelectRect(&SelectRect, LimitRect, pp, event->xbutton.button,
		      Hv_FIXEDYPOLICY, 0.0);
	...
	Hv_SelectRect(&SelectRect, LimitRect, pp, event->xbutton.button,
		      Hv_NOSELECTPOLICY, 0.0);
    
    Hv_ZoomPlotToRect(plot, &SelectRect);
}
    ...
    
{
    ...
    pvdata->plotxmin = -2.0e30;
    pvdata->plotxmax =  2.0e30;
    pvdata->plotymin = -2.0e30;
    pvdata->plotymax =  2.0e30;
    pvdata->plotxlog = False;
    pvdata->plotylog = False;
    pvdata->plotlegendleft = 2.0e30;
    pvdata->plotlegendtop =  2.0e30;
    pvdata->data = NULL;
}
    ...
    
{
    ...
    fcolor = plot->view->hotrectborderitem->scheme->fillcolor;
...
/* no need to go outside the plot */
    ...
    if (sx2 <= sx1) {
	fprintf(stderr, "[%d] [%d] [%f] [%f] premature return in PlotSegment\n", sx1, sx2, x1, x2);
	return;
    }
...
/* no need to go outside the plot */
    ...
    if (sx2 <= sx1) {
	fprintf(stderr, "premature return in PSPlotSegment\n");
	return;
    }
			     ...
			     short gap)
{
    ...
    Hv_DrawCompoundString(x, y + *h + gap, hvstr);
    Hv_DestroyString(hvstr);
...
/* background used for standard plots only */
	...
	if ((ybg > t) && (ybg < b)) {
	    Hv_SetRect(&bgrect, l+1, ybg, r-l-2, b-ybg-1);
...
/* still call drawcurve for fits */
	...
	fprintf(stderr, "drawing special plot\n");
...
/* convert the data to plot coordinates */
	...
	Hv_DrawLines(pp, n, curve->linecolor);
	break;
	...
	for (i = 0; i < n; i++) {
	    ...
	    if (j == 0)
		Hv_SetPoint(ppstair+j,   pp[0].x - delR,   pp[i].y);
	    else
		Hv_SetPoint(ppstair+j,   ppstair[j-1].x,   pp[i].y);
	    Hv_SetPoint(ppstair+j+1, pp[i].x + delR, pp[i].y);
	}
	
	Hv_DrawLines(ppstair, nstair, curve->linecolor);
	Hv_Free(ppstair);
	break;
...
/* check ordering */
	...
	xdel = (plotdata->xmax - plotdata->xmin)/30.0;
...
/* if an error occurred, try svdfit */
		...
		if (Hv_nrerror) {
		    Hv_nrerror = False;
		    fprintf(stderr, "NR routine lfit failed. Trying svdfit.\n");
...
/* error bars ? */
	...
	for (i = 0; i < n; i++) {
	    ...
	    if (plotdata->ebartops) {
		Hv_SetSegment(errseg+i+n, x1, y1-4, x1, y1+4);
		Hv_SetSegment(errseg+i+(2*n), x2, y2-4, x2, y2+4);
	    }
	}
	
	Hv_DrawSegments(errseg, nseg, curve->linecolor);
	Hv_Free(errseg);
    }
	...
	for (i = 0; i < n; i++) {
	    ...
	    Hv_SetSegment(errseg+i, x1, y1, x2, y2);
	    if (plotdata->ebartops) {
		Hv_SetSegment(errseg+i+n, x1-4, y1, x1+4, y1);
		Hv_SetSegment(errseg+i+(2*n), x2-4, y2, x2+4, y2);
	    }
	}
...
/* add the standard set of tools */
    ...
    View->hotrectborderitem->scheme->bottomcolor = Hv_lavender;
    
    x = rightmost->area->right;
    ...
    
{
    ...
    x = placementitem->area->right+10;
    y = placementitem->area->top+2;
    
    Item = Hv_VaCreateItem(View,
			   Hv_FEEDBACKDATACOLS,    30,
...
/*stuff the feedback labels*/
    ...
    Hv_AddFeedbackEntry(Item, Hv_XYPLOTDATA, text0,  Hv_fixed2, Hv_red);
    return Item;
}
    ...
    
{
    ...
    Hv_SetRect(rects[1], r-slop, t,      slop, slop);
    Hv_SetRect(rects[2], r-slop, b-slop, slop, slop);
    Hv_SetRect(rects[3], l,      b-slop, slop, slop);
}
...
/* now the histo slider */
	...
	if ((plotdata == NULL) || (plotdata->plottype != Hv_1DHISTO))
	    ...
	    fdel = (plotdata->histoxmax - plotdata->histoxmin)*2.0;
