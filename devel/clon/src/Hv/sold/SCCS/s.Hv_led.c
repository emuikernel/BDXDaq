...
c Name history : 1 0 Hv/sold/Hv_led.c
e
s 00345/00000/00000
...

{
  ...
  for (i = 0; i < led->numled; i++) {
    ...
    if (led->orientation == Hv_HORIZONTAL) {
      Hv_DrawCompoundString(x + (Hv_LEDWIDTH - w)/2, rect.bottom+1, hvstr);
      x += (Hv_LEDWIDTH + Hv_LEDGAP);
    }
    else {
      Hv_DrawCompoundString(rect.right+2, y+(Hv_LEDHEIGHT-h)/2, hvstr);
...
static void Hv_EditLEDColor(Widget w)
