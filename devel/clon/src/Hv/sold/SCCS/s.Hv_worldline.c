...
c Name history : 1 0 Hv/sold/Hv_worldline.c
e
s 00834/00000/00000
...
static void Hv_EditWorldLineColor (Widget  w);

static void Hv_CheckWorldLineEnlarge(Hv_Item  Item,
				     Hv_Point  pp,
				     Boolean  Shifted,
				     Boolean  *enlarged);
...
/* item edit for Hv_WORLDLINEITEMs  */
...

{
  ...
  Hv_DrawLine(l, yc-1, r, yc-1, Hv_black);
  Hv_DrawLine(l, yc,   r, yc,   Hv_lavender);
  Hv_DrawLine(l, yc+1, r, yc+1, Hv_white);
  Hv_DrawLine(l, yc+2, r, yc+2, Hv_black);
}
...
/* the mutilplexing is used for multiple connections
	...
	for (i = 1; i <= 4; i++) { 
	  if ((wline->multiplexcolors[i] > -1) && (Hv_CheckBitPosition(wline->multiplexbits, i))) {
	    ...
	    if (np > 2)
	      Hv_FillPolygon(poly, np, False, wline->multiplexcolors[i], Hv_black);
	  }
	} 
      }  /* end multiplexed */
      else {
   ...
   normal arrows have styles starting at 100 */
	...
	if (wline->arrowstyle >= Hv_ARROWATEND) {
	  ...
	  if (wline->arrowstyle == Hv_ARROWATBOTH) {
	    ...
	    Hv_FillPolygon(poly, np, False, Item->color, Hv_black);
	  ...
	  if (np > 2)
	    Hv_FillPolygon(poly, np, False, wline->arrowcolor, Hv_black);
	}
      }
    }
...

{
  ...
  Item->region =  Hv_CreateRegionFromLine(x1, y1, x2, y2, wline->thickness+1);
  ...
  if (wline->arrowstyle > 0) {
	...
	if ((wline->multiplexcolors[i] > -1) && (Hv_CheckBitPosition(wline->multiplexbits, i))) {
	  Hv_GetArrowPolygon(Item, poly, &np, i);
	  if (np > 2) {
	    polyreg = Hv_CreateRegionFromPolygon(poly, np);
...
/* added in 10041 -- use standard enlarge default if none provided */
  ...
  startx = attributes[Hv_STARTX].f;
  starty = attributes[Hv_STARTY].f;
  stopx  = attributes[Hv_STOPX].f;
  stopy  = attributes[Hv_STOPY].f;
				     ...
				     Boolean  *enlarged)
{
  ...
  *enlarged = False;
  ...
  Hv_GetLineCorner(wline->start, Item->area, &p1);
  Hv_GetLineCorner(wline->stop,  Item->area, &p2);
  Hv_SetRect(&tempr1, p1.x-slop2, p1.y-slop2, slop, slop);
  Hv_SetRect(&tempr2, p2.x-slop2, p2.y-slop2, slop, slop);
  Hv_CopyRect(&limitrect, View->hotrect);
...
/* fp1 is the "start" point */
  ...
  *np = 3;
  ...
  Hv_SetPoint(poly+1, pc.x-10, pc.y-6);
  Hv_SetPoint(poly+2, pc.x-10, pc.y);
