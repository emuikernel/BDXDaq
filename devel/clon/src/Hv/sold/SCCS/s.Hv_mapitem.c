h17311
s 00000/00000/00000
d R 1.2 02/08/25 23:21:15 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_mapitem.c
e
s 01897/00000/00000
d D 1.1 02/08/25 23:21:14 boiarino 1 0
c date and time created 02/08/25 23:21:14 by boiarino
e
u
U
f e 0
...
static void Hv_BuildLocalMaps(Hv_View  View);
...

{
  ...
  if (View->mapdata != NULL) {
    fprintf(stderr, "Warning: attempt to malloc an additional Hv_ViewMapDataRec.\n");
    return;
  }

  mdata = (Hv_ViewMapData)Hv_Malloc(sizeof(Hv_ViewMapDataRec));

  mdata->lambda0 = Hv_DEGTORAD*12.0;
  mdata->phi1 = Hv_DEGTORAD*30.0;
...
/* set default map colors */
  ...
  mdata->waterfill = View->hotrectborderitem->scheme->fillcolor;
...

{
  ...
  totreg = Hv_CreateRegion();

  for (temp = mdata->landmaps; temp != NULL; temp = temp->next) 

    if (temp->numpoints >= MINPOINTDRAW) {
      polyreg = Hv_CreateRegionFromPolygon(temp->mappoly, temp->numpoints);
      inhrreg = Hv_IntersectRegion(hotrectreg, polyreg);
      Hv_AddRegionToRegion(&totreg, inhrreg);

      Hv_DestroyRegion(polyreg);
      Hv_DestroyRegion(inhrreg);
   }  /* end if numpoints > */
...

{
  ...
  Hv_SetFRect(Item->worldarea, -3.2, 3.2, -1.6, 1.6);
...
/* undo zoom is no possible */
  ...
  View->lastworld = NULL;
  XtSetSensitive(Hv_popupItems[Hv_FirstSetBit(Hv_UNDOZOOM)], False);
  ...
  case Hv_MERCATOR:
    View->minzoomw = Hv_MERCATORMINW;
    View->maxzoomw = Hv_MERCATORMAXW;
    View->minzoomh = Hv_MERCATORMINH;
    View->maxzoomh = Hv_MERCATORMAXH;
  ...
  case Hv_ORTHOGRAPHIC:
    View->minzoomw = Hv_ORTHOGRAPHICMINW;
    View->maxzoomw = Hv_ORTHOGRAPHICMAXW;
    View->minzoomh = Hv_ORTHOGRAPHICMINH;
    View->maxzoomh = Hv_ORTHOGRAPHICMAXH;
...

{
  ...
  dellat  = (latmax  - latmin)/10.0;
  dellong = (longmax - longmin)/10.0;
...

{
  ...
  if (mdata->landmaps != NULL) {
    fprintf(stderr, "Warning! attempted to build over existing local maps!\n");
    return;
  }
...
/* a final check that the map doesn't encompass the hotrect */
  ...
  Hv_SetPoint(&hrc, (View->hotrect->left + View->hotrect->right)/2,
     (View->hotrect->top + View->hotrect->bottom)/2);
 ...
 isvisible:
  ...
  default:
    fprintf(stderr, "Unknown map type encountered. Exiting.\n");
    exit(-1);
  }
...
/* must undo the mercator y, and count the number of good points */
    ...
    Hv_LatLongToXY(View, &(x[i]), &(y[i]), lat, map->longitude[i]);
...
/*  fprintf(stderr, "np = %d  1stgood = %d ngood = %d   nreq = %d\n",
	  np, firstgood, ngood, nreq); */
...
/*fprintf(stderr, "jm1 = %d y = %f x = %f\n", jm1, y[jm1], x[jm1]); */

  theta = atan2(goody[jm1], goodx[jm1]);
  goodx[j] = mdata->radius*cos(theta);
  goody[j] = mdata->radius*sin(theta);
...
/* second point can be filled in now */
  ...
  goodx[j] = r2*cos(ctheta1);
  goody[j] = r2*sin(ctheta1);
/*fprintf(stderr, "j = %d CP1 [%f, %f]\n",
	j, goodx[j], goody[j]); */
  j++;
...
/* next block is where we go to complete the connector */
 ...
 connect:
  ...
  theta = atan2(y[nextgood], x[nextgood]);
  goodx[j+2] = mdata->radius*cos(theta);
  goody[j+2] = mdata->radius*sin(theta);

  ctheta2 = Hv_NearestCompassAngle(theta);
  goodx[j+1] = r2*cos(ctheta2);
  goody[j+1] = r2*sin(ctheta2);

  cctheta = Hv_ConnectingCompassAngle(ctheta1, ctheta2);
  goodx[j] = r2*cos(cctheta);
  goody[j] = r2*sin(cctheta);
...
/* if nextgood is firstgood, we have ended on a string of bads
  ...
  if (nextgood != firstgood)
    goto step2;
/*  else
    fprintf(stderr, "ended on a bad string.\n\n\n");
*/
...
/* next is the "end of data" block */
 ...
 eod:
  ...
  if (j != nreq)
    fprintf(stderr, "error in Hv_MakeAGoodMap j = %d   nreq = %d\n", j, nreq);
...

{
  ...
  Hv_ShrinkRect(&LimitRect, -LIMITRECTDEL, -LIMITRECTDEL);
...
/* use oldfx to watch for map changing from one side to the other */
  ...
  for (i = ChangeSidePoint; i < map->numpoints; i++) {
...
/* note: we stored the mercator y value rather than the latitude */
    ...
    if (useit) {
      xx = local->left + rx*(fx - world->xmin);
      yy = local->top  + ry*(world->ymax - map->latitude[i]);
...

{
  ...
  switch (mode) {
  case FRAMEMAP:
    Hv_DrawLines(map->mappoly, map->numpoints, framecolor);
    break;

  case FILLMAP:
    if ((map->numpoints < MINPOINTDRAW) && (map->type != Hv_SINGLEMAP))
      return;
    Hv_FillPolygon(map->mappoly, map->numpoints, False, fillcolor, framecolor);
    break;

  case FRAMEANDFILLMAP:
    if ((map->numpoints < MINPOINTDRAW) && (map->type != Hv_SINGLEMAP))
      return;
    Hv_FillPolygon(map->mappoly, map->numpoints, True, fillcolor, framecolor);
    break;
...
/*      tlong = Hv_fMin(179.99, Hv_fMax(-179.99, tlong)); */
    ...
    if (j > 1)
      Hv_DrawLines(pp, j, mdata->gridcolor);
...
{
  float          latmax = 85.01;
  float          longmax = 180.01;
  float          longmin = -179.01;
  float          tlat, tlong;
  short          xt, yt, xs, ys;
  Hv_Rect        hr;
  Hv_String      *hvstr;
