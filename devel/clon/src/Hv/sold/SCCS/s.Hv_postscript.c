h56043
s 00000/00000/00000
d R 1.2 02/08/25 23:21:19 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_postscript.c
e
s 01613/00000/00000
d D 1.1 02/08/25 23:21:18 boiarino 1 0
c date and time created 02/08/25 23:21:18 by boiarino
e
u
U
f e 0
...
/* bb must be scaled so we do not have too much white space */
  ...
  bb->xmin *= scalex;
  bb->xmax *= scaley;
  offset = (bb->xmin + bb->xmax - Hv_psWorld.width)*0.5;   /* CEO */
  offset = offset - 72.*Hv_PSLEFTMARGIN;                   /* CEO */
  bb->xmin -= offset;                                      /* CEO */
  bb->xmax -= offset;                                      /* CEO */
  
  bb->ymin *= scaley;
  bb->ymax *= scaley;
  offset = (bb->ymin + bb->ymax - Hv_psWorld.height)*0.5;   /* CEO */
  offset = offset - 72.*Hv_PSBOTTOMMARGIN;                  /* CEO */
  bb->ymin -= offset;                                       /* CEO */
  bb->ymax -= offset;                                       /* CEO */
  Hv_FixFRectWH(bb);

#ifdef PSDEBUG
  fprintf(stderr, "Bounding Rect [%d, %d, %d, %d] [%d, %d] \n", br.left, 
	  br.top, br.right, br.bottom, br.height, br.width);
  
  fprintf (stderr, "%%%%BoundingBox: %f %f %f %f\n",
	   bb->xmin, bb->ymin,
	   bb->xmax, bb->ymax);
#endif
...
void Hv_LocalRectangleToPSLocalRectangle(XRectangle *local,
					 XRectangle *ps)

{
  Hv_LocalToPSLocal(local->x, local->y, &(ps->x), &(ps->y));
  ps->width =  Hv_psResolutionFactor*(local->width);
  ps->height = Hv_psResolutionFactor*(local->height);
}
...

{
  ...
  scale = Hv_psResolutionFactor*72.0/Hv_PSPIXELDENSITY;    /*converts screen pixels to "points" */
...

{
  ...
  if (color == Hv_black) {
    ...
    *r = ((double)cdef.red)   / 65535;   /* get the red   component and scale it between 0 and 1 */
    *g = ((double)cdef.green) / 65535;   /* get the green component and scale it between 0 and 1 */
    *b = ((double)cdef.blue)  / 65535;   /* get the blue  component and scale it between 0 and 1 */
  }
}
...
#ifdef PSDEBUG  
  fprintf(stderr, "  **calling SAVE    \t level: [%d]\n", savelevel);
#endif
...

{
  ...
  if (savelevel > 0)
    ...
    fprintf(stderr, "  **calling RESTORE \t level: [%d]\n", savelevel);
#endif
}
...
#ifdef PSDEBUG
fprintf(stderr, "\tin PSClipPolygon\n");
#endif
...
/*    Hv_psEntireView = False; */  /* CEO added this to center figure */
    ...
    if (!GoodOpen) {
      message = (char *)Hv_Malloc(25 + strlen(Hv_psFileName));
...
/* as of 1.0054, added ability to tile views */
  ...
  if (Hv_psTile) {
      ...
      if (count < 1) {
	  fprintf(stderr, "No nontrivial views open. Not tiling.\n");
      }
...
/* figure out how many rows and columns */
	  ...
	  nc = Hv_iMin(count, Hv_iMax(1, rint(sqrt((double)count))));
	  nr = (count-1)/nc + 1;
...
/* set up some tiling params */
	  ...
	  xg = 0.025*(psworld.width);  /*h gap */
	  yg = 0.025*(psworld.height); /*v gap */
...
/* now draw the view in this reduced area */
		      ...
		      Hv_SetScale(1.0/ps_scalex, 1.0/ps_scaley);
...
/* Hv_psWorld system: a world system (aka PS) in units of 72ths of an inch */
{
  ...
  if (Hv_psOrientation == Hv_PSPORTRAIT) {
    ...
    Hv_SetFRect(&Hv_psWorld,
		  72*Hv_PSLEFTMARGIN, 
		  72*(8.5 - Hv_PSRIGHTMARGIN),
		  72*Hv_PSBOTTOMMARGIN,
		  72*(11.0 - Hv_PSTOPMARGIN));
...
/* I want to fill the page.  Don't limit this to a maximum of 1. CEO */
  ...
  scale = 1.0/Hv_fMax(*scalex, *scaley);
...
static void Hv_SetScale(float scalex,
    ...
    Hv_psUnitsPerInch = 72.0/Hv_fMin(scalex, scaley);
    Hv_PSSetLineWidth(3);
...
/* scale to fit */
  ...
  psxc = (bb->xmin + bb->xmax) / (2.0 * ps_scalex);
  psyc = (bb->ymin + bb->ymax) / (2.0 * ps_scaley);
...
/* first, generate the comments and the prolog */
  ...
  if (Hv_toPrinter) {
      fprintf(stderr, "Printing to printer.");
  }
  else {
      fprintf(stderr, "Printing to file only.");
  }
...
/* if a footer was provided, write it at the bottom (not for eps)*/
  ...
  if (footer != NULL) {
      ...
      Hv_PSDrawHCenteredString(Hv_psWorld.xmin, Hv_psWorld.xmax, Hv_psWorld.ymin-12,
			       9.1, "Times-Roman", footer,  Hv_black);
  }
