...
c Name history : 1 0 Hv/sold/Hv_xyplotedit.c
e
s 01194/00000/00000
...
static void  Hv_EditPlotColors (Widget  w);

static void  Hv_EditCurvePattern(Widget  w);
...
static Hv_Widget       cshowfit, conlegend, cdisplay, cyspike, cusexerrs, cuseyerrs;
...
/* some menu buttons */
	...
	dummy = Hv_StandardActionButtons(rowcol, 110, Hv_ALLACTIONS);
    }
...
/* remove existing list items from the curve selector; add those for
   this plot */
	...
	text[Hv_sMin(17, strlen(tcurve->name))] = '\0';
...
/* The font menus area bit tricky since there is more
   than one so can't use the Hv_hot variables */
	...
	ThePlotData->majxtick = Hv_sMax(0, Hv_sMin(12, Hv_GetIntText(majxtick)));
	ThePlotData->majytick = Hv_sMax(0, Hv_sMin(12, Hv_GetIntText(majytick)));
	ThePlotData->minxtick = Hv_sMax(0, Hv_sMin(12, Hv_GetIntText(minxtick)));
	ThePlotData->minytick = Hv_sMax(0, Hv_sMin(12, Hv_GetIntText(minytick)));
	ThePlotData->xprecision = Hv_sMax(0, Hv_sMin(8, Hv_GetIntText(xprecision)));
	ThePlotData->yprecision = Hv_sMax(0, Hv_sMin(8, Hv_GetIntText(yprecision)));
	
	
	ThePlotData->background = Hv_GetFloatText(background);
	ThePlotData->xtietag = Hv_GetIntText(xtietag);
	ThePlotData->scilowlim = fabs(Hv_GetFloatText(scinotlow));
	ThePlotData->scihilim  = fabs(Hv_GetFloatText(scinothi));
	
	
	ThePlotData->xmin = Hv_GetFloatText(cxmin);
	check = Hv_fMax(1.0e-10, 0.001*fabs(ThePlotData->xmin));
	...
	check = Hv_fMax(1.0e-10, 0.001*fabs(ThePlotData->ymin));
    ...
    
{
    ...
    TheCurve->symbolsize = Hv_sMax(3, Hv_sMin(12, Hv_GetIntText(csymbsize)));
    TheCurve->thickness  = Hv_sMax(1, Hv_sMin(6, Hv_GetIntText(cthickness)));
    TheCurve->polyorder  = Hv_sMax(2, Hv_sMin(10, Hv_GetIntText(cpolyorder)));
    TheCurve->polyorder  = Hv_sMin(TheCurve->npts-1, TheCurve->polyorder);
    TheCurve->legendreorder  = Hv_sMax(2, Hv_sMin(10, Hv_GetIntText(clegendreorder)));
    TheCurve->legendreorder  = Hv_sMin(TheCurve->npts-1, TheCurve->legendreorder);
    
    TheCurve->numgauss   = Hv_sMax(1, Hv_sMin(10, Hv_GetIntText(cnumgauss)));
    ...
    text[Hv_sMin(17, strlen(TheCurve->name))] = '\0';
