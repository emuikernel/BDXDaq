h64243
s 00000/00000/00000
d R 1.2 02/08/25 23:21:25 Codemgr 2 1
c SunPro Code Manager data about conflicts, renames, etc...
c Name history : 1 0 Hv/sold/Hv_virtual.c
e
s 00777/00000/00000
d D 1.1 02/08/25 23:21:24 boiarino 1 0
c date and time created 02/08/25 23:21:24 by boiarino
e
u
U
f e 0
...
/* the hot rect height is fixed, hrh to give aspect
  ...
  if (hrfill < 0) {
    if (Hv_numColors == Hv_EXTENDEDCOLORS)
	hrfill = Hv_wheat+4;
    else
	hrfill = Hv_rosyBrown;
  }
...

{
  ...
  tinyhr.left   = Hv_sMax(Item->area->left+2,   tinyhr.left);
  tinyhr.top    = Hv_sMax(Item->area->top+2,    tinyhr.top);
  tinyhr.right  = Hv_sMin(Item->area->right-2,  tinyhr.right);
  tinyhr.bottom = Hv_sMin(Item->area->bottom-2, tinyhr.bottom);

  Hv_FixRectWH(&tinyhr);

  if((tinyhr.width > 1) && (tinyhr.height > 1))
    Hv_Simple3DRect(&tinyhr, False, VRView->hotrectborderitem->scheme->fillcolor);
}
