 ...
 
{
  ...
  if ( suc_time == header.time )
      ...
      else
	{
	  ...
	  status = map_overwrite(array,item.length*sizeof(float),fd,header.next+ARRAY_VALUES_OFFSET,swapfloatarray);
	  if ( status != MAP_OK )
	    {
	      map_close_map(fd);
	      return status;
      ...
      if ( status < 0 )
	{
      ...
      status = map_write(array,item.length*sizeof(float),fd,&dummy,swapfloatarray);
      if ( status < 0 )
	{
	  map_close_map(fd);
	  return status;
