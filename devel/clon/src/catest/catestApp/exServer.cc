...
{
  pvInfo(1.0e-1,"test1",   10.0f,   0.0f, aitEnumFloat32, excasIoSync, 1u),
  pvInfo(1.0e-1,"test10",  10.0f,   0.0f, aitEnumFloat32, excasIoSync, 2u),
  /*
...
//
pvInfo exServer::billy (-1.0, "billy", 10.0f, -10.0f, aitEnumFloat64,
                                                         excasIoAsync, 1u);
pvInfo exServer::bloaty (-1.0, "bloaty", 10.0f, -10.0f, aitEnumFloat64,
                                                         excasIoSync, 100000u);
        ...
        scanOn ( scanOnIn )
{
  ...
  for(pPVI = exServer::pvList; pPVI < pPVAfter; pPVI++)
  {
    ...
    {
      sprintf(pvAlias, pAliasFmtStr, pvPrefix, pPVI->getName(), i);
      this->installAliasName(*pPVI, pvAlias);
    }
  }


  /* Install create on-the-fly PVs into the PV name hash table */
...
{
  for(unsigned i = 0; i < NELEMENTS(exServer::pvList); i++)
  {
...
//
epicsTimerNotify::expireStatus exAsyncExistIO::expire ( const epicsTime &
                                                              /*currentTime*/ )
{
  //
  // post IO completion
...
// (a virtual function that runs when the base timer expires)
//
epicsTimerNotify::expireStatus exAsyncCreateIO::expire ( const epicsTime &
                                                              /*currentTime*/ )
{
  exPV *pPV;
