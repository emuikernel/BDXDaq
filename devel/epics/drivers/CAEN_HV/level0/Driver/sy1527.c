...
///--------------------------------
    if(ret != CAENHV_OK)
    {
     printf("CAENHVGetChParam error: %s (num. %d)\n\n", CAENHVGetError(name), ret);
...
if(absent_error==0 && Demand[id].board[board].nchannels==0)absent_error=2;
else if(absent_error==0 && mainframes_disconnect[i10]==1)absent_error=3;
///--------
 ...
 char smi_obj_name1[150]; /// temporal
  ...
  else if(absent_error)strcpy(smi_command,"SET_ERROR"); // my_n:
...
pthread_mutex_lock(&global_mutex);
  ...
  tmp2[strlen(tmp2)-strlen(tmp22)-1]=0;
    ...
    {
      sprintf(tmp3,"caput %s_%d_pwonoff %d", tmp2,chan, onoff);
      system(tmp3);
