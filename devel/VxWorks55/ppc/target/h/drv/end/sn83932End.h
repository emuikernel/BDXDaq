...
--------------------
01k,20sep97,pul  added # defines for  new MBLK buffering scheme
01j,15Apr97,sal  Addop for End Driver
...
/* Total number of cluster pools */
#define NUM_CLUSTER_POOLS 2

/* Configurable number of clusters of each type */
#define NUM_OPTIMAL_CLUSTER 32
#define NUM_MTU_CLUSTER 16

/* Configurable number of cluster blks and mblks */
#define NUM_CL_BLK 32
#define NUM_MBLK 32
...
#define MAX_TX_FRAGS  (32)            /* max transmit frags to support */

/* The following values denote the maximum # of RX/TX events to process
 * in one pass (ie. Process up to x received packets before processing up
 * to y transmitted packets.
 */
#define RX_PROCESS_FLOW	    16	 
#define TX_PROCESS_FLOW	    8

/* DATA STRUCTURES */

typedef struct rra_desc                    /* Receive Resource Descriptor */
    {
    unsigned long buff_ptr0;     /* these two point to the starting location */
    unsigned long buff_ptr1;     /* of the receive buffer arear (RBA) */

    unsigned long buff_wc0;      /* these two define the number of 16 bit */
    unsigned long buff_wc1;      /* words that the RBA occupies */
    } RRA_DESC;
...
typedef struct Rx_desc                           /* Receive Descriptor */
    {
    unsigned long status;    /* indicates the status of the received packet */
    unsigned long byte_count;/* length of the complete packet */ 
    unsigned long pkt_ptr0;  /* these two pointers locate the */
    unsigned long pkt_ptr1;  /* packet in the RBA */
    unsigned long seq_no;    /* displays the sequence the RBAs used and the */
			     /* packets buffered */
    struct Rx_desc *link;    /* location of the next receive descriptor */
    unsigned long in_use;    /* ownership of the descriptor */
			     /* zero value: driver (system) is the owner */
		             /* non -zero value: SONIC (device) is the owner */
    } RX_DESC;
...
#define IN_USE      (1)				/*SL*  In use by the device */
#define NOT_IN_USE  (0)				/*SL*  Free to manipulate */
...
#define CAM_DESC_SIZE  sizeof (CAM_DESC)

/* Define size of CAM descriptor area plus a word for the CAM enable */
#define CAM_SIZE (CAM_COUNT * CAM_DESC_SIZE) + 4

typedef struct frag_desc
    {
    unsigned long frag_ptr0;        /* 0,1 pointer to the fragment in */
    unsigned long frag_ptr1;        /* the  transmit buffer area */
    unsigned long frag_size;        /* byte count of the fragment */
    } FRAG_DESC;          /* Fragment descriptors */



typedef struct Tx_desc                  /* Transmit Descriptor */
    {
    unsigned long status;               /* status of the transmission */ 
					/* written into transmit control */
					/* register (TCR) */          
    unsigned long config;               /* programming SONIC to various */
					/* transmiting modes */
    unsigned long pkt_size;             /* byte count of the packet */
    unsigned long frag_count;           /* number of fragments of the packet */
    FRAG_DESC frag[MAX_TX_FRAGS]; 		/*  max Fragment descriptors */
    struct Tx_desc *pLink;		/* OUR real link ptr */
    unsigned long flag;			/* IN USE flag */
    unsigned long number;		/* Descriptor number */
    } TX_DESC;
...
/* State Definitions */
#define SN_POLLING        0x01          /* Poll Mode */
#define SN_PROMISC        0x02          /* Promicuous Mode */
#define SN_ALLMULTI       0x04          /* Allow all multicasts */
#define SN_RECLAIMING     0x80          /* Performing a Reclaim of TXDs */
...
#define XCVR_LB  (0x0600)     /* Transceiver loopback */
...
#define LBK      (0x0002)     /* Loopback packet received */
...
#define EXDIS (0x1000)        /* Disable Excessive Deferral Timer */
#define EXD   (0x0400)        /* Excessive Deferral */
#define DEF   (0x0200)        /* Deferred transmission */
...
#define RDEEN  (0x0040)       /* Receive Descriptors Enable */
...
#define TXDN      (0x0200)    /* Transmission Done */
#define TXER      (0x0100)    /* Transmit Error  */
#define TC        (0x0080)    /* Timer Complete  */
#define RDE       (0x0040)    /* Receive Descriptors  */
