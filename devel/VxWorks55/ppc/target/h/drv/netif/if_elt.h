...
--------------------
01h,19apr02,rcs  added obsolescence warning SPR# 76010 
01g,06mar96,hdn  added ELT_CTRL.
01f,13oct95,hdn  changed reset to elt-reset.
...
#warning "if_elt driver is obsolete, please use elt3c509End driver"
...
typedef struct elt_ctrl
    {
    IDR                 idr;        /* interface data record */

    BOOL                attached;   /* indicates attach() called */
    volatile BOOL       rxHandling; /* flag, indicates netTask active/queued */
    volatile BOOL       txHandling; /* flag, indicates netTask active/queued */
    int                 nRxFrames;  /* how many receive frames to create */
    ELT_FRAME *         pFrameArea; /* whole clump of rcv and transmit frames */
    ELT_FRAME *         pTxFrame;   /* transmit frame buffer (only one) */
    volatile ELT_FRAME *pRxHead;    /* head of received frame list */
    ELT_FRAME *         pRxTail;    /* tail of received frame list */
    volatile ELT_FRAME *pRxCurrent; /* frame currently being received */
    int                 nLoanFrames;/* count of loanable frame buffers */
    int                 rxFilter;   /* current receive address filter bits */
    int                 port;       /* base I/O port of this board */
    int                 ivec;       /* interrupt vector */
    int                 intLevel;   /* interrupt level */
    int                 attachment; /* board connector to use (AUI,BNC,TPE) */
    int                 intMask;    /* current board interrupt mask */
    ELT_STAT            eltStat;    /* statistics */
#ifdef ELT_TIMING
...
extern STATUS	eltattach	(int unit, int port, int ivec, int intLevel,
		  		 int nRxFrames, int attachment, char *ifName);

#else

extern STATUS	eltattach	();
