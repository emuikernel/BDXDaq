...
/* Sergey: cleanup 512M on mv6100 takes about 2 minutes ! */
...
/*Sergey: for mv6100 clean 16MB max, otherwise it takes too long*/
/* why we have to clean ut at all ??? */
...
#else
{
  ...
  if((UINT)SYS_MEM_TOP>0x01000000) size = 0x01000000;
  else                             size = (UINT)SYS_MEM_TOP;

#if     defined (UNCOMPRESS)
/*gorit*/
	fillLongs ((UINT *)((UINT)romInit + ROM_COPY_SIZE),
		    (size - ((UINT)romInit + ROM_COPY_SIZE))
		    / sizeof(long), 0);
#else
/*ne gorit*/
	fillLongs ((UINT *)wrs_kernel_data_end,
		(size - (UINT)wrs_kernel_data_end) / sizeof (long), 0);
#endif 	/* UNCOMPRESS */
}
...
/*gorit*/
    ...
    (absEntry) (startType); /* Sergey: goto usrInit() at all/bootConfig.c ? */
