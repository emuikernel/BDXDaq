...
char txt[100];
/*
sprintf(txt,"kernel stage: 0x10000000 -> 0x%08x\r\n",*(volatile UINT32 *)0x10000000);
...
/*gorit*/
	...
	/*
sprintf(txt,"usrInit: sysHwInit() done\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
/*ne gorit*/
/*(volatile UINT8 *)0xf1100005 = 3;*/
	...
	/*
sprintf(txt,"usrInit: usrKernelInit() done\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
...
#undef USER_I_CACHE_ENABLE	/* XXX disable instruction cache */
	...
	/*
sprintf(txt,"usrInit: cache 1\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    cacheEnable (INSTRUCTION_CACHE);		/* enable instruction cache */
	/*
sprintf(txt,"usrInit: cache 2\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
	...
	/*
sprintf(txt,"usrInit: calling kernelInit() ...\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
	...
	/*
sprintf(txt,"usrRoot 5\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
#ifdef  INCLUDE_TYCODRV_5_2
#ifdef  INCLUDE_TTY_DEV
	/*
sprintf(txt,"usrRoot 6\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    if (NUM_TTY > 0)
        {
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
	...
	/*
sprintf(txt,"usrRoot 8\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/

#ifdef INCLUDE_PC_CONSOLE
	/*
sprintf(txt,"usrRoot 9\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    pcConDrv ();
...
#endif
	/*
sprintf(txt,"usrRoot 18\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/

#ifdef  INCLUDE_PCMCIA                                       
	/*              
sprintf(txt,"usrRoot 19\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
	...
	/*
sprintf(txt,"usrRoot: spawn tBoot ...\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
...
bootCmdLoop (void)
{
  ...
  char txt[100];
  /*
sprintf(txt,"bootCmdLoop reached\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  ...
  /*
sprintf(txt,"bootCmdLoop 1\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  if(sysStartType & BOOT_CLEAR) printBootLogo ();

  /*
sprintf(txt,"bootCmdLoop 2\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  usrBootLineInit(sysStartType);

  /*
sprintf(txt,"bootCmdLoop 3\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  ...
  /*
sprintf(txt,"bootCmdLoop 5\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
#if defined(INCLUDE_TSFS_BOOT) && defined(INCLUDE_TSFS_BOOT_VIO_CONSOLE)

  /*
sprintf(txt,"bootCmdLoop 6\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  /* Wait for Target Server connection */
  while(!wdbTargetIsConnected()) taskDelay (sysClkRateGet());

  /*
sprintf(txt,"bootCmdLoop 7\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  /*
  ...
  /*
sprintf(txt,"bootCmdLoop 8\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  if(!(sysStartType & BOOT_NO_AUTOBOOT) && !(sysFlags & SYSFLG_NO_AUTOBOOT))
  {
	int timeout = TIMEOUT;
	   ...
	   (sysFlags & SYSFLG_QUICK_AUTOBOOT))
	{
	  timeout = 1;
	  /*
sprintf(txt,"bootCmdLoop: timeout=%d\r\n ",timeout);
sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
	}
	/*
sprintf(txt,"bootCmdLoop 81\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    key = autoboot (timeout);	/* doesn't return if successful */
	/*
sprintf(txt,"bootCmdLoop 82\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
  }
  /*
sprintf(txt,"bootCmdLoop 9\r\n ");
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
LOCAL STATUS netLoad(char *hostName, char *fileName, char *usr, char *passwd, FUNCPTR *pEntry)
{
  int fd;
  ...
  {
	if(tftpXfer (hostName, 0, fileName, "get", "binary", &fd, &errFd) == ERROR)
      return(ERROR);
  }
  else
#endif
...
printf ("rcmd: host=>%s< remotePort=%d localUser=>%s< remoteUser=>%s< command >%s<\n",
  hostName,RSHD,usr,usr,command);
#endif
	    fd = rcmd (hostName, RSHD, usr, usr, command, &errFd);
#ifdef MVME5500_DEBUG
printf ("rcmd done, errFd=%d\n",errFd);
#endif
