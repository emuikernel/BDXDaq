...
value = 0 = 0x0
->
interrupt: Done2: 0x08000200 -> 0x00000200 (40)
interrupt: Done2: 0x08000200 -> 0x00000200 (40)
interrupt: Done2: 0x08000200 -> 0x00000200 (40)
.....
...
/*
usrVme2MemDmaListSet: Link-List DMA setup for 6 transfers
        VME Address         Memory Address         Nbytes
  0     0x08210000            0x00628d00            296
  1     0x08220000            0x00628e28            488
  2     0x08230000            0x00629010            40
  3     0x08240000            0x00629038            40
  4     0x08250000            0x00629060            40
  5     0x08260000            0x00629088            104
interrupt: Done1: 0x08000800 -> 0x00000800
...
interrupt: usrVme2MemDmaListSet: Link-List DMA setup for 7 transfers
interrupt:         VME Address         Memory Address         Nbytes
interrupt:   0     0x08210000            0x00628d20            296
interrupt:   1     0x08220000            0x00628e48            488
interrupt:   2     0x08230000            0x00629030            40
interrupt:   3     0x08240000            0x00629058            40
interrupt:   4     0x08250000            0x00629080            40
interrupt:   5     0x08260000            0x006290a8            104
interrupt:   6     0x08270000            0x00629110            40
interrupt: Done1: 0x08000800 -> 0x00000800
...
following parameters can be set for universe:
...
sysVmeDmaCnfgSet
    ...
    UINT32 addrSpace,	 input: VMEbus Address Space type		
			 ...
			 DCTL_VAS_A24					
			 DCTL_VAS_A32					
    UINT32 dataType,	 input: Ptr to Program/Data AM Code		
			 Valid range:					
			 DCTL_PGM_DATA				
			 DCTL_PGM_PRGM				
    UINT32 userType	 input: Supervisor/User AM Code		
...
usrVmeDmaConfig(UINT32 addrType, UINT32 dataType)
{
  ...
  dType  = DCTL_PGM_DATA/*DCTL_PGM_PRGM */;
  userType  = DCTL_SUPER_USER/*DCTL_SUPER_SUP*/;
...
wait_more_for_chain:
  ...
  logMsg("usrVme2MemDmaDone: ERROR: 0x%08x -> 0x%08x 0x%08x 0x%08x 0x%08x\n",
    dgcsReg,
...
static volatile struct univDma_ll uDmaLL[UNIV_DMA_MAX_LL];
...
usrVme2MemDmaListSet(UINT32 *vmeAdrs, UINT32 *locAdrs, int *size, int numt)
{
  ...
  */
  ...
  pciAddr = ((UINT32)(&(uDmaLL[0].dctl))+(UINT32)PCI_SLV_MEM_BUS);
  ...
  if((numt > 0) && (numt <= UNIV_DMA_MAX_LL))
  {
    for(ii=0; ii<numt; ii++)
    {
      ...
      uDmaLL[ii].dctl = LONGSWAP(*UNIVERSE_DCTL);
      uDmaLL[ii].dtbc = size[ii];
	  uDmaLL[ii].dla  = ((UINT32)locAdrs[ii]+(UINT32)PCI_SLV_MEM_BUS);   
      uDmaLL[ii].dva  = vmeAddress[ii];
      if(ii == (numt-1))
      {
	    uDmaLL[ii].dcpp = 1;
      }
      ...
      {
	    temp1 = (UINT32)(&(uDmaLL[ii+1].dctl));
	    uDmaLL[ii].dcpp = (temp1+(UINT32)PCI_SLV_MEM_BUS);
      }
    }
  }
  else
  ...
  /*
  logMsg("usrVme2MemDmaListSet: Link-List DMA setup for %d transfers\n",numt,2,3,4,5,6);
  logMsg("        VME Address         Memory Address         Nbytes\n",1,2,3,4,5,6);
  */
  for(ii=0; ii<numt; ii++)
  {
	/*	
    logMsg("  %d     0x%08x            0x%08x            %d\n",
	   ii, uDmaLL[ii].dva, uDmaLL[ii].dla, uDmaLL[ii].dtbc,5,6);
	*/

    /* Swap the bytes in the structure */
    uDmaLL[ii].dctl = LONGSWAP(uDmaLL[ii].dctl);
    uDmaLL[ii].dtbc = LONGSWAP(uDmaLL[ii].dtbc);
    uDmaLL[ii].dla  = LONGSWAP(uDmaLL[ii].dla);
    uDmaLL[ii].dva  = LONGSWAP(uDmaLL[ii].dva);
    uDmaLL[ii].dcpp = LONGSWAP(uDmaLL[ii].dcpp);
  }
...
usrVmeDmaReset__test()
{
  ...
  *UNIVERSE_DGCS = LONGSWAP (DGCS_STOP_REQ);
  while(1)
  {
    if(usrVmeDmaActive__test() == 0) break;
  }

  *UNIVERSE_DGCS = LONGSWAP (dgcsReg | DGCS_RESET_STAT);
}
...
usrPci2MemDmaStart(UINT32 chan, UINT32 *sourceAddr, UINT32 *destAddr,
                   UINT32 nbytes)
{
  printf("usrPci2MemDmaStart is not supported yet\n");
  return(0);
}
STATUS
usrMem2PciDmaStart(UINT32 chan, UINT32 *sourceAddr, UINT32 *destAddr,
                   UINT32 nbytes)
{
  printf("usrMem2PciDmaStart is not supported yet\n");
  return(0);
}
STATUS
usrPci2PciDmaStart(UINT32 chan, UINT32 *sourceAddr, UINT32 *destAddr,
                   UINT32 nbytes)
{
  printf("usrPci2PciDmaStart is not supported yet\n");
  return(0);
}
