...
--------------------
01d,08oct00,dmw  Fixed range read functions to use dual byte addresses.
01c,08sep00,dmw  Added dual-address I2C EEPROM accesses.
01b,07sep00,dmw  Updated I2C access names.
01a,31aug00,dmw  Written (from version 01a of ppmc750/vpdUtil.c) and
                 add Harrier calls.
*/
...
#define I2C_BYTE_READ(device, offset, bfr, num) \
    sysMotI2cRead(device, offset, bfr, num)
#endif

#ifndef I2C_BYTE_WRITE
IMPORT STATUS sysMotI2cWrite(UCHAR, UCHAR, UCHAR *, UCHAR);
#define I2C_BYTE_WRITE(device, offset, bfr, num) \
    sysMotI2cWrite(device, offset, bfr, num)
#endif

#ifndef I2C_BYTE_RANGE_READ
IMPORT STATUS sysMotI2cRangeRead(UCHAR, USHORT, UINT16, UCHAR *, UCHAR);
#define I2C_BYTE_RANGE_READ(device, offset, count, bfr, num) \
    sysMotI2cRangeRead(device, offset, count, bfr, num)
#endif

#ifndef I2C_BYTE_RANGE_WRITE
IMPORT STATUS sysI2cSromRangeWrite(UCHAR, USHORT, UINT16, UCHAR *);
...
STATUS sysI2cSromRangeRead 
    ...
    USHORT   devOffset,  /* starting offset within the serial eeprom to read */
    ...
    for ( ; byteCount != 0; ++devOffset, ++pBfr, --byteCount)
        {
        if (I2C_BYTE_READ(devAdrs, devOffset, pBfr, I2C_ADDRESS_BYTES) != OK)
            return (ERROR);
        }
    return (OK);
    }
...
STATUS sysI2cSromRangeWrite 
    (
    UCHAR    devAdrs,    /* i2c address of the serial eeprom */
    USHORT   devOffset,  /* starting offset within the serial eeprom to write */
    ...
    for ( ; byteCount != 0; ++devOffset, ++pBfr, --byteCount)
        {
        if (I2C_BYTE_WRITE(devAdrs, devOffset, pBfr, I2C_ADDRESS_BYTES) != OK)
            return (ERROR);
        }
    return (OK);
    }
...
STATUS sysVpdPktInit
    ...
    *pVpdPtr = NULL;
    ...
    if (I2C_BYTE_RANGE_READ (devAdrs, devOffset, sizeof(VPD_HEADER),
                        (UCHAR *)pVpd, I2C_ADDRESS_BYTES) != OK)
        return (ERROR);
    ...
    if (I2C_BYTE_RANGE_READ (devAdrs, devOffset + sizeof(VPD_HEADER),
                        ...
                        (UCHAR *)&pVpd->packets, I2C_ADDRESS_BYTES) != OK)
        return (ERROR);
