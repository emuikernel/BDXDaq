...
--------------------
01b,20jun01,srr  Added support for Nitro.
01a,31aug00,dmw  Written (from version 01c of mv5100/sysCache.c).
*/
...
LOCAL STATUS sysL2InlineCacheInit(void)
    ...
    sysL2crPut(l2crVal);
    SYNC;    /* wait for all memory transactions to finish */
    ...
    /* Enable access to cache functionality to OS */
...
LOCAL UINT sysL2InlineCacheSize()
    ...
    if (CPU_TYPE != CPU_TYPE_750 && CPU_TYPE != CPU_TYPE_MAX &&
        CPU_TYPE != CPU_TYPE_NITRO)
        return(0);    /* Not an Arthur/Max/Nitro chip so no in-line L2 cache */
