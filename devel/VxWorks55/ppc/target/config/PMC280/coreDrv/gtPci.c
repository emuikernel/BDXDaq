...
void gtPci0MapMemory2space(unsigned int pci0Mem2Base, unsigned int pci0Mem2Size)
{
    ...
    /* Sergey (GT64360 page 81): "When Base Address register is written to,
    ...
    interface is the same address presented on the CPU bus."
      ...
      for example: when following command writes to 0xf1000258 (PCI_0_MEMORY2_BASE_ADDR)
	0xf10002f8 (PCI_0_MEMORY2_LOW_ADDR_REMAP) updates automatically;
      for example if 0x00840001 is written to 0xf1000258,
    then 0x00840000 is written into 0xf10002f8 */
        GT_REG_WRITE(PCI_0_MEMORY2_BASE_ADDR,pci0Mem2Base);



	/* SO: if we want to map PCI address 0, we have to cleanup 0xf10002f8 ! */
	/* *(unsigned int *)0xf10002f8 = 0;*/
    adr=gtMemorySetPciRemapValue(PCI_0_MEM2,0,0);
    *(unsigned int *)0x10001000 = adr;
...
void gtPci0MapMemoryBank0(unsigned int pci0Dram0Base,unsigned int pci0Dram0Size)
{
    if(pci0Dram0Size > 0)
    {
        ...
        /*Sergey: comment out following line (need it if we are master only ???)*/
        /*frcPci0WriteConfigReg(PCI_SCS_0_BASE_ADDR_LOW,PCI_SELF,pci0Dram0Base);*/

		/*Sergey: config register offet PCI_SCS_0_BASE_ADDR_LOW=0x10 contains address
        written by host board during enumeration process, we do not want to rewrite it;
        prpmc880 had a procedure to synchronize host and slave using config register
        offset 0x10 (see sysLib.c function prpmcSlaveWaitConfig(), we may need something
        similar here .. */
        /*Sergey: NOTE: PCI_SELF force usage of our local bridge even if it is not 0*/
