...
void dblprint (double d)
    {
    printf ("%d.%d\n", (int)d, (int)((d - (double)(int)d) * 1000));
    }

/*************
**** main ****
*************/
void nbench (int argc, char *argv[])
{
    int i;			/* Index */
 ...
 */
	...
	sprintf (outBuffer, "**Date and time of benchmark run: %s", asctime (loctime));
	output_string (outBuffer);
	sprintf (outBuffer, "**Sizeof: char:%u short:%u int:%u long:%u u8:%u u16:%u u32:%u int32:%u\n",
 ...
 */
	...
	if (tests_to_do[i])
	{
	    sprintf (outBuffer, "%s    :", ftestnames[i]);
	    output_string (outBuffer);
...
#if TRUE
	    sprintf (outBuffer, " %15.5g  :  %9.2g  :  %9.2g\n",
		     bmean, bmean / bindex[i], bmean / lx_bindex[i]);
/*
	    dblprint (bmean);
	    dblprint (bmean / bindex[i]);
	    dblprint (bmean / lx_bindex[i]);
*/
#else
	    sprintf(outBuffer,"  Iterations/sec.: %13.2f  Index: %6.2f\n",
	    ...
	    if (global_allstats)
	    {
		sprintf (outBuffer, "  Absolute standard deviation: %g\n", bstdev);
		output_string (outBuffer);
		if (bmean > (double) 1e-100)
		{
		    /* avoid division by zero */
		    sprintf (outBuffer, "  Relative standard deviation: %g %%\n",
			     (double) 100 * bstdev / bmean);
		    output_string (outBuffer);
		}
		sprintf (outBuffer, "  Number of runs: %lu\n", bnumrun);
		output_string (outBuffer);
		show_stats (i);
		sprintf (outBuffer, "Done with %s\n\n", ftestnames[i]);
		output_string (outBuffer);
	    }
	}
    }
 ...
 */
	...
	sprintf (outBuffer, "INTEGER INDEX       : %.3f\n",
		 pow (intindex, (double) .142857));
	output_string (outBuffer);
	sprintf (outBuffer, "FLOATING-POINT INDEX: %.3f\n",
...
#include "sysinfoc.c"
	sprintf (outBuffer, "MEMORY INDEX        : %.3f\n",
...
/* Here is a list of the student-t distribution up to 29 degrees of
   ...
   degrees of freedom. */
    double student_t[30] =
    {
	0.0, 12.706, 4.303, 3.182, 2.776, 2.571,
...
** return at the end of the outBuffer.
*/
static void output_string (char *outBuffer)
{

    printf ("%s", outBuffer);
    if (write_to_file != 0)
	fprintf (global_ofile, "%s", outBuffer);
    return;
}
...
{
    char outBuffer[80];		/* Display outBuffer */

    switch (bid)
    {
    case TF_NUMSORT:		/* Numeric sort */
	sprintf (outBuffer, "  Number of arrays: %d\n",
		 global_numsortstruct.numarrays);
	output_string (outBuffer);
	sprintf (outBuffer, "  Array size: %ld\n",
		 global_numsortstruct.arraysize);
	output_string (outBuffer);
	break;
	...
	sprintf (outBuffer, "  Operations array size: %ld\n",
		 global_bitopstruct.bitoparraysize);
	output_string (outBuffer);
	sprintf (outBuffer, "  Bitfield array size: %ld\n",
		 global_bitopstruct.bitfieldarraysize);
	output_string (outBuffer);
	break;

	case TF_FPEMU:		/* Floating-point emulation */
	sprintf (outBuffer, "  Number of loops: %lu\n",
		 global_emfloatstruct.loops);
	output_string (outBuffer);
	sprintf (outBuffer, "  Array size: %lu\n",
		 global_emfloatstruct.arraysize);
	output_string (outBuffer);
	break;

	case TF_FFPU:		/* Fourier test */
	sprintf (outBuffer, "  Number of coefficients: %lu\n",
		 global_fourierstruct.arraysize);
	output_string (outBuffer);
	break;

	case TF_ASSIGN:
	sprintf (outBuffer, "  Number of arrays: %lu\n",
		 global_assignstruct.numarrays);
	output_string (outBuffer);
	break;
...
** Udummy1 = This is a 2-byte outBuffer that holds a "*", and acts as the first
