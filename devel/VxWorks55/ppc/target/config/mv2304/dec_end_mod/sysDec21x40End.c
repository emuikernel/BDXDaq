...
#define DEC_USR_FLAGS_140 (DEC_USR_21140 | DEC_USR_MII)
...
STATUS sysDec21x40PciInit (void)
	...
	sprintf(&davedump[0],"pciBus=%d, pciDevice=0x%x pciFunc=0x%x\n",
    ...
    /* DJA -7/2003 - force interrrupt info for netwrork */
#if 0
    irqvec = (UINT32) LN_INT_VEC;
    irqnum = (UINT32) LN_INT_LVL;
#else
    irqvec = (UINT32)intLine;
    irqnum = (UINT32)intLine;
#endif

    sprintf(&davedump[64],"iobaseCsr=0x%x membaseCsr=0x%x irqvec=0x%x irqnum=0x%x\n",
           iobaseCsr,membaseCsr,irqvec,irqnum);
