...
)
{
  ...
  /* Sergey: following note from mvme5500:  Don't confuse bus numbers
  ...
  if(pciLoc->device == PCI_HOST_BRIDGE_DEV_NUM)
  {
#ifdef MYDEBUG
sprintf(txt,"sysPciAutoConfigInclude: exclude host bridge\r\n");
...
)
{
  ...
  for(busInterface = 0; busInterface < maxBusIF; busInterface++)
  {
    ...
    /*pciAutoCfg (&sysParams); above is obsolete ? if use this, bus=2
    become bus=1 - Sergey*/
