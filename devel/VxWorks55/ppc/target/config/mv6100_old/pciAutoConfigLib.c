...
)
{
  ...
  listSize = 0;
  /*
sprintf(txt,"pciMaxBusCalc: calls pciAutoDevProbe for bus=%d\r\n",bus);
  ...
  */
  subBus = pciAutoDevProbe(&pSystem, bus, (UCHAR)0,
                            (UCHAR)( PCI_AUTO_ATTR_BUS_4GB_IO |
                             ...
                             &pPciList, &listSize);
  /*
sprintf(txt,"pciMaxBusCalc: pciAutoDevProbe returns %d\r\n",subBus);
sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
#ifdef MYDEBUG
sprintf(txt,"==> pciAutoCfgFunc: calls pciAutoListCreate 1\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"==> pciAutoCfgFunc: calls pciAutoListCreate 2\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoListCreate: calls pciAutoDevProbe for bus=%d\r\n",pciLoc.bus);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoListCreate: pciAutoDevProbe returns %d\r\n",pciMaxBus);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoBusProbe: calls pciAutoDevProbe for bus=%d\r\n",secBus);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoBusProbe: pciAutoDevProbe returns %d\r\n",subBus);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoFuncEnable: enable device [%d,%d,%d,0x%02x]\r\n",
	    (pFunc->bus),
	    (pFunc->device),
	    (pFunc->function),
	    (pFunc->attribute));
sysDebugMsg(txt,CONTINUE_EXECUTION);
  ...
  PCI_AUTO_DEBUG_MSG("pciAutoFuncEnable: enable device [%d,%d,%d,0x%02x]\n",
	(pFunc->bus),
	(pFunc->device),
	(pFunc->function),
	(pFunc->attribute),
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
	...
	if ((addrInfo & PCI_BAR_SPACE_MASK) == PCI_BAR_SPACE_IO)
	{
	  ...
	  sizeMask = (1 << 2);
#ifdef MYDEBUG
sprintf(txt,"  --> pciAutoFuncConfig: IO Space found at BAR[%d] (mask=0x%08x)\r\n",
  baI,sizeMask);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	}
	else
	{
      ...
      sizeMask = (1 << 4);
#ifdef MYDEBUG
sprintf(txt,"  --> pciAutoFuncConfig: Mem Space found at BAR[%d] (mask=0x%08x)\r\n",
  baI,sizeMask);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
    }
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
  ...
  else
  {
	...
	register64Bit = pciAutoMemAlloc(pSystem, pPciFunc, &addr, nSize, addrInfo);
#ifdef MYDEBUG
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
  ...
  else
  {
	...
	if (retStat == ERROR)
	    {
#ifdef MYDEBUG
sprintf(txt,"000> Warning: PCI Memory allocation failed\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
