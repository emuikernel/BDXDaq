/*
  ...
  $Id: ppp_async.c,v 1.9 1994/05/27 00:40:02 paulus Exp $
*/
...
static PAI pai[NPPP];		/* our private cache of async ctrl structs */
...
static int blah[10];	/* does this make modload happy? */
...
#define	STUFF_CHAR(p,c)	(*(p)->pai_bufftail->b_wptr++ = (c), \
			 (p)->pai_buffcount++)
#define	FLUSHEM(q,p)	(INPUT_ERROR(q), (p)->pai_flags |= PAI_FLAGS_FLUSH)
	...
	for (; mp != NULL; mp = mp->b_cont) {	/* for each message block */
		...
		if (c == PPP_FLAG) {
		    /* if the flush indicator is set, then we have
		    ...
		    if (p->pai_buffcount > PPP_FCSLEN) { /* discard FCS */
			adjmsg(p->pai_buffer, -PPP_FCSLEN);
			p->pai_buffcount -= PPP_FCSLEN;
		    }
			...
			if (p->pai_fcs == PPP_GOODFCS) {
				...
				ppp_dump_frame(p, p->pai_buffer, " got input");
#endif /*DEBUGS*/
...
/* we could be discarding this buffer because of data errors
   ...
   we throw it away and the next allocation will be the correct size */
			}
			p->pai_buffer = NULL;
			p->pai_buffcount  = 0;
		    }
		...
		if (p->pai_bufftail->b_wptr >=
		    p->pai_bufftail->b_datap->db_lim) {
