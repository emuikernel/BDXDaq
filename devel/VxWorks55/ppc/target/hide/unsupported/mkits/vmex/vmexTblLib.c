/* vmexTblLib.c -  local object table manipulation routines */
...
--------------------
01h,22jan97,jmb  Added intLocks in table creation and delete routines because
		 q_*send can do a table lookups at interrupt level.
		 Also added semaphore to table entry remove routine.
01g,03jul96,jmb  VMEX_MAX_TABLES incremented
...
01d,25apr96,jmb  Made the tables static and eliminated the list of tables
                 Also added vmexObjToShadowInt for accessing tables at
...
#define VMEX_MAX_TABLES 5

VMEX_TABLE vmexTableArray[VMEX_MAX_TABLES];

/******************************************************************************
*
* vmexNameToObj - local name table name to object conversion
*
...
void * vmexNameToObj
    (
    VMEX_TABLE *table,  	   /* ptr to table to search */
...
/******************************************************************************
*
* vmexObjToShadow - get the "shadow" of the given vxWorks object
...
/******************************************************************************
*
* vmexObjToShadowInt - get the "shadow" of the given vxWorks object
...
/******************************************************************************
*
* vmexTblRemove - remove an entry from a local object table
*
* INCLUDE FILES: vmexLib.h
*
...
STATUS vmexTblRemove
    ...
    * Don't need to intLock this loop since vmexObjShadowInt does a
...
/******************************************************************************
*
* vmexTblAdd - add an entry to a local object table 
*
...
STATUS vmexTblAdd
    ...
    * Don't need to intLock this loop since vmexObjShadowInt
...
/******************************************************************************
*
* vmexTblCreate - create a local object table
*
...
VMEX_TABLE *vmexTblCreate(
    char * name,
    VMEX_TABLE_NODE * nodeArray,
    int size
    )
    {
    VMEX_TABLE *		rvalue;

    if ( numVmexTables < VMEX_MAX_TABLES )
	rvalue = & vmexTableArray[numVmexTables++];
    else
	rvalue = NULL;
