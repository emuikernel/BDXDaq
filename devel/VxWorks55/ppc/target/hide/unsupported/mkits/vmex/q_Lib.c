...
--------------------
01f,22jan97,jmb  Fix a typo and add casts to get rid of warnings.
01e,21jan97,jmb  Make q_send routines callable from interrupt level.
		 Fix bug handling bad queue id's.
...
IMPORT VMEX_TABLE_NODE vmexFixQArray[];
IMPORT int vmexMaxFixQ;

IMPORT VMEX_TABLE_NODE vmexVarQArray[];
IMPORT int vmexMaxVarQ;

/* Locals */

LOCAL VMEX_TABLE *pFixQTbl;
LOCAL VMEX_TABLE *pVarQTbl;
...
* VMEexec allows a count of zero, but this will fail in vxWorks.
*
* RETURNS:
* EOK if successful.
...
UINT32 q_create
    ...
    /* convert VMEexec options to vxWorks options */
	...
	{
	vmexUnsupported ("Q_NOLIMIT option not supported",
	    0, 0, 0, 0, 0, 0 );
	return (ERROR);
	}
	...
	{
	vmexUnsupported ("Zero-length queues not supported",
	    0, 0, 0, 0, 0, 0 );
	return (ERROR);
	}
    ...
    *qid = (int) msgQCreate (count, 16, vxOptions);
    ...
    if (pFixQTbl == NULL)
        pFixQTbl = vmexTblCreate (nameFixedQTbl, vmexFixQArray, vmexMaxFixQ);

    if (vmexTblAdd (pFixQTbl, NAME_TO_INT(name), (void *) *qid, NULL) == ERROR)
	{
...
UINT32 q_delete
    ...
    */
    if (pFixQTbl)
        rc = (unsigned int) vmexObjToShadow (pFixQTbl, (void *) qid);
    else
	rc = ERROR;
	...
	{
	if (pVarQTbl)
	    if (vmexObjToShadow (pVarQTbl, (void *) qid) != (void *) ERROR)
	        return(ERR_VARQ);
	return(ERR_OBJID);
        }
    ...
    */
    ...
    vmexTblRemove (pFixQTbl, (void *) qid);
...
UINT32 q_send
    ...
    if (!INT_CONTEXT ())
        func_ObjToShadow = (FUNCPTR) vmexObjToShadow;
    else
        func_ObjToShadow = (FUNCPTR) vmexObjToShadowInt;
...
UINT32 q_ident
    ...
    if (pFixQTbl)
        *qid = (int) vmexNameToObj (pFixQTbl, NAME_TO_INT(name));
    else
	*qid = ERROR;
	...
	/* if it isn't there, look in the variable length queue name table */
        ...
        if (pVarQTbl)
            *qid = (int) vmexNameToObj (pVarQTbl, NAME_TO_INT(name));

	/* if it isn't there, it isn't a vmex queue */
...
* be supplied. VMEexec allows a zero length queue, but this will fail in vxWorks.
*
* RETURNS:
* EOK if successful.
* ERR_NOMGB if unable to allocate message buffers.
...
UINT32 q_vcreate
    ...
    *qid = (int) msgQCreate (count, maxlength, MSG_Q_PRIORITY);
    ...
    if (pVarQTbl == NULL)
        pVarQTbl = vmexTblCreate (nameVarQTbl, vmexVarQArray, vmexMaxVarQ);

    if (vmexTblAdd (pVarQTbl, NAME_TO_INT(name), (void *) *qid, NULL) == ERROR)
	{
	msgQDelete ((MSG_Q_ID) *qid);
	return (ERR_OBJTFULL);
	}
...
UINT32 q_vdelete
    ...
    */
    ...
    if (pVarQTbl)
       rc = (unsigned int) vmexObjToShadow (pVarQTbl, (void *) qid);
    else
       rc = ERROR;
    ...
    if (rc == ERROR)
       {
       if (pFixQTbl)
	  if (vmexObjToShadow (pFixQTbl, (void *) qid) != (void *) ERROR)
	     return(ERR_NOTVARQ);
       return(ERR_OBJID);
       }
    ...
    */
    ...
    vmexTblRemove (pVarQTbl, (void *) qid);
