...
IMPORT VMEX_TABLE_NODE vmexSemArray[];
IMPORT int vmexMaxSem;

/* Local variables  */

LOCAL BOOL initialized = FALSE;
LOCAL VMEX_TABLE *pSemTbl;		/* semaphore local name table */
...
UINT32 sm_create
    ...
    *smid = (UINT32) semCCreate (vxOptions, count);
    ...
    if (!initialized)
	{
        pSemTbl = vmexTblCreate (semTableName, vmexSemArray, vmexMaxSem);
	initialized = TRUE;
	}

    if ( vmexTblAdd (pSemTbl, NAME_TO_INT(name), (void *) *smid, NULL) == ERROR) 
	{
...
UINT32 sm_delete
    ...
    if (vmexObjToShadow (pSemTbl, (void *) smid) == (void *) ERROR)
	return (ERR_OBJID);
    ...
    */
    ...
    vmexTblRemove (pSemTbl, (void *) smid);
...
UINT32 sm_ident
    ...
    *smid = (UINT32) vmexNameToObj (pSemTbl, NAME_TO_INT(name));
