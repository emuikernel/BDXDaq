/* asrdemo.c - vmex Transition Kit Demo Application */
...
01b,20jan97,jmb  Added KILL code and S_T_I to pass strings as ints.
...
#include "/vobs/wpwr/target/unsupported/mkits/vmex/vmexLib.h"

/* Macro to change a 4 character string to an int */
#define S_T_I() *(unsigned int *)
 ...
 */
   ...
   if (amp == START_CODE_SHORT )
	 ...
	 {
	 t_getreg (0, SEM_REG, &smid);
         while ( sm_p (smid, SM_NOWAIT, 0) == E_NOERR )
	   ;
      ...
      strcpy (taskName, "AMP");
      taskName[3] = suffix;
      if (t_ident (S_T_I()taskName, 0, &tid) == E_NOERR) 
         t_delete(tid);
...
chatter(unsigned int value)
   ...
   strcpy (semName, "SMT");
   semName[3] = value;
   sm_create (S_T_I() semName, 1, 0, &smid);   /* Initial value = 1, RUNNING */
...
asrdemo(int suffix)
   ...
   xmitName[3] = suffix;
   args[0] = suffix;
   t_create (S_T_I()xmitName, 153, 2000, 1000, 0x0, &txid);
   ...
   strcpy (ampName, "AMP");
   ampName[3] = suffix;
   t_create (S_T_I()ampName, 154, 2000, 1000, 0x0, &tid);
   ...
   strcpy (freqName, "FRQ");
   freqName[3] = suffix;
   t_create (S_T_I()freqName, 154, 2000, 1000, 0x0, &tid);
