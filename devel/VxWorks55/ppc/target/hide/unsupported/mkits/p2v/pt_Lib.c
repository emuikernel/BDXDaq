...
01c,03jul96,jmb  fix alignment and initialization
...
* fixed-size buffers.  These routines are built on top of vxWorks'
...
IMPORT P2V_TABLE_NODE p2vPartArray[];
IMPORT int p2vMaxPart;

/* LOCAL variables  */

LOCAL BOOL initialized = FALSE;
LOCAL P2V_TABLE *pPartTbl;		/* partition local name table */
...
UINT32 pt_create 
    ...
    if (bsize < 4)   			/* pSOS+m(r) requirement */
	return (ERR_BUFSIZE);
    ...
    *nbuf = length / bsize;
    ...
    if (!initialized)
	{
        pPartTbl = p2vTblCreate (partTableName, p2vPartArray, p2vMaxPart); 
	initialized = TRUE;
	}

    if ( p2vTblAdd (pPartTbl, NAME_TO_INT(name), (void *) pBufPool, 
        (void *) pBufAux) == ERROR) 
	{
	if (allocated)
	    free (partAddr);
...
UINT32 pt_delete 
    ...
    if ((pAux = p2vObjToShadowInt (pPartTbl, (void *) ptid)) == (void *) ERROR)
	{
    ...
    if ( p2vTblRemove (pPartTbl, (void *) ptid) == ERROR )
	{
	intUnlock(intKey);		/* UNLOCK INTERRUPTS */
	return (ERR_OBJDEL);
	}
...
UINT32 pt_ident 
    ...
    *ptid = (UINT32) p2vNameToObj (pPartTbl, NAME_TO_INT(name));
