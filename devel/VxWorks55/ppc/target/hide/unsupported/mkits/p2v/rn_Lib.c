...
* memPartLib.  These routines may be used as in pSOS+(r), but the
...
IMPORT P2V_TABLE_NODE p2vRgnArray[];
IMPORT int p2vMaxRgn;
IMPORT BOOL p2vStrictMode;

/* LOCAL variables  */

LOCAL BOOL initialized;
LOCAL P2V_TABLE *pRgnTbl;		/* regions local name table */
...
UINT32 rn_create 
    ...
    if (!initialized)
	{
        pRgnTbl = p2vTblCreate (rgnTableName, p2vRgnArray, p2vMaxRgn); 
	initialized = TRUE;
	}

    if ( p2vTblAdd (pRgnTbl, NAME_TO_INT(name), (void *) partId, 
        (void *) pRgnAux) == ERROR) 
	{
	free (pRgnAux);
	return (ERR_OBJTFULL);
...
UINT32 rn_delete 
    ...
    if ((pAux = p2vObjToShadow (pRgnTbl, (void *) rnid)) == (void *) ERROR)
	return (ERR_OBJID);
    ...
    if ( p2vTblRemove (pRgnTbl, (void *) rnid) == ERROR )
	return (ERR_OBJDEL);
...
UINT32 rn_getseg 
	...
	partId = memSysPartId;
        ...
        if ((pAux = p2vObjToShadowInt (pRgnTbl, (void *) rnid)) == 
	    (void *) ERROR)
	    return (ERR_OBJID);
...
UINT32 rn_ident 
    ...
    *rnid = (UINT32) p2vNameToObj (pRgnTbl, NAME_TO_INT(name));
...
*        S_memLib_BLOCK_ERROR, so most of the pSOS+ error codes
