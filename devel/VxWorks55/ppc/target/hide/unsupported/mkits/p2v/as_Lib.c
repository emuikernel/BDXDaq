...
--------------------
01d,17jan97,jmb  simplify ASR wrapper...get rid of loop and let POSIX
...
*
*   This function is unnecessary.  It is implemented in the p2vLib.h
...
*
* This is the function that gets called when a p2v task gets a
...
LOCAL void ASRWrapper
    ...
    P2V_SHADOW_TCB *	pShadow;		/* TCB extension of the task */
...
* RETURNS: 
* EOK if successful.
* ERROR if calling task is not a p2v task or if signal function fails.
*/
...
UINT32 as_catch
    ...
    {
    P2V_SHADOW_TCB *pShadow;			/* TCB extension */

    if ( (pShadow=p2vTcb (0)) == NULL )     /* get the TCB extension */
	return(ERROR);
...
UINT32 as_send
    ...
    P2V_SHADOW_TCB *	pShadow;	/* p2v TCB extension of task */

    /* get the task's TCB extension, and verify it is a p2v task */
