...
* ERROR if calling task not a p2v task or if some error (other than
...
UINT32 ev_receive
    ...
    P2V_SHADOW_TCB *	pShadow;	/* tcb extension(where events are kept*/
    int 	timeoutPrev;
    BOOL 	waitForever;

    /* get the tcb extension and make sure this is a p2v task */
    ...
    */
    ...
    /* In pSOS+, a failed NO_WAIT ev_receive returns ERR_NOEVS instead of 
     * ERR_TIMEOUT 
     */
    if(flags & EV_NOWAIT) 
	if(!done)
...
UINT32 ev_send
    ...
    /* make sure the task we're sending to is a p2v task */
