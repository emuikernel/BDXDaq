...
--------------------
01l,30jan97,jmb  Added support for timer array and removed unused macros.
01k,22jan97,jmb  Eliminate as_return.
01j,21jan97,jmb  Eliminate ASEnable from the TCB
01i,16jan97,jmb  add a missing define and correct the value of EV_ALL
01h,13jun96,jmb  comment clean-up
01g,10jul96,jmb  add currentMode to TCB
...
This library contains implementations of various PSOS+(r) kernel routines,
in vxWorks code.  
*/
...
struct time_ds 		/* temporary until we get real thing from Motorola */
    {
    UINT32 year;
    UINT32 month;
    UINT32 day;
...
typedef struct 
    ...
    P2V_TABLE_NODE *nodes;      /* ptr to array of object nodes */
    } P2V_TABLE;	
...
#else
    P2V_TIMER_NODE *pTimerList;	/* List of armed timers */
#endif
    UINT32 initialPriority;	/* initial task priority */
    UINT32 initialMode;		/* initial task mode */
    UINT32 currentMode;		/* initial task mode */
    BOOL asrBusy;		/* ASR currently executing flag */
    } P2V_SHADOW_TCB;			

/* Marker used to indicate that a vxWorks TCB has a p2v extension */

#define P2V_COOKIE 0x70534f53
...
/*   No variable queues in VMEexec(r)
 *
 * #define ERR_VARQ 	0x3a
 * #define ERR_NOTVARQ 	0x3b
 */
 ...
 * into a p2v task or back to a normal vxWorks task
 */

extern STATUS p2vTCBInit(int tid);
extern void p2vTCBRemove(int tid);

/* Shortcut for spawning a p2v task from the shell using reasonable defaults.
 * Don't need to go though t_create/t_start.
 */

extern int p2vTaskSpawn (FUNCPTR entrypt, int arg1, int arg2, int arg3, 
			 int arg4);

/* Library utility function */

extern P2V_SHADOW_TCB * p2vTcb( UINT32);

/* Library initialization routines...
 * deleted...initialization is automatic
 *
 * extern void p2vSemInit(); 
 * extern void p2vQInit();
 */

/* Local object table manipulation routines */

extern void *p2vNameToObj (P2V_TABLE *pTable, UINT32 name);
extern void *p2vObjToShadow (P2V_TABLE *pTable, void *pObject);
extern void *p2vObjToShadowInt (P2V_TABLE *pTable, void *pObject);
extern STATUS p2vTblAdd (P2V_TABLE *pTable, UINT32 name, void *pObject,
    void *shadow);
extern STATUS p2vTblRemove (P2V_TABLE *pTable, void *pObject);
extern P2V_TABLE *p2vTblCreate (char * name, P2V_TABLE_NODE * pNode, int size); 
...
extern void p2vUnsupported (char * fmt, int arg1, int arg2, int arg3, int arg4,
    int arg5, int arg6);
extern STATUS p2vMessage ( unsigned int msgcode );
