...
#ifdef PCI_AUTO_DEBUG
  sprintf(txt,"pciAutoFuncConfig: baseAddr[%d] = 0x%08x, readVar=0x%08x\r\n",
    baI,baseAddr,readVar);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef PCI_AUTO_DEBUG
	    sprintf(txt,"pciAutoFuncConfig: baI=%d, baseAddr=0x%08x sizeMask=0x%08x addrInfo=0x%08x\r\n",
		    baI,baseAddr,sizeMask,addrInfo);
        sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
