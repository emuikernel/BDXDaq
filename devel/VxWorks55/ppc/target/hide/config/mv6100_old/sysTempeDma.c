...
usrVmeDmaInit(void)
{
  /*sysTempeDmaInit();*/ /* Dave removed it */
  return(OK);
}
...
usrVme2MemDmaStart(UCHAR *localVmeAddr, UCHAR *localAddr, UINT32 nbytes)
{
  ...
  /* Setup Descriptor */
  dmaDesc = dmaDescSample;
  dmaDesc.sourceAddr = (UINT32 *)localVmeAddr;  /* map lower 32 bit addr onto 64 bit field */
  dmaDesc.destAddr = (UINT32 *)localAddr;       /* map lower 32 bit addr onto 64 bit field */
  dmaDesc.byteCount = nbytes;

  /* Setup Attributes */
  dmaAttr = dmaAttrSample;
  /*dmaAttr.userHandler = (INT_HANDLER) WAIT_FOREVER;*/ /* Don't return till DMA done */
  dmaAttr.userHandler = NULL; /* return immediately */


  /*
logMsg("start: 0x%08x 0x%08x\n",localVmeAddr,localAddr,3,4,5,6);
  ...
  if((ret=sysTempeDmaStart(chan, &dmaDesc, &dmaAttr, &dmaStat)) == ERROR)
  {
...
usrVme2MemDmaDone()
{
  ...
  */
  ...
  if(dmaStatus.dmaDone == 0)  /* There was an Error */
  {
    ...
    {
      /* get the number BYTES NOT Transfered */
