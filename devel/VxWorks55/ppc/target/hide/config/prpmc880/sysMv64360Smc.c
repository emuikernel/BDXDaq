...
UINT32 cpuSmcBusSpeed = 0;		/* CPU Bus Speed */
...
UINT32 sysMv64360SdramInit (void);
...
*
* This routine calculates the CPU bus frequency using the max6900 rtc time 
*
* RETURNS: The bus speed (in Hz).
*/
...
UINT32 sysCpuBusSpd ()
    ...
    int x;
    UINT32 busRate = 0;
    UCHAR firstsecond = 0, lastsecond = 0;

    /* loop until a new second beginning */

    PLDI2CRead(MAX6900__RTC_ADDR, 0x81,&firstsecond);
    PLDI2CRead(MAX6900__RTC_ADDR, 0x81,&lastsecond);
    while (lastsecond ==  firstsecond) PLDI2CRead(0xa0,0x81,&lastsecond);
    ...
    for (x = 0; x < BAUDOUT_TICKS; x++)
	{
	    ...
	    /* loop until one second end */

	    PLDI2CRead(MAX6900__RTC_ADDR, 0x81,&firstsecond);
	    while (lastsecond ==  firstsecond) PLDI2CRead(MAX6900__RTC_ADDR, 0x81, &firstsecond);

        }

    /* Calculate CPU Bus Rate */

    busRate = (sysTimeBaseLGet() * 4);

    /*
     * 'busRate' contains the "raw" calculated bus rate in Hz - a 
     * typical value might be 133335862.  We need to a "normalize" a 
     * number such as this to its precise value - in this case 133333333.
     * The table below accomplishes this.  The "normalized" value is
     * what is returned by this function.
     */

    if ((45833333 <= busRate) && (busRate <= 54166666))
        return (50000000);
    else if ((62500000 <= busRate) && (busRate <= 70833333))
        return (66666666);
    else if ((70833333 <= busRate) && (busRate <= 79166666))
        return (75000000);
    else if ((79166666 <= busRate) && (busRate <= 87500000))
        return (83333333);
    else if ((95833333 <= busRate) && (busRate <= 104166666))
        return (100000000);
    else if ((129166666 <= busRate) && (busRate <= 137500000))
        return (133333333);
    else if ((145833333 <= busRate) && (busRate <= 154166666))
        return (150000000);
    else if ((170833333 <= busRate) && (busRate <= 179166666))
        return (175000000);
    else if ((195833333 <= busRate) && (busRate <= 204166666))
        return (200000000);
    else if ((220833333 <= busRate) && (busRate <= 229166666))
        return (225000000);
    else if ((229166666 <= busRate) && (busRate <= 237500000))
        return (233333333);
    else if ((245833333 <= busRate) && (busRate <= 254166666))
        return (250000000);
    else if ((258333333 <= busRate) && (busRate <= 266666666))
        return (262500000);
    else if ((262500000 <= busRate) && (busRate <= 270833333))
        return (266666666);
    else if ((270833333 <= busRate) && (busRate <= 279166666))
        return (275000000);
    else if ((287500000 <= busRate) && (busRate <= 295833333))
        return (291666666);
    else if ((295833333 <= busRate) && (busRate <= 304166666))
        return (300000000);
    else if ((320833333 <= busRate) && (busRate <= 329166666))
        return (325000000);
    else if ((329166666 <= busRate) && (busRate <= 337500000))
        return (333333333);
    else if ((333333333 <= busRate) && (busRate <= 341666666))
        return (337500000);
    else if ((345833333 <= busRate) && (busRate <= 354166666))
        return (350000000);
    else if ((362500000 <= busRate) && (busRate <= 370833333))
        return (366666666);
    else if ((370833333 <= busRate) && (busRate <= 379166666))
        return (375000000);
    else if ((395833333 <= busRate) && (busRate <= 404166666))
        return (400000000);
    else if ((408333333 <= busRate) && (busRate <= 416666666))
        return (412500000);
    else if ((412500000 <= busRate) && (busRate <= 420833333))
        return (416666666);
    else if ((445833333 <= busRate) && (busRate <= 454166666))
        return (450000000);
    else if ((454166666 <= busRate) && (busRate <= 462500000))
        return (458333333);
    else if ((462500000 <= busRate) && (busRate <= 470833333))
        return (466666666);
    else
        return (100000000);
#endif
...
LOCAL UINT32 sysMv64360DramBusSpd 
	...
	((n == 0xA0) && (m == 0x28)))
	{
	return (cpuSmcBusSpeed);		/* Sync Mode */
	}
    else				/* Async Mode */
	{
        return ((cpuSmcBusSpeed / 1000000) * (((n + 2) * 1000000) / (m + 2)));
	}
    }
...
LOCAL STATUS sysMv64360SdramSpeedInit
    ...
    if (busSpeed == cpuSmcBusSpeed)
	pSmcReg->dUnitCtrlLo = SYNC_MODE;
    else
	pSmcReg->dUnitCtrlLo = ASYNC_MODE;
    ...
    /* 
     * tWTR is the Write Command to Read Command. The MV64360 spec 
     * requirement for tWTR is 1 to 4 clocks, adjust any value outside 
     * of this requirement to the closest spec value.  The value of
     * tWTR is dependent on the CAS latency.
     */

    if (sdramTcl == 0x5)			/* CAS = 1.5 */
	sdramTwtr = 0x1;			/* 2 clocks */
    else
	sdramTwtr = 0x0;			/* 1 clock */
	...
	for (Bit = 7; Bit >= 0; Bit--)
		...
		if (Bit == 1) 			/* CAS = 1.5 */
		    {
		    if (sdramRegDimm == 0x0)	/* Unbuffered */
			{
			if (pSmcReg->dUnitCtrlLo == SYNC_MODE) /* Sync Mode */
			    pSmcReg->dUnitCtrlLo |=
			    ...
			    pSmcReg->dUnitCtrlLo |=
                                ((1 << DDR_DUNIT_CTRL_LO_RDPIPE_BIT) |     
                                 (1 << DDR_DUNIT_CTRL_LO_RDSYNCEN_BIT) |   
                                 (1 << DDR_DUNIT_CTRL_LO_RMWSYNCEN_BIT) |  
                                 (2 << DDR_DUNIT_CTRL_LO_STBURSTDEL_BIT) | 
                                 (1 << DDR_DUNIT_CTRL_LO_STBURSTNEG_BIT) | 
                                 (1 << DDR_DUNIT_CTRL_LO_STBURSTSCR_BIT) | 
			...
			if (pSmcReg->dUnitCtrlLo == SYNC_MODE) /* Sync Mode */
			    pSmcReg->dUnitCtrlLo |=
				 ...
				 (0 << DDR_DUNIT_CTRL_LO_RDSYNCEN_BIT) |
				 (0 << DDR_DUNIT_CTRL_LO_RMWSYNCEN_BIT) |
				 (6 << DDR_DUNIT_CTRL_LO_STBURSTDEL_BIT) |
				 (0 << DDR_DUNIT_CTRL_LO_STBURSTNEG_BIT) |
				 (0 << DDR_DUNIT_CTRL_LO_STBURSTSCR_BIT) |
				 (1 << DDR_DUNIT_CTRL_LO_RDDATANEG_BIT));
			else				  /* Async Mode */
...
LOCAL UINT32 sysMv64360SdramSizeInit
    ...
    for (bank = 0; bank < 1; bank++)
	     ...
	     * The PRPMC880 supports up to 1 Bank for each SPD.  The
	     * MV64360 supports up to 4 Banks of memory.  Bank 0 and 2's SPD
	     * EEPROM is located at I2C address $A0.  Bank 1 and 3's SPD 
	     * EEPROM is located at I2C address $A2.
...
UINT32 sysMv64360SdramInit ()
     ...
     * bank pair.  Bank 0's SPD is located at $A0.  
     */
    ...
    for (spd = 0; spd < 1; spd++)
        ...
        if (sysMv64360GetSpdData (SPD_EEPROM_ADDR_0 + (spd * 2), 0, SPD_SIZE, 
				  pBfr) == OK)
	    {
	        valid = TRUE;
    ...
    sysMv64360SdramSet(pSmcReg, base);
...
LOCAL void sysMv64360SdramSet
     ...
     * consecutive data to SRAM Data 0 (0x1494) register.  When the DRAM bus 
     * is running at 133 MHz the first data value to be written is 0x0
     * with each consecutive data value increasing by 0x41 until 0xFFF.
     * Note: Other frequencies will require different DFCDL values.
     */
 
    for (x = 0; x < 64; x++)
	{
	MV64360_WRITE32_PUSH(base, DDR_SRAM_DATA0, dfcdlVal);
	}
    ...
    MV64360_WRITE32_PUSH(base, DDR_SDRAM_CFG, pSmcReg->sdramCfg);
    MV64360_WRITE32_PUSH(base, DDR_SDRAM_TMNG_LO, pSmcReg->sdramTmngLo);
    MV64360_WRITE32_PUSH(base, DDR_SDRAM_TMNG_HI, pSmcReg->sdramTmngHi);
    ...
    /*
     * Step 5 - Programming of DUnit Control (Low) Register
     *
     ...
     * be configured to use 5 drivers.  This is done be setting Bits [4:0]
     * and Bits [9:5] to 0x5.
     */
	...
	;
    MV64360_WRITE32_PUSH(base, DDR_SDRAM_MODE, pSmcReg->sdramMode);
