...
*(unsigned int *)0x14000004 = 0x12340001;
...
*(unsigned int *)0x14000008 = 0x12340001;
...
*(unsigned int *)0x14000000 = 0x12340001;
*(unsigned int *)(0x13000000 + 4*(iii++)) = 0x12340002;

  /* Initialize variables */
  bzero ((char *)&pciLoc, sizeof (PCI_LOC));
  pciLoc.bus = bus;
*(unsigned int *)(0x13000000 + 4*(iii++)) = pciLoc.bus;
...
*(unsigned int *)(0x13000000 + 4*(iii++)) = pciLoc.bus;
*(unsigned int *)(0x13000000 + 4*(iii++)) = pciLoc.device;
*(unsigned int *)(0x13000000 + 4*(iii++)) = pciLoc.function;
*(unsigned int *)(0x13000000 + 4*(iii++)) = PCI_CFG_SUBCLASS;
*(unsigned int *)(0x13000000 + 4*(iii++)) = pciclass;
...
LOCAL void pciAutoFuncConfig
	...
	if ((addrInfo & PCI_BAR_SPACE_MASK) == PCI_BAR_SPACE_IO)
	    {
	    PCI_AUTO_DEBUG_MSG("pciAutoFuncConfig: IO Space found at BAR[%d] %x, %x,\n",
		    baI, addrInfo, readVar, 0, 0, 0);
	    sizeMask = (1 << 2);
	    }
	else
	    {
	    PCI_AUTO_DEBUG_MSG("pciAutoFuncConfig: MemSpace found at BAR[%d] %x, %x,\n",
		    baI, addrInfo, readVar, 0, 0, 0);
	    sizeMask = (1 << 4);
	    }
...
LOCAL UINT pciAutoMemAlloc
	...
	default:
	    *pAlloc = NO_ALLOCATION;
	PCI_AUTO_DEBUG_MSG("pciAutoMemAlloc: addrInfo error"
		"\n", 0, 0, 0, 0, 0, 0);
	    return 0;
	}
