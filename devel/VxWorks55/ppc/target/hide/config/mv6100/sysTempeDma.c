...
LOCAL void sysTempeDmaInit (void)
	...
	/*sergey: comment it out on Dave's advice; v1190's readout was hunging
              on high rate (10kHz regular pulser), now it works fine
...
usrVme2MemDmaStart(UCHAR *localVmeAddr, UCHAR *localAddr, UINT32 nbytes)
{
...
#ifdef DBG123
  logMsg("sourceAddr=0x%08x (0x%08x)\n",(UINT32)dmaDesc->sourceAddr,(UINT32)localVmeAddr,3,4,5,6);
  logMsg("destAddr=0x%08x (0x%08x)\n",(UINT32)dmaDesc->destAddr,(UINT32)localAddr,3,4,5,6);
  logMsg("nbytes=%d\n",nbytes,2,3,4,5,6);
#endif
  ...
  EIEIO;
  SYNC;
  logMsg("start: 0x%08x 0x%08x\n",(UINT32)dmaDesc->sourceAddr,(UINT32)dmaDesc->destAddr,3,4,5,6);
  EIEIO;
  SYNC;
      *(localAddr+0) = 0xff;
      *(localAddr+1) = 0xff;
...
SYNC;
  ...
  if(ret == ERROR)
  {
    logMsg("channel 0: ERROR: DMA start error\n",1,2,3,4,5,6);
    logMsg("channel 0: ERROR: DMA start error\n",1,2,3,4,5,6);
    logMsg("channel 0: ERROR: DMA start error\n",1,2,3,4,5,6);
    logMsg("channel 0: ERROR: DMA start error\n",1,2,3,4,5,6);
    logMsg("channel 0: ERROR: DMA start error\n",1,2,3,4,5,6);
  ...
  EIEIO;
  SYNC;
    logMsg("channel 0: INFO: ret=%d (0x%08x) nbytes=%d\n",ret,ret,nbytes,4,5,6);
  EIEIO;
  SYNC;
    logMsg("data1-> %02x %02x %02x %02x\n",
      *(localAddr+0),
      *(localAddr+1),
      *(localAddr+2),
      *(localAddr+3),5,6);
  EIEIO;
  SYNC;
    logMsg("data2-> %02x %02x %02x %02x %02x %02x\n",
      *(localAddr+(nbytes-5)),
      *(localAddr+(nbytes-4)),
      *(localAddr+(nbytes-3)),
      *(localAddr+(nbytes-2)),
  ...
  EIEIO;
  SYNC;
   logMsg("data3-> %02x %02x %02x %02x\n",
      *(localAddr+(nbytes+1)),
      *(localAddr+(nbytes+2)),
      *(localAddr+(nbytes+3)),
      *(localAddr+(nbytes+4)),5,6 );
  EIEIO;
  SYNC;
  }
#endif
...
usrVme2MemDmaDone()
{
  ...
  /* Dave has channel number as first parameter */
  if(chan >= TEMPE_DMA_CHANNEL_COUNT)
  {
    logMsg("ERROR: invalid channel\n",0,0,0,0,0,0);
    return(ERROR);
  ...
  */
  ...
  if(dmaStatus.dmaDone == 0)  /* There was an Error */
  {
    ...
    if(excStatus&0x80080000)  /* VME Bus Error (Sergey: must be (0x80080000 || 0x80100000) ?? see linux version) */
    {
#ifdef DOITFAST
      dmaStatus.dcsaLx = TEMPE_READ32(TEMPE_REG_BASE, dcsaLx[chan]);
      dmaStatus.dcntx  = TEMPE_READ32(TEMPE_REG_BASE, dcntx[chan]);
...
#ifdef DBG123
  printf("sourceAddr=0x%08x (0x%08x)\n",(UINT32)dmaDesc->sourceAddr,(UINT32)localVmeAddr[i]);
  printf("destAddr=0x%08x (0x%08x)\n",(UINT32)dmaDesc->destAddr,(UINT32)localAddr[i]);
  printf("nbytes=%d\n",size[i]);
#endif
...
/* SERGEY (PRIMEX): VERY TEMPORARY UNTIL pTickTimer/ppcTimer problem resolved */
void sysPciWrite32 (UINT32 a, UINT32 b)
{
  return;
}
void sysPciRead32 (UINT32 a, UINT32 *b)
{
  return;
}
