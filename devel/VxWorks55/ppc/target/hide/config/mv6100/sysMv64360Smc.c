...
UINT32 cpuBusSpeed = 0;         /* CPU Bus Speed */
...
{
  return (/*sysBusSpeed*/cpuBusSpeed);
}
...
LOCAL UINT32 sysMv64360DramBusSpd 
	...
	((n == 0xA0) && (m == 0x28)))
	{
	return (cpuBusSpeed);		/* Sync Mode */
	}
    else				/* Async Mode */
	{
        return ((cpuBusSpeed / 1000000) * (((n + 2) * 1000000) / (m + 2)));
	}
    }
...
UINT32 sysMv64360SdramInit 
	...
	spdPtrs[spd] = NULL;
        ...
        if (sysMv64360GetSpdData (SPD_EEPROM_ADDR_0 + (spd * 2), 0, SPD_SIZE, pBfr) 
	    == OK)
            {
            valid = TRUE;
