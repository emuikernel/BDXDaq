...
/*#endif*/
    ...
    #ifdef DEBUG
          print_status ("Program Status Reg = ", poll);
    #endif        
...
{
    unsigned int *StartAddress,*DestAddr,i;

        frcFlashReadRst (StartAdd);
        DestAddr = (unsigned int *)buffer;
    StartAddress = (unsigned int *)StartAdd;
    for(i=0;i< noLongs;i++)
    {
...
int frcFlashBlockErase(unsigned int BlockNo,unsigned int Flashbase)
{
    ...
    unsigned int StartBlock,flashDat,FlashAddress,Mnfct,DevCode,BlockLockcode; 
...
/*  Check wether the block is locked */
    FlashAddress = Flashbase + BlockNo * 0x40000;
    frcFlashReadIdCommand (&Mnfct,&DevCode,&BlockLockcode, FlashAddress);
    if(BlockLockcode == 0x1) {
        printf("Erase unsuccesful, block is locked!!\n");
        return(FLASH_TIMEOUT);
    }

    StartBlock = Flashbase + BlockNo * 0x40000;
    FlashAddress =  StartBlock;
