...
void	cpy(TYPE *buf, TYPE *dst, TYPE *lastone);
...
int bwMem
	...
	int	nbytes,
	char	*what,
	void	(*pCopy) (    const char * ,    char *       ,    int              )
	)
{
    	char *av[4];
	TYPE   *buf2 = 0, *lastone;
	...
	if (streq(what, "cp") ||
	    streq(what, "fcp") || streq(what, "bcopy")) {
	...
	tbReset_lmbench();
	if (streq(av[2], "rd")) {
		BENCHO(rd((TYPE *)buf, lastone), rd((TYPE *)buf, 0), 0);
	} else if (streq(av[2], "wr")) {
		BENCHO(wr((TYPE *)buf, lastone), wr((TYPE *)buf, 0), 0);
	} else if (streq(av[2], "rdwr")) {
		BENCHO(rdwr((TYPE *)buf, lastone), rdwr((TYPE *)buf, 0), 0);
	} else if (streq(av[2], "cp")) {
		BENCHO(cpy((TYPE *)buf, buf2, lastone), cpy((TYPE *)buf, buf2, 0), 0);
	} else if (streq(av[2], "frd")) {
		BENCHO(frd((TYPE *)buf, lastone), frd((TYPE *)buf, 0), 0);
	} else if (streq(av[2], "fwr")) {
		BENCHO(fwr((TYPE *)buf, lastone), fwr((TYPE *)buf, 0), 0);
	} else if (streq(av[2], "fcp")) {
		BENCHO(fcp((TYPE *)buf, buf2, lastone), fcp((TYPE *)buf, buf2, 0), 0);
		 ...
		 * to cost around 0...
		 */
		BENCHO(pCopy((void*)buf, (void*)buf2, nbytes), pCopy((void*)buf, (void*)buf2, 1), 0);
	}
	bandwidth(nbytes, get_n(), 1);
...
void
cpy(register TYPE *p, register TYPE *dst, register TYPE *lastone)
{	
	while (p <= lastone) {
