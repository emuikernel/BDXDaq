...
*/
farvoid *
AllocateMemory (unsigned long nbytes,	/* # of bytes to alloc */
 ...
 */
    ...
    returnval = (farvoid *) malloc ((size_t) 
	(nbytes + 2L * (long) global_align) + 0x10000);
...
*/
void 
FreeMemory (farvoid * mempointer,	/* Pointer to memory block */
	    int *errorcode)
{
...
#ifdef TEST_DEBUG    
    printf("memory is being freed at :0x%x\n",mempointer);
#endif /*#ifdefd TEST_DEBUG*/
...
unsigned long minMove = 0xffffffff;
unsigned long maxMove = 0;
...
*/
void 
MoveMemory (farvoid * destination,	/* Destination address */
...
#else
    mymemmove (destination, source, nbytes);
#endif
...
*/
void 
FarDOSmemmove (farvoid * destination,	/* Destination pointer */
...
*/
int 
RemoveMemArray (ulong adj_addr, ulong * true_addr)
{
    int i, j;
...
int 
bmOpenFile (char *fname,	/* File name */
 ...
 */
void 
CloseFile (int fhandle,		/* File handle */
	   int *errorcode)	/* Returned error code */
{
...
#ifdef LINUX
void 
CloseFile (FILE * fhandle,	/* File handle */
	   int *errorcode)	/* Returned error code */
{
    fclose (fhandle);
    *errorcode = 0;
...
/*
   ** Do the read.
 */
    readcode = read (fhandle, outBuffer, (unsigned) (nbytes & 0xFFFF));
    if (readcode == -1)
...
#ifdef LINUX
void 
readfile (FILE * fhandle,	/* File handle */
	  unsigned long offset,	/* Offset into file */
	  unsigned long nbytes,	/* # of bytes to read */
	  void *outBuffer,		/* Buffer to read into */
	  int *errorcode)	/* Returned error code */
 ...
 */
    ...
    readcode = fread (outBuffer, (size_t) 1, nelems, fhandle);
    if (readcode != nelems)
	*errorcode = ERROR_FILEREAD;
...
void 
writefile (int fhandle,		/* File handle */
	   unsigned long offset,	/* Offset into file */
	   unsigned long nbytes,	/* # of bytes to read */
	   void *outBuffer,	/* Buffer to read into */
	   int *errorcode)	/* Returned error code */
...
/*
   ** Do the write.
 */
    writecode = write (fhandle, outBuffer, (unsigned) (nbytes & 0xFFFF));
    if (writecode == -1)
...
void 
writefile (FILE * fhandle,	/* File handle */
	   unsigned long offset,	/* Offset into file */
	   unsigned long nbytes,	/* # of bytes to read */
	   void *outBuffer,	/* Buffer to read into */
	   int *errorcode)	/* Returned error code */
 ...
 */
    nelems = (size_t) (nbytes & 0xFFFF);
    writecode = fwrite (outBuffer, (size_t) 1, nelems, fhandle);
    if (writecode == nelems)
	*errorcode = ERROR_FILEWRITE;
...
*/
void 
ReportError (char *errorcontext,	/* Error context string */
...
#define	DEFAULT_DEC_CLK_FREQ	66000000 
...
int gettimeofday(struct timeval *tp, void *ptr )
    {
...
StartStopwatch ()
{
    ...
    return (tv.tv_sec * 1000000 + tv.tv_usec) / 10;
#endif
...
*/
unsigned long 
StopStopwatch (unsigned long startticks)
{
#ifdef VXTIMER
    struct timeval tv;
    ...
    return ((tv.tv_sec * 1000000 + tv.tv_usec) / 10) - startticks;
#endif
...
*/
unsigned long 
TicksToSecs (unsigned long tickamount)
{
#ifdef CLOCKWCT
...
#ifdef VXTIMER
    return ((unsigned long) (tickamount / 100000));
#endif
}
...
*/
double 
TicksToFracSecs (unsigned long tickamount)
{
#ifdef CLOCKWCT
...
#ifdef VXTIMER
    return ((double)tickamount / 100000.0);
#endif
}
