...
STATUS pciAutoCfgCtl
  ...
  char txt[100];
  /*
  sprintf(txt,"pciAutoCfgCtl reached\r\n ");
...
)
{
  ...
  char txt[100];
  /*
  sprintf(txt,"pciAutoCfgFunc reached\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  ...
  /*
  sprintf(txt,"pciAutoCfgFunc 111\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  if (pSystem->pciRollcallRtn != NULL)
  {
	...
	/*
    sprintf(txt,"pciAutoCfgFunc 222\r\n ");
    sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
	while ( ! rollcallSuccess )
	{
       ...
       */
	  /*
      sprintf(txt,"pciAutoCfgFunc 333\r\n ");
      sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
	  pPciList = pciAutoListCreate (pSystem, &listSize);
	  /*
      sprintf(txt,"pciAutoCfgFunc 444\r\n ");
      sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
	  /* Perform roll call function, if we pass, exit the loop */
	  if( (*pSystem->pciRollcallRtn)() == OK) rollcallSuccess = TRUE;
	  /*
      sprintf(txt,"pciAutoCfgFunc 555\r\n ");
      sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
   ...
   */
  /*
  sprintf(txt,"pciAutoCfgFunc 666\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  pPciList = pciAutoListCreate (pSystem, &listSize);
  /*
  sprintf(txt,"pciAutoCfgFunc 777\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  pciAutoFuncConfigAll (pSystem, pPciList, listSize);
  /*
  sprintf(txt,"pciAutoCfgFunc 888\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
#endif
  /*
  sprintf(txt,"pciAutoCfgFunc done\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  return(OK);
}
...
)
{
  ...
  char txt[100];
  /*
  sprintf(txt,"pciAutoListCreate reached\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
#if defined(PCI_AUTO_STATIC_LIST)
  /*
  sprintf(txt,"pciAutoListCreate 1\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
...
#else
	/*
  sprintf(txt,"pciAutoListCreate 2\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
...
#endif
	/*
  sprintf(txt,"pciAutoListCreate 3\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    lastPciListSize = 0;
    *pListSize = 0;
     ...
     */
	/*
  sprintf(txt,"pciAutoListCreate 4\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    ...
    )
{
  ...
  char txt[100];
  /*
  sprintf(txt,"pciAutoDevProbe reached\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
	...
	{
	  /*
  sprintf(txt,"pciAutoDevProbe: device=%d\r\n ",device);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
	...
	{
	  /*
  sprintf(txt,"pciAutoDevProbe: function=%d\r\n ",function);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	  */
		...
		/*
  sprintf(txt,"pciAutoDevProbe 1\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
		*/
		
#ifdef MYDEBUG
sprintf(txt,"pciAutoDevProbe: bus=%d dev=%d fun=%d offset=%d\r\n ",
    pciLoc.bus, pciLoc.device, pciLoc.function, PCI_CFG_VENDOR_ID);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif

/* skip PIB */
if(pciLoc.bus==0 && pciLoc.device==11)
{
#ifdef MYDEBUG
  sprintf(txt,"pciAutoDevProbe: skip device 0x%02x (%d) on bus 0\r\n ",
    pciLoc.device,pciLoc.device);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
  continue;
}
...
/* skip GT-64360 */
/*
if(pciLoc.bus==0 && pciLoc.device==16)
{
  sprintf(txt,"pciAutoDevProbe: skip device 0x%02x (%d) on bus 0\r\n ",
    pciLoc.device,pciLoc.device);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
...
*/
		...
		/*
  sprintf(txt,"pciAutoDevProbe 2\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
		*/
#ifdef MYDEBUG
sprintf(txt,"pciAutoDevProbe: dev_vend=0x%08x\r\n ",dev_vend);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoDevProbe: found [%d,%d,%d,0x%08x] -> 0x%08x\r\n",
pciLoc.bus, pciLoc.device, pciLoc.function, PCI_CFG_SUBCLASS, pciclass);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
  ...
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	...
	/*
  sprintf(txt,"pciAutoDevProbe 999\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
	*/
    return (subBus);
    }
...
#ifdef MYDEBUG
sprintf(txt,"pciAutoFuncDisable: disable device [%d,%d,%d,0x%02x]\r\n",
	    (pPciFunc->bus),
	    (pPciFunc->device),
	    (pPciFunc->function),
	    (pPciFunc->attribute));
...
#ifdef MYDEBUG
  sprintf(txt,"pciAutoFuncConfig reached.\r\n");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
  sprintf(txt,"pciAutoFuncConfig: exc [%d,%d,%d,0x%02x]\r\n",
...
#ifdef MYDEBUG
sprintf(txt,"--> pciAutoFuncConfig: baseAddr = 0x%08x\r\n",baseAddr);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt," --> pciAutoFuncConfig: [%d,%d,%d,0x%08x] -> 0x%08x\r\n",
  pPciFunc->bus, pPciFunc->device, pPciFunc->function, baseAddr, readVar);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
	    sprintf(txt,"pciAutoFuncConfig: IO Space found at BAR[%d]\r\n",
		    baI);
        sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
	    sprintf(txt,"pciAutoFuncConfig: MemSpace found at BAR[%d]\r\n",
		    baI);
        sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"   --> pciAutoFuncConfig: 0x%08x & 0x%08x -> %d\r\n",
  readVar, sizeMask, (readVar & sizeMask));
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"    --> call pciAutoRegConfig(,,0x%08x,0x%08x,0x%08x)\r\n",
  baseAddr, sizeMask, addrInfo);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt," +++> pciAutoRegConfig: IO: addr = 0x%08x, size = 0x%08x (%d)\r\n",
  addr, nSize, nSize);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	}
	...
	register64Bit = pciAutoMemAlloc (pSystem, pPciFunc, &addr, nSize,
					  addrInfo);
#ifdef MYDEBUG
sprintf(txt," +++> pciAutoRegConfig: ME: addr = 0x%08x, size = 0x%08x (%d)\r\n",
  addr, nSize, nSize);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	}
...
#ifdef MYDEBUG
sprintf(txt,"  +++> pciAutoRegConfig: 0x%08x 0x%08x\r\n",addr,NO_ALLOCATION);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	...
	pciConfigModifyLong (pPciFunc->bus, pPciFunc->device,
				...
				baseaddr_mask, addr);
#ifdef MYDEBUG
sprintf(txt,"   +++> pciAutoRegConfig: [%d,%d,%d,0x%08x] -> 0x%08x 0x%08x\r\n",
  pPciFunc->bus, pPciFunc->device, pPciFunc->function, baseAddr,
  baseaddr_mask, addr);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
LOCAL UINT pciAutoMemAlloc
  ...
  char txt[100];
	...
	case PCI_BAR_MEM_RESERVED:
	    /* fall through */
#ifdef MYDEBUG
sprintf(txt,"000> PCI_BAR_MEM_RESERVED (addrInfo=0x%08x)\r\n",addrInfo);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif

	default:
#ifdef MYDEBUG
sprintf(txt,"000> NO_ALLOCATION (addrInfo=0x%08x)\r\n",addrInfo);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	    *pAlloc = NO_ALLOCATION;
	    return 0;
...
#ifdef MYDEBUG
sprintf(txt,"000> proceed (addrInfo=0x%08x)\r\n",addrInfo);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	...
	{
#ifdef MYDEBUG
sprintf(txt,"000> PF Mem requested\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"000> No PF Mem available, trying MemIO\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
		...
		{
#ifdef MYDEBUG
sprintf(txt,"000> Warning: PCI PF Mem alloc failed\r\n");
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
	...
	if (retStat == ERROR)
	    {
#ifdef MYDEBUG
sprintf(txt,"000> PCI Memory allocation failed: pBase[0x%08x], pAvail[0x%08x], size=0x%08x\r\n",
(int)(*pBase),(int)(*pAvail),size);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
#ifdef MYDEBUG
sprintf(txt,"111> Pre: pBase[0x%08x], pAvail[0x%08x]\r\n",
...
#ifdef MYDEBUG
sprintf(txt,"111> Post: pBase[0x%08x], pAvail[0x%08x]\r\n",
  (int)(*pBase),(int)(*pAvail));
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
    PCI_AUTO_DEBUG_MSG("  Post: pBase[0x%08x], pAvail[0x%08x]\n",
...
)
{
  ...
  char txt[100];
  /*
  sprintf(txt,"pciAutoConfig reached\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  pCookie = pciAutoConfigLibInit(NULL);
  /*
  sprintf(txt,"pciAutoConfig 111\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */

#ifdef MYDEBUG
sprintf(txt,"! pciMem32: 0x%08x 0x%08x \r\n",
pSystem->pciMem32,pSystem->pciMem32Size);
sysDebugMsg(txt,CONTINUE_EXECUTION);

sprintf(txt,"! pciMemIo32: 0x%08x 0x%08x \r\n",
pSystem->pciMemIo32,pSystem->pciMemIo32Size);
sysDebugMsg(txt,CONTINUE_EXECUTION);

sprintf(txt,"! pciIo32: 0x%08x 0x%08x \r\n",
pSystem->pciIo32,pSystem->pciIo32Size);
sysDebugMsg(txt,CONTINUE_EXECUTION);

sprintf(txt,"! pciIo16: 0x%08x 0x%08x \r\n",
pSystem->pciIo16,pSystem->pciIo16Size);
sysDebugMsg(txt,CONTINUE_EXECUTION);
#endif
...
sysDebugMsg(txt,CONTINUE_EXECUTION);
  ...
  /*
  sprintf(txt,"pciAutoConfig 222\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  pciAutoCfgFunc(pCookie);
  /*
  sprintf(txt,"pciAutoConfig 333\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
  pciAutoConfigCopyback(pCookie, pSystem);
  /*
  sprintf(txt,"pciAutoConfig 444\r\n ");
  sysDebugMsg(txt,CONTINUE_EXECUTION);
  */
