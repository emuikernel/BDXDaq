...
void sysHarrierPhbInit (void)
    ...
    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_0_REG) = 0;
    EIEIO_SYNC;

    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_1_REG) = 0;
    EIEIO_SYNC;

    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_2_REG) = 0;
    EIEIO_SYNC;

    /* clear the Outbound Translation Attribute Registers 0, 1, and 2 */

    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_0_REG + 
                HARRIER_OUTBOUND_TRANSLATION_OFFSETINFO_OFFSET) = 0;
    ...
    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_0_REG +
                HARRIER_OUTBOUND_TRANSLATION_OFFSETINFO_OFFSET) =
                ((CPU2PCI_OFFSET0 << 16) | CPU2PCI_MSATT0);
    ...
    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_1_REG +
                HARRIER_OUTBOUND_TRANSLATION_OFFSETINFO_OFFSET) =
                ((CPU2PCI_OFFSET1 << 16) | CPU2PCI_MSATT1);
    ...
    *(UINT32 *)(HARRIER_OUTBOUND_TRANSLATION_ADDR_2_REG +
                HARRIER_OUTBOUND_TRANSLATION_OFFSETINFO_OFFSET) =
                ((CPU2PCI_OFFSET2 << 16) | CPU2PCI_MSATT2);
    ...
    *(UINT32 *)(HARRIER_MSG_PASSINGATTR_REG) = HARRIER_MPAT_ENA;
    ...
    /* Set up the PCFS Register. Turn everthing off and clear error status */
...
sysHarrierInitPhbExt (void)
{
  ...
  *(UINT32 *)HARRIER_INBOUND_TRANSLATION_ATTRIBUTE_0   = PCI2CPU_0_ATTR;
  *(UINT8 *)HARRIER_INBOUND_TRANSLATION_SIZE_0_REG     = 
            inboundSizeCode(PCI2CPU_0_SIZE);
  *(UINT16 *)HARRIER_INBOUND_TRANSLATION_OFFSET_0_REG  = 
             SHORTSWAP(PCI2CPU_0_TARG);
...
sysHarrierInitPhbExt2(void)
{
  ...
  sprintf(txt,"sysHarrierInitPhbExt2: bar,offset: 0x%08x 0x%08x\r\n",
    whichBar,whichOffset);
  sysDebugMsg(txt,CONTINUE_EXECUTION);
