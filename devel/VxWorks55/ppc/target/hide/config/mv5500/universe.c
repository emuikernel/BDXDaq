...
--------------------
01a,20jan03,scb  Ported from MVME5100 version 01p, corrected funtion
 ...
 * the highest priority interrupt.  By "higest priority interrupt" we mean
 ...
 * table the interrupt priority diminishes.  Notice the the "Int Level Mask"
 ...
 * corresponds the interrupt identified by index poistion [N + 1].  e.g.
 * The bit for AC-Fail (highest priority interrupty) appears at index
...
* Bridge.  It counts the occurences at the specified counter and clears the
...
/* macroses for DMA list operations */
...
static UNIVDMA_LL *ull;

STATUS
usrVmeDmaInit(void)
{
  static int init_is_done = 0;
  if(init_is_done == 0)
  {
    init_is_done = 1;
	/* MUST be 8-byte aligned (better 16-byte ?); we assume that
	   malloc will take care about it */
    ull = (UNIVDMA_LL *) malloc(UNIV_DMA_MAX_LL*sizeof(UNIVDMA_LL));
    printf("Allocate 'ull' at 0x%08x\n",ull);
  }
