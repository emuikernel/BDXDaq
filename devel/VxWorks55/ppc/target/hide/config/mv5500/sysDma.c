...
*
* for example: usrMem2MemDmaCopy(0,0x10000000,0x11000000,0x100)
*/
                  ...
                  UINT32 nbytes)
{
  ...
  dmaDesc.nextDescPtr = 0;	       /* Next descriptor pointer */

  /* Sergey: important ! */
  dmaAttrib.holdSourceAddr = 0; /* 0 - increment the source address */
  dmaAttrib.holdDestAddr = 0;   /* 0 - increment the destination address */
  ...
  */
  sysMv64260DmaStart(chan, &dmaDesc, &dmaAttrib, &dmaStatus);
  /*
  printf("============ dmaStatus ============\n");
...
usrWriteGPR(unsigned int data)
{
  ...
  pciConfigOutLong(0,6,0,offset,data);
