...
#else
	frcEEPROMRead8(EEPROM1,0x1fde,2, temp1);
	temp = ((*temp1 << 24) |(*(temp1+1) << 16) | 0x00001146);
...
*********************************************************************************/
int pci0SetConfigReg(int bus, int dev,int func, int RegNum, UINT32 RegData)
{
    unsigned int data;
    int key ;
...
* Returns      : OK on success
*******************************************************************************/
int pci0GetConfigReg(int bus, int dev,int func, int RegNum,UINT32 *RegData)
{
    unsigned int data;
    int key;
