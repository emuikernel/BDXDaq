...
IMPORT bool  bibInit();
IMPORT bool frcEEPROMRead8 (UINT8 devAdd, UINT16 offset, int noofBytes, UINT8 *buffer);
...
#ifdef ROHS
/* as there is only one bank is there in ROHS PMC 280*/
...
#ifdef PMC280_CACHE_COHERENCY_WT
    (void *) LOCAL_MEM_LOCAL_ADRS,
    (void *) LOCAL_MEM_LOCAL_ADRS,
    RAM_LOW_ADRS, 
    VM_STATE_MASK_VALID | VM_STATE_MASK_WRITABLE | VM_STATE_MASK_CACHEABLE,
    VM_STATE_VALID      | VM_STATE_WRITABLE      | VM_STATE_CACHEABLE_WRITETHROUGH/*VM_STATE_CACHEABLE*/
    },
#else
    (void *) LOCAL_MEM_LOCAL_ADRS,
    (void *) LOCAL_MEM_LOCAL_ADRS,
...
void frcPrintMv64360Version()
{
	...
	case 4:	printf("MV64360 Revsion: B1\n"); break;
    }
}
...
void flashBootSectorInit(void)
{
    ...
    if(data == 0xFFFFFFFF) {
        data = EOS;
/*	frcFlashUnlock((FLASH_BANK0+NV_BOOT_OFFSET));*/
#ifdef PMC280_DUAL_CPU
		if(frcWhoAmI())
		{
			frcFlashUnlock((unsigned *)(FLASH_BANK0+0x10000));
		}
    else {

			frcFlashUnlock((unsigned *)(FLASH_BANK0));
		}
#endif
...
mvI2cMasterInit(_100KHZ, TCLK_RATE);
...
#else
  _pSysL2CacheFlush = sysL2BackHWFlush;/*sysL2BackSWFlush;*/
...
)
{
			...
			if(frcWhoAmI())
    			{
				sysMsDelay(1500);/* This Delay is given as the boot parameters from the I2C device (EEPROM 0x54) is read at the same 
				time from both the CPU's and the I2C Read failure was happening to avoid this situation Delay is given*/
				if( ((frcEEPROMRead8 (0x54,(0x1000+0x100) ,BOOT_LINE_SIZE, string) )== false)
					||(*BOOT_LINE_ADRS == EOS))
				    {
		      ...
		      {
			       if( ((frcEEPROMRead8 (0x54,0x1000 ,BOOT_LINE_SIZE, string) )== false)
					||(*BOOT_LINE_ADRS == EOS))
				    {
				    /* either no non-volatile RAM or empty boot line */
    ...
    frcFlashReadRst((unsigned int )FLASH_BANK0);

    frcFlashRead((FLASH_BANK0+offset+NV_BOOT_OFFSET), (strLen)+1, (UINT32 *)string);
...
)
{
    ...
    if(offset + NV_BOOT_OFFSET < 0)
        return ERROR;
			...
			if(frcWhoAmI())
    			{
       			if ((frcEEPROMWrite8 (0x54,(0x1000 + 0x100 ),strlen (BOOT_LINE_ADRS) + 1, string)) == false)
       				{
		      ...
		      {
		         	if ((frcEEPROMWrite8 (0x54,0x1000  ,strlen (BOOT_LINE_ADRS) + 1, string)) == false)
		          	{
		          	printf("\nNOTE: Bootline not saved to NVRAM\n");
					return ERROR;
		         		}
    ...
    frcFlashReadRst((unsigned int )( FLASH_BANK0+NV_BOOT_OFFSET ));
#ifdef ROHS
    if(frcFlashBlockErase((offset + NV_BOOT_OFFSET)/0x10000, FLASH_BANK0 ) != 0 )
        return ERROR;
#else
...
void sysHardReset()
{
    ...
    /* Make GPP[18] as an active high signal */
...
void cpldGppDisablePin13 ()
{
 ...
 int temp1;
    ...
    if(frcWhoAmI())
    {
        dbg_puts1("CPU1: Entering cpldGppDisablePin13\n");
    }
    else
    {
        dbg_puts0("CPU0: Entering cpldGppDisablePin13\n");
    }
#endif /* PMC280_DUAL_CPU */
#endif /* PMC280_DEBUG_UART_VX */
 #ifdef PMC280_DEBUG_UART
 
    dbg_puts0("Entering cpldGppDisablePin13\n");
 #endif

 /* Program Mpp[13] as Gpp[13] */
    GT_REG_READ( MPP_CONTROL1, &temp1 );
    temp1 &= 0xff0fffff;
    GT_REG_WRITE( MPP_CONTROL1, temp1 );
 

   /* Program Gpp[13] as Ouput */
    GT_REG_READ( GPP_IO_CONTROL, &temp1 );
    temp1 |= 0x00002000;
    GT_REG_WRITE(GPP_IO_CONTROL, temp1 );
 
    /* Program Gpp[13] as Active High */
    ...
    if(frcWhoAmI())
    {
        dbg_puts1("CPU1: nExiting cpldGppDisablePin13\n");
    }
    else
    {
        dbg_puts0("CPU0: nExiting cpldGppDisablePin13\n");
    }
#endif /* PMC280_DUAL_CPU */
#endif /* PMC280_DEBUG_UART_VX */
 #ifdef PMC280_DEBUG_UART
 
dbg_puts0("Exiting cpldGppDisablePin13\n");
 #endif    
 }
#endif 
