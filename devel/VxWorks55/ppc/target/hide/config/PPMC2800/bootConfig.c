...
#ifdef ROHS
IMPORT void cpldGppDisablePin13();
IMPORT bool frcEEPROMWrite8(unsigned int,unsigned int,int,UINT8);
IMPORT bool frcEEPROMRead8(UINT8,UINT16,int,UINT8);
#endif
...
#ifdef PMC280_DUAL_CPU
LOCAL STATUS	frcLoad (BOOT_PARAMS *, FUNCPTR *);
#endif /* PMC280_DUAL_CPU */
...
unsigned long frc_vxworks_size;
...
void usrInit 
    ...
    {
#ifdef PMC280_DUAL_CPU	
    UINT32 * frcSyncSign1 = (UINT32 *)PMC280_SYNC_SIGN1;

	sdramSize = getSdramSize();  

	frcSyncSign1 = (UINT32 *)PMC280_SYNC_SIGN1;
#endif /* PMC280_DUAL_CPU */
    /*
...
#ifdef PMC280_DUAL_CPU
/* Here CPU1 will write to sync1 location its signature.
   Seeing this CPU0 will allow the user to do netload */
	
    if(frcWhoAmI())
        *(frcSyncSign1) = 0xdeadbeef;
#endif /* PMC280_DUAL_CPU */
...
LOCAL void bootCmdLoop (void)
    ...
    if(frcWhoAmI())
    {
        dbg_puts1("CPU1: Entering bootCmdLoop\n");
    }
    else
    {
        dbg_puts0("CPU0: Entering bootCmdLoop\n");
    }
#endif /* PMC280_DUAL_CPU */
#endif /* PMC280_DEBUG_UART_VX */
	...
	switch (*(pLine++))
		...
		bootParamsPrompt (BOOT_LINE_ADRS);
		  if (strlen(BOOT_LINE_ADRS) <= BOOT_LINE_SIZE)
                    ...
                    if ((sysNvRamSet (BOOT_LINE_ADRS,strlen (BOOT_LINE_ADRS) + 1, 0)) == ERROR)
...
#ifdef PMC280_DUAL_CPU
                if(!frcWhoAmI()) {
                    /* If CPU0 then wait for CPU1 to copy its code to DRAM */
                    if(*(frcSyncSign1) != 0xdeadbeef) {
                        printf("CPU1 has not yet copied the code from ROM to DRAM !!\n");
                        break;
                    }
                }
#endif /* PMC280_DUAL_CPU */
...
LOCAL char autoboot 
...
#ifdef PMC280_DUAL_CPU
    UINT32 * frcSyncSign1 = (UINT32 *)PMC280_SYNC_SIGN1;
    UINT32 * frcSignature = (UINT32 *)PMC280_SIGNATURE;
#endif /* PMC280_DUAL_CPU */
	...
	printf ("\nauto-booting...\n\n");
                ...
                if(!frcWhoAmI()) {
                    /* If CPU0 then wait for CPU1 to copy its code to DRAM */
                    if(*(frcSyncSign1) != 0xdeadbeef) {
                        printf("Waiting for CPU1 to copy code from ROM to DRAM...");
                        while(*(frcSyncSign1) != 0xdeadbeef);
                        printf("done.\n");
                    }
                }
                else {
                    if(*(frcSignature) != 0xdeadbeef) {
                        printf("Waiting for image in Shared Memory..."); 
                        while(*(frcSignature) != 0xdeadbeef);
                        printf("OK.\n");
                    }
                }
#endif /* PMC280_DUAL_CPU */
...
#ifdef PMC280_DUAL_CPU
/* Only pseudo network is available for CPU1 */
...
#ifdef PMC280_DUAL_CPU
    UINT32 *frcSignature = (UINT32 *)PMC280_SIGNATURE;
    UINT32 *frcVxEntry   = (UINT32 *)PMC280_VX_ENTRY;
    UINT32 *frcVxSize    = (UINT32 *)PMC280_VX_SIZE;
#endif /* PMC280_DUAL_CPU */
...
#ifdef PMC280_DUAL_CPU
/* CPU-1 need not execute the below code. */

    if(frcWhoAmI()) {
        if( frcLoad(&params, pEntry) == OK)
            return(OK);
        else
            return(ERROR);
    }
...
#ifdef PMC280_DEBUG_UART_VX
        printf("Loaded file successfuly for CPU0\n");
        printf("Size of vxWorks = %ld\n",frc_vxworks_size);
#endif /* PMC280_DEBUG_UART_VX */
        
       /* Copy the image from &entry to shared memory */
       memcpy((void *)PMC280_VX_SM_START,(void *)*pEntry,(size_t)frc_vxworks_size);
 
/* The below information will be used by CPU1 to copy its image */
       *frcSignature = (UINT32)0xdeadbeef;
       *frcVxEntry   = (UINT32)*pEntry;
       *frcVxSize    = (UINT32)frc_vxworks_size;
        printf("Copied the image to shared memory\n");
    }
#endif /* PMC280_DUAL_CPU */

/* PMC280 DUAL CPU stuff here */
...
#ifdef PMC280_DUAL_CPU
    if(!frcWhoAmI()) {
        /* If CPU0 wait for CPU1 to come up to vxWorks */
        if(*frcSyncSign != 0xdeadbeef) {
            printf("Waiting for CPU1 to boot vxWorks ....\n");
            while(*frcSyncSign != 0xdeadbeef);
            *frcSyncSign = 0;
        }
    }
#endif /* PMC280_DUAL_CPU */
...
LOCAL void usrBootLineInit 
    ...
    {
/*if (startType & BOOT_CLEAR)sergey*/
	{
...
/*************************************************************************
* frcLoad - Loads the image for CPU1 from the share memory.
*
* RETURNS : OK or ERROR
*/

STATUS frcLoad( BOOT_PARAMS *pParams, FUNCPTR *pEntry) {

    UINT32 *frcSignature = (UINT32 *)PMC280_SIGNATURE;
    UINT32 *frcVxEntry   = (UINT32 *)PMC280_VX_ENTRY;
    UINT32 *frcVxSize    = (UINT32 *)PMC280_VX_SIZE;

    if (strncmp (pParams->bootDev, "pseunet", 7) != 0) {
        printf("Unknown device \n");
        return(ERROR);
    }

    if (*frcSignature != 0xdeadbeef ) {
        printf("The image is not present in Shared memory\n");
        return(ERROR);
    }

    if (*frcVxEntry == (UINT32)NULL || *frcVxSize == (UINT32)NULL) {
        printf("Protocol error !!\n");
        return(ERROR);
    }
    memcpy ((void *)*frcVxEntry,(void *)PMC280_VX_SM_START,(size_t)*frcVxSize);
    *pEntry = (FUNCPTR) *frcVxEntry;

    printf("Loaded the image successfuly from shared memory\n");
    return(OK);
