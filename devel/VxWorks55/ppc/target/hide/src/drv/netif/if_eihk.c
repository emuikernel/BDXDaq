/* if_eihk.c - Intel 82596 Ethernet network interface driver for hkv3500 */
...
--------------------
04e,15jul97,spm  added ARP request to SIOCSIFADDR ioctl handler
04f,30apr97,map  doc update.
04e,29apr97,map  Modified receive to use RBDs, and buf loaning [SPR# 3511]
04d,04mar97,dat  fixed watchdog to use netJobAdd, SPR 7771
04d,07apr97,spm  code cleanup, corrected statistics, and upgraded to BSD 4.4
04c,13aug96,dat  fixed wdog timer restart. SPR 6958
04b,12nov94,tmk  removed some IMPORTed function decs and included unixLib.h
04a,03nov93,caf  derived hkv3500-specific driver from version 03g of if_ei.c.
		 changed eiattach() to eihkattach(); changed MAX_RFDS_LOANED
		 from 8 to 0; added second parm to sys596Init(); connected
		 interrupt to sysSpareInt1() instead of eiInt(); made eiInt()
		 global.
...
/*
This module implements a hkv3500 specfic Intel 82596 Ethernet network
interface driver.

This driver is derived from the generic if_ei ethernet driver to support
hkv3500 target board. The receive buffer scheme has been modified from a
simplified memory structure to a flexible memory structure so that receive
buffers can be word-aligned, and thus support buffer loaning on a MIPS CPU
architecture.

The driver requires several target-specific parameters, and some external
support routines which are detailed below.

This driver can run with the device configured in either big-endian or
little-endian modes.  Error recovery code has been added to deal with some of
the known errata in the A0 version of the device.  This driver supports up to
four individual units per CPU.
...
The only user-callable routine is eihkattach(), which publishes the `ei'
...
routine, once per unit, using eihkattach().
.iP "STATUS sys596Init (int unit, SCB *pScb)"
...
The driver calls this routine, once per unit, using eihkattach().
...
in the eihkattach() call.  The size of one RFD (Receive Frame
...
created at run-time.  These parameters are given to the driver when eihkattach()
...
static void eiRxQPut
    ...
    /* Initialize the RFD */
    pRfd->status = 0;
    pRfd->command = RFD_M_EL | RFD_M_FLEXMODE;
    pTemp = CACHE_DRV_VIRT_TO_PHYS (&pDrvCtrl->cacheFuncs, pRbd);
    LINK_WR (&pRfd->lBufDesc, pTemp);
    pRfd->actualCnt = 0;
    pRfd->bufSize = EH_SIZE;

    /* Initialize the RBD */
    pRbd->actualCnt = 0;
    pTemp = CACHE_DRV_VIRT_TO_PHYS (&pDrvCtrl->cacheFuncs, pRfd->enetData);
    LINK_WR (&pRbd->pData, pTemp);
    pRbd->size = RBD_EL | ETHERMTU;
    ...
    if (pTail != NULL)
        ...
        /* update RBD links */
        pTemp = CACHE_DRV_VIRT_TO_PHYS (&pDrvCtrl->cacheFuncs, pRbd);
        LINK_WR (&pTail->rbd.lNext, pTemp);
        pTail->rbd.size &= ~RBD_EL;
