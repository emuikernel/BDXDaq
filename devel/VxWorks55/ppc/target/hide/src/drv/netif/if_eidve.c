/* if_eidve.c - Intel 82596 Ethernet network interface driver for DVE-SH7XXX */
...
--------------------
03k,03mar99,jmb  Implement 2-byte alignment shift for RFD's, similar to
                 offset shift in ei82596End.c
03j,15dec98,jmb  Bring driver up to BSD 4.4 for SENS compatibility.
03i,24apr97,hk   added DVE-SH7XXX support.
03h,05feb97,dat  fixed wdog reset action to use netJobAdd(). SPR 7771
03g,13aug96,dat  fixed wdog timer restart. SPR 6958
03g,12nov94,tmk  removed some IMPORTed function decs and included unixLib.h
...
#define DMA_MEMORY_IS_DUALPORTED		/* DVE-SH7XXX specific */
...
IMPORT void *sysDmaVirtToPhys (void * address);
IMPORT void *sysDmaPhysToVirt (void * address);
...
static EI_NODE *eiQGet
    ...
    if ((pNode = (EI_NODE *) pQueue->head) != NULL)     /* if list not empty */
        pQueue->head = (EI_NODE *) LINK_RD ( (EI_LINK *) &pNode->pNext);    /* advance ptr */
...
static void eiQPut
    ...
    LINK_WR ((EI_LINK *) &pNode->pNext, NULL);
    ...
    if (pQueue->head == NULL)                        /* if list empty */
        ...
        LINK_WR ((EI_LINK *) & pQueue->tail->pNext, pNode);
