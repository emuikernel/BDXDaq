...
--------------------
01p,18nov96,db	 put missing brackets in if-else statement(SPR #7411).
01m,25jan95,vin	 removed magic numbers, replaced with macros.
...
STATUS tyCoDevCreate
    ...
    if (channel == 0)
	{
	sysOutByte (UART_IER (COM1_BASE_ADR), I8250_IER_RDI_ENBL);
	sysIntEnablePIC (COM1_INT_LVL);
	}
    else
	{
	sysOutByte (UART_IER (COM2_BASE_ADR), I8250_IER_RDI_ENBL);
	sysIntEnablePIC (COM2_INT_LVL);
	}

#endif /* TY_CO_POLL XXX */
...
LOCAL void tyCoHrdInit (void)
    ...
    sysOutByte (UART_LCR (COM1_BASE_ADR),(I8250_LCR_WRD8 | I8250_LCR_DLAB));
    sysOutByte (UART_BRDL (COM1_BASE_ADR), BAUD_9600_LSB);
    sysOutByte (UART_BRDH (COM1_BASE_ADR), BAUD_9600_MSB);
    sysOutByte (UART_LCR (COM1_BASE_ADR), (I8250_LCR_WRD8 | I8250_LCR_SB1));
    sysOutByte (UART_MDC (COM1_BASE_ADR), (I8250_MCR_OUT2 | 
					   I8250_MCR_RTS  | 
					   I8250_MCR_DTR));
    sysOutByte (UART_LCR (COM2_BASE_ADR), (I8250_LCR_WRD8 | I8250_LCR_DLAB));
    sysOutByte (UART_BRDL (COM2_BASE_ADR), BAUD_9600_LSB);
    sysOutByte (UART_BRDH (COM2_BASE_ADR), BAUD_9600_MSB);
    sysOutByte (UART_LCR (COM2_BASE_ADR), (I8250_LCR_WRD8 | I8250_LCR_SB1));
    sysOutByte (UART_MDC (COM2_BASE_ADR), (I8250_MCR_OUT2 |
					   I8250_MCR_RTS  | 
					   I8250_MCR_DTR));
...
void tyCoInt
	...
	interruptID &= I8250_IIR_RLS;

        if (interruptID == I8250_IIR_RLS)
            lineStatus = sysInByte (pTyCoDv->lst);
        else if (interruptID == I8250_IIR_RD)
	    {
            if (pTyCoDv->created)
        ...
        else if (interruptID == I8250_IIR_THR)
	    {
            if ((pTyCoDv->created && tyITx (&pTyCoDv->tyDev, &outChar)) == OK)
	        sysOutByte (pTyCoDv->data, outChar);
            else
	        sysOutByte (pTyCoDv->ier, I8250_IER_RDI_ENBL);
	    }

        interruptID = sysInByte (pTyCoDv->iid);

	} while (((interruptID & I8250_IIR_PIMASK) == I8250_IIR_PI) && 
		 (ix++ < 10));
    }
...
LOCAL void tyCoStartup
    ...
    sysOutByte (pTyCoDv->ier, (I8250_IER_THR_ENBL | I8250_IER_RDI_ENBL));
