...
enum { extra = 8 };     // Size of space used to store size.  Note
                        ...
                        // alignment.
static size_t extra_chunk() {
    return 8/sizeof(value_type)+(size_t)(8%sizeof(value_type)>0);
}
public:
    ...
    static void * allocate() { allocate(sizeof(value_type));}
...
// c_s 28apr98: renamed ROUND_UP to __stl__ROUND_UP to avoid 
//              conflict with host.h

private:
  static size_t __stl__ROUND_UP(size_t bytes) {
...
# endif
    result = *my_free_list;
    if (result == 0) {
        void *r = refill(__stl__ROUND_UP(n));
        return r;
    }
...
__alloc<threads, inst>::chunk_alloc(size_t size, int& nobjs)
{
    ...
    if (bytes_left >= total_bytes) {
        ...
        size_t bytes_to_get = 2 * total_bytes + __stl__ROUND_UP(heap_size >> 4);
                                                      ...
                                                      size_t new_sz)
{
    ...
    if (__stl__ROUND_UP(old_sz) == __stl__ROUND_UP(new_sz)) return(p);
