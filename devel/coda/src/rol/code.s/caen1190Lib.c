...
static int window_extra_margin = 200;
...
tdc1190Init(UINT32 addr, UINT32 addr_inc, int ntdc, int iFlag)
{
  ...
  usrVmeUserToPhysOffset(tdcAddrOffset);
#endif
...
tdc1190InitList(UINT32 *addr_list, int ntdc, int iFlag)
{
  ...
  for(ii=0; ii<ntdc; ii++) 
  {
    ...
    if(tdc1190CommonInit(ii,laddr) == OK)
	{
      Nc1190++;
	}
	else
...
void
tdc1190SetDefaultWindowExtraMargin(int offset)
{
  window_extra_margin = offset;
  return;
}
...
tdc1190Config(int options)
{
  ...
  */
  ...
  for(ii=0; ii<Nc1190; ii++)
  {
    if(ww < 25)         value = 1; /*Minimo 25 ns, 1 colpo di clock */
...
/******************************************************************************
 *
 * tdc1190TriggerTimeTag - Enable/Disable Extended Trigger Time Tag
 *                   flag = 0 : Disable
 *                          1 : Enable
 *
 * RETURNS: 0 if successful, ERROR otherwise.
...
int
tdc1190TriggerTimeTag(int id, UINT32 flag)
{
  ...
  if(flag>1)
    {
      printf("tdc1190TriggerTimeTag: ERROR: Invalid flag = %d",flag);
      return ERROR;
    }

  LOCK_1190;
  printf("tdc1190TriggerTimeTag before: 0x%09x\n",
	 tdcRead16(&(c1190p[id]->control)));

  if(flag == 1)
    tdcWrite16(&(c1190p[id]->control),
	       tdcRead16(&(c1190p[id]->control)) | V1190_EXT_TRIG_TIME_TAG_ENABLE);
    ...
    printf("tdc1190TriggerTimeTag: unknown flag=%d\n",flag);

  printf("tdc1190TriggerTimeTag after: 0x%09x\n",
	 tdcRead16(&(c1190p[id]->control)));
  UNLOCK_1190;
...
tdc1190ReadBoard(int itdc, UINT32 *tdata)
{
  ...
  if(berr_fifo == 0x01)
  {
    ...
    fifodata = /* *fifo*/(tdcRead32(&(c1190p[itdc]->fifo))&0xffff);
  }

  do
  {
    *(++output) = /* *data*/tdcRead32(&(c1190p[itdc]->data[0]));
...
tdc1190ReadBoardDmaStart(int ib, UINT32 *tdata)
{
  ...
  {
    /* get event length in words */
    fifodata = (tdcRead32(&(c1190p[ib]->fifo))&0xffff);
  ...
  {
    ndata_save[ib] = V1190_MAX_WORDS_PER_BOARD;
    mdata = 0;
    /*
  ...
  if(berr_fifo == 0x01)
  {
	...
	*/
    ...
    nbytes = ((ndata_save[ib]+extra_save[ib])<<2);
  }
  else
  {
    nbytes = (ndata_save[ib]<<2);
  }

  vmeAdr = (unsigned int)(c1190p[ib]);
...
tdc1190ReadBoardDmaDone(int ib)
{
  ...
  if(berr_fifo == 0x01)
  {
    ...
    else
    {
	  ...
	  /*
logMsg("tdc1190ReadBoardDmaDone: res=%d ndata_save=%d extra_save=%d -> mdata=%d\n",
res,ndata_save[ib],extra_save[ib],mdata,5,6);
	  */
	  ...
	  /*
      logMsg("tdc1190ReadBoardDmaDone: INFO: usrVme2MemDmaDone returned = %d (%d)\n",
             res,mdata,3,4,5,6);
	  */
    }
  }
  else
  {
    ...
    else if(res == 0)
    {
      logMsg("tdc1190ReadBoardDmaDone: ERROR: usrVme2MemDmaDone returned = %d\n",
             res,2,3,4,5,6);
      logMsg("tdc1190ReadBoardDmaDone: ERROR: we are expecting VME BUS error !\n",
             res,2,3,4,5,6);
      return(0);
    }
    else
...
/* part1: 6 usec */
  ...
  for(jj=0; jj<Nc1190; jj++)
  {
    ...
    if(nev == 0)
    {
      logMsg("tdc1190ReadStart: [%2d] not ready !\n",jj,2,3,4,5,6);
    }
...
TIMER_VAR;
  */
  ...
  if(berr_fifo == 0x01) /* use FIFO reaout */
  {
    ...
    for(jj=0; jj<Nc1190; jj++)
    {
      fifo = (UINT32 *) ( ((unsigned int)c1190p[jj]) +0x1038);

      /* get event length in words */
      fifodata = (tdcRead32(&(c1190p[jj]->fifo))&0xffff);
...
logMsg("[%d] ask=%d (%d bytes), got=%d (0x%08x to 0x%08x)\n",
 jj,ndata_save[jj]+extra_save[jj],size_save[jj],rlenbuf[jj],(unsigned int)c1190p[jj],destination[jj]);
	*/
  ...
  else /* use BERR readout */
  {
    ...
    for(jj=0; jj<Nc1190; jj++)
    {
      ndata_save[jj] = V1190_MAX_WORDS_PER_BOARD;
      extra_save[jj] = 0;
...
*/
  ...
  /* set DMA list */
  {
    unsigned int tdcadr[21];
    for(ii=0; ii<Nc1190; ii++) tdcadr[ii] = (unsigned int)c1190p[ii];
    usrVme2MemDmaListSet(tdcadr, destination, size_save, Nc1190);
  }
...
tdc1190ReadBlock(int id, volatile UINT32 *data, int nwrds, int rflag)
{
  ...
  unsigned int vmeAdrLL[V1190_MAX_MODULES],destAdrLL[V1190_MAX_MODULES], numLL;
  unsigned int itdcbuf=0;
  unsigned int fifowords=0;
  unsigned int dmaSizeLL[V1190_MAX_MODULES];

  CHECKID(id);

  if(nwrds <= 0) nwrds= (V1190_MAX_WORDS_PER_BOARD*V1190_MAX_MODULES) + 10;
  ...
  {     
    ...
    if (rflag == 2)
	{
#ifdef VXWORKS /*sergey: was before 'if' -  error ???*/
#ifdef WITHLL
