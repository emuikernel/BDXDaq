...
v1720Init(UINT32 addr, UINT32 addr_inc, int nadc, int iFlag)
{
  ...
  for(ii=0; ii<nadc; ii++) 
  {
    printf("ADDR: %x %x %x \n",laddr,ii*addr_inc,laddr+ii*addr_inc);
    c1720p[ii] = (struct v1720_struct *)(laddr + ii*addr_inc); /*This points to the local address the VME is mapped to*/
  ...
  /* Check if Board exists at that address */
    ...
    else 
    {
      ...
      if((rdata != V1720_FIRMWARE_REV)&&(rdata != (V1720_FIRMWARE_REV1)))
	  {
	    printf("WARN: Firmware does not match: 0x%08x (expected 0x%08x or 0x%08x)\n",
		   rdata,V1720_FIRMWARE_REV,V1720_FIRMWARE_REV1);
        break; 
	  } 
    }
  ...
  usrVmeserToPhysOffset(adcAddrOffset);
#endif
...
v1720SetChannelDACOffset(int id, int ch, int offset)
{
  ...
  LOCK_1720;
  ret=vmeRead32(&(c1720p[id]->chan[ch].status))&V1720_CH_DAC_BUSY;
  UNLOCK_1720;                                       
...
STATUS
v1720PrintChannelDACOffset(int id, int ch)
{
  ...
  CHECKID(id);
  LOCK_1720;
  ret=vmeRead32(&(c1720p[id]->chan[ch].dac_offset));
  UNLOCK_1720;
...
int v1720ReadOne(int id){
  int ret;
  ret=vmeRead32(&(c1720p[id]->data[0]));
  printf("%x \n",ret);
  return OK;
...
int v1720AutoSetDCOffset(int id,float BASELINE_GOAL){ /*BASELINE_GOAL is >0 in mV. Should be greater than the noise level, otherwise the mean is not fine.*/
  int ii,jj;
  int var_t;
  ...
  LOCK_1720; 
  old_trigger_mask=vmeRead32(&(c1720p[id]->trigger_source_enable_mask));
  old_channel_mask=vmeRead32(&(c1720p[id]->channel_enable_mask));
  trigger_mask=1;
  trigger_mask=(trigger_mask<<31); /*enable sw_trigger and disable any external: in any case, we need it!!*/
  vmeWrite32(&(c1720p[id]->trigger_source_enable_mask),trigger_mask); 
  UNLOCK_1720;   
  ...
  for (ii=0;ii<V1720_MAX_CHANNELS;ii++){
    DC_OFFSET=AUTO_DC_OFFSET_START;
    if (((old_channel_mask>>ii)&0x1)==1){
      ...
      LOCK_1720;
      vmeWrite32(&(c1720p[id]->channel_enable_mask),channel_mask);
      UNLOCK_1720;
      v1720SetChannelDACOffset(id,ii,DC_OFFSET);
    

      while((flag)||((-mean)<BASELINE_GOAL)){
	v1720SetAcquisition(id,0,1,0);
	if (ret>400) break;
	LOCK_1720;
	vmeWrite32(&(c1720p[id]->sw_trigger),0x1);
        UNLOCK_1720;

	v1720SetAcquisition(id,0,0,0);
	...
	while(((*buffp)>>28)!=0xA){ /*Seems that, reading one by one, the REAL first word could be later*/
	  ...
	  if (var_t>40) break;
	  LOCK_1720;	 
	  (*buff_point)=vmeRead32(&(c1720p[id]->data[0]));
	  UNLOCK_1720;	
	}
  ...
  LOCK_1720; 
  vmeWrite32(&(c1720p[id]->trigger_source_enable_mask),old_trigger_mask);
  vmeWrite32(&(c1720p[id]->channel_enable_mask),old_channel_mask);
