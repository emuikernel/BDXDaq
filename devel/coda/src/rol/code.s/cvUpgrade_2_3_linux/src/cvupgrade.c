...
static void Usage() {
		...
		printf("Syntax: cvUpgrade -License ConnType[USB|PCI_OPTLINK|PCIE_OPTLINK] [LicenseOptions]\n");
        printf("Syntax: cvUpgrade -Key N ConnType[USB|PCI_OPTLINK|PCIE_OPTLINK] [DeviceOptions]\n\n");
...
static int KeyInfo(cvUpgradeParams *Config, cvFlashAccess *Flash) {
	...
	if( strlen( Config->modelName)) {
		sprintf(KeyFilename, "BoardInfo-%s-%d.dat", Config->modelName, rom.crom_serial);
	} else {
		sprintf(KeyFilename, "BoardInfo-%d.dat", rom.crom_serial);
	}
...
int main(int argc,char *argv[])
{
	...
	*Params.modelName= '\0';
	...
	if (strcmp(argv[argIndex],"PCI_OPTLINK") == 0) Params.Type = CAENComm_PCI_OpticalLink;
    ...
    for (; argIndex<argc; argIndex++) {
		...
		if ( strcmp(argv[argIndex],"-VMEbaseaddress") == 0 )
			sscanf(argv[++argIndex], "%x", &Params.BaseAddress);
