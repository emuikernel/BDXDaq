...
struct TID_A24RegStruct
{
  ...
  /* 0x0002C */ volatile unsigned int blocksReady;
  /* 0x00030 */ volatile unsigned int TSTriggerInfo;
  ...
  /* 0x00040 */ volatile unsigned int timing[(0x54-0x40)/4];
...
/* 
   Define TID Modes of operation:    Polling Mode     0
                                     Interrupt mode   1
*/
#define TID_READOUT_POLL 0
#define TID_READOUT_INT  1

/* FIXME: These are not used yet... */
#define TID_SOURCE_EXT    0
#define TID_SOURCE_TS     1
#define TID_SOURCE_TS2    2
#define TID_SOURCE_SOFT   3
#define TID_SOURCE_RANDOM 4
...
#define TID_DATASETUP_TRIGSRC_FBR1     (1<<17)
#define TID_DATASETUP_TRIGSRC_LOOPBACK (1<<18)
#define TID_DATASETUP_TRIGSRC_FP       (1<<19)
#define TID_DATASETUP_TRIGSRC_VME      (1<<20)
#define TID_DATASETUP_TRIGSRC_FPcode   (1<<21)
...
#define TID_DATASETUP_SYNCSRC_FBR1     (1<<25)
#define TID_DATASETUP_SYNCSRC_FBR5     (1<<26)
...
#define TID_INTSETUP_CLK_FIBER1       (1<<13)
#define TID_INTSETUP_CLK_FIBER5       (1<<12)
...
/* Function prototypes */
int  tidInit(unsigned int tAddr, unsigned int mode, unsigned int source, int force);
void tidStatus();
int  tidReadFPGAUserCode();
int  tidReset();
int  tidReload();
int  tidClockResync();
int  tidSetCrateID(unsigned int crateID);
int  tidSetBlockLevel(unsigned int blockLevel);
int  tidSetTriggerSource(int trig, int flag);
...
void tidClearDataBuffer();
int  tidEnableFiber(unsigned int mask);
int  tidDisableFiber(unsigned int mask);
...
int  tidSetBlockWaitingThreshold(unsigned int thres);
int  tidSetupFiberLoopback();
int  tidLoadTriggerTable();
unsigned int tidGetDaqStatus();
unsigned int tidGetTSTriggerInfo();
int tidSetGTPInput(unsigned int inpMask);
int tidSetClockSource(unsigned int source);
int tidTrigger2();
void FiberMeas();
void FiberMeasWrite(unsigned int number);
