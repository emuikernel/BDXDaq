...
#define PCI_ASYNC(code,id)  {printf("linking async PCI trigger to id %d \n",id); \
     PCI_handlers = (id);PCI_isAsync = 1; pcitriglink(code,PCI_int_handler);}

#define PCI_SYNC(code,id)   {printf("linking sync PCI trigger to id %d \n",id); \
			       PCI_handlers = (id);PCI_isAsync = 0;}
...
#define PCI_GETID(code) PCI_handlers
