/*  "vme_link.c"  -  Test VME Trigger Interface with NEW Trigger Supervisor --- computer 
			 in TS crate should be running "ts5_link" (TS is in different crate).
...
int ts_events(void);
int ran(void);
void statistics( struct roc_stats *stats );
...
/*  assign board interrupt vector 0xEC  */
	vme_int->vector = 0xEC;

/*  connect routine to interrupt  */
	status = intDisconnect(INUM_TO_IVEC(0xEC));
	printf("\n connect interrupt service routine \n");
	status = intConnect(INUM_TO_IVEC(0xEC), (FUNCPTR)acknowledge, 0);
	printf("\n intConnect status = %d\n",status);
   ...
   and increment interrupt count */
{
	...
	stats->int_count++;
	roc_data = (long)((0xFF) & vme_int->trig_dat);	/* read roc data and mask unused bits */
	roc_code = roc_data >> 2;
