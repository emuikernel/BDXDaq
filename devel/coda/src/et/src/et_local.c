...
{     
  ...
  if (etid->version != etid->sys->version) {
    if (etid->debug >= ET_DEBUG_ERROR) {
      	printf("ERROR , etl_open: ET system & user's ET different. User: %i, sys: %i \n",etid->version,etid->sys->version);
	et_logmsg("ERROR", "etl_open: ET system & user's ET versions are different.User: %i, sys: %i \n",etid->version, etid->sys->version);
	}
...
/*printf("et_heartmon %d: mine = %d, sys = %d\n", me, sys->proc[me].heartbeat, newheartbt);*/
	 ...
	 */
        id->offset = (char *)id->pmap - (char *)id->sys->pmap;
        id->alive = 1;
        disconnected = 0;
      }
    }
