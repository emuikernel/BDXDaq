...
public class Consumer {
  ...
  public static void main(String[] args) {
    ...
    try {
     ...
     statConfig.setFlowMode(Constants.stationSerial);
      //if (blocking == 0) {
...
System.out.println("Station attachment = " + att.getId()); 
	...
	for (int i=0; i < numAttachments-1; i++) {
	  ...
	  mevs = sys.getEvents2(att, Constants.sleep, 0, chunk);
	  ...
	  if (true) {
		...
		for (int k=0;k<400;k++){
	      		num = bytesToInt(data,k*4);
              		//System.out.print(Integer.toHexString(data[k])+" ");
