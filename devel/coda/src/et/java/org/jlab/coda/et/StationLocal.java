...
public class StationLocal extends Thread implements EventSelectable {
  ...
  LinkedList            parallelStations;  // protected by stopTransfer & systemLock
  ...
  StationLocal(SystemCreate _sys, String _name, StationConfig _config, int _id)
    ...
    // attachments
    attachments = new HashSet(Constants.attachmentsMax);
  ...
  private void insertStation(StationLocal newStation, int position, int parallelPosition)
	...
	newStation.parallelStations = new LinkedList();
	...
	if (StationConfig.compatibleParallelConfigs(stat.config, newStation.config) == false) {
...
private void shellSort(int n, int[] a, int[] b) {
  ...
  public void run() {
    ...
    ArrayList getList = new ArrayList(sys.config.numEvents);
    // events to be put into the next station's inputList
    ArrayList putList = new ArrayList(sys.config.numEvents);
	...
	while (getList.size() > 0) {
	  ...
	  // events to a single group of parallel stations.
	    ...
	    int index, skip, numOfEvents, min, eventsToPut, eventsLeft;
