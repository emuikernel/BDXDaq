#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/colors.c,v 1.1.1.1 1998/08/25 17:48:01 rwm Exp $";
#endif
/*****
...
static int CreateColormap(HTMLW(html), XColor *cmap);
...
*****/
Pixel
_XmHTMLGetPixelByName(HTMLW(html), String color, Pixel def_pixel)
{
	...
	*****/
	if(tka->win == None)
		_XmHTMLToolkitAbstractionSetDrawable(tka,
			XXDefaultRootWindow(tka->dpy));

	_XmHTMLCheckXCC(html);
	colormap = CORE_ATTR(colormap);
...
XmHTMLAllocColor(Widget w, String color, Pixel def_pixel)
{
	...
	tka = _XmHTMLCreateToolkitAbstraction();
...
XmHTMLFreeColor(Widget w, Pixel pixel)
{
	...
	*/
	tka = _XmHTMLCreateToolkitAbstraction();
	_XmHTMLToolkitAbstractionSetDisplay(tka, w);
	tka->FreeColors(tka->dpy, ATTR_CORE(w, colormap), &pixel, 1, 0L);
	free(tka);
}

Boolean
_XmHTMLAddPalette(HTMLW(html))
{
...
static int
CreateColormap(HTMLW(html), XColor *cmap)
{
	int i, idx, ncolors;
	float mul;
