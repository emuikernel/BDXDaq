#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/paint.c,v 1.1.1.1 1998/08/25 17:48:02 rwm Exp $";
#endif
/*****
...
* Last modification: 	$Date: 1998/08/25 17:48:02 $
* By:					$Author: rwm $
...
* ChangeLog 
* $Log: paint.c,v $
* Revision 1.1.1.1  1998/08/25 17:48:02  rwm
...
#include <Xm/DrawP.h>		/* Private Motif render functions */
...
static void DrawText(HTMLW(html), XmHTMLObjectTableElement data);
static void DrawAnchor(HTMLW(html), XmHTMLObjectTableElement data);
static void DrawImageAnchor(HTMLW(html), XmHTMLObjectTableElement data);
static void DrawRule(HTMLW(html), XmHTMLObjectTableElement data);
static void DrawBullet(HTMLW(html), XmHTMLObjectTableElement data);
static XmHTMLObjectTableElement DrawTable(HTMLW(html),
...
static void DrawFrame(HTMLW(html), XmHTMLImage *image, int xs, int ys);
static void DrawTableBorder(HTMLW(html), XmHTMLTable *table);
static void DrawCellFrame(HTMLW(html), TableCell *cell);
static void DrawCellContent(HTMLW(html), XmHTMLObjectTableElement start,
...
*****/
static void
DrawText(HTMLW(html), XmHTMLObjectTableElement data)
{
	...
	tka->SetFont(tka->dpy, gc, words[0].font->xfont->fid);
	...
	for(i = 0 ; i < nwords; i++)
	{
		...
		*/
		...
		tka->DrawString(tka->dpy, tka->win, gc, xs, ys, tmp->word, tmp->len);
...
*****/
static void
DrawAnchor(HTMLW(html), XmHTMLObjectTableElement data)
{
		...
		if(ys != y || i == nwords-1)
		{
				...
				if(HTML_ATTR(anchor_buttons))
				{
					...
					if(HTML_ATTR(highlight_on_enter))
					{
						...
						tka->FillRectangle(tka->dpy, tka->win, hiGC, x,
							y - font->xfont->ascent, width, font->lineheight);
					}
					/* and draw as requested */
					DrawAnchorButton(tka, x, y - font->xfont->ascent,
			...
			*/
			...
			if(words)
			{
				for(j = start; j < i+1; j++)
				{
					tka->SetFont(tka->dpy, gc, words[j].font->xfont->fid);
					tka->DrawString(tka->dpy, tka->win, gc,
			...
			else
			{
				for(j = start; j < i+1; j++)
				{
					tka->SetFont(tka->dpy, gc,all_words[j]->font->xfont->fid);
					tka->DrawString(tka->dpy, tka->win, gc,
...
static void
DrawImageAnchor(HTMLW(html), XmHTMLObjectTableElement data)
{
	...
	*/
	...
	{
		tka->SetFont(tka->dpy, gc, data->words->font->xfont->fid);
		tka->SetForeground(tka->dpy, gc,
				data->anchor_state == ANCHOR_SELECTED ?
					HTML_ATTR(anchor_activated_fg) : HTML_ATTR(anchor_fg));
		...
		tka->DrawString(tka->dpy, tka->win, gc, x, y, data->words->word,
				data->words->len);
	}
}
...
*****/
static void
DrawFrame(HTMLW(html), XmHTMLImage *image, int xs, int ys)
{
...
*****/
static void
DrawRule(HTMLW(html), XmHTMLObjectTableElement data)
{
	int dy;
	GC gc;
	int xs, ys;
	...
	*/
	...
	/* vertical offset */
	dy = (int)(0.75*(HTML_ATTR(default_font)->height));
	...
	if(data->height)
	{
		...
		else
		{
			...
			if(data->fg != HTML_ATTR(body_fg))
				_XmHTMLRecomputeShadowColors(html, HTML_ATTR(body_bg));
		}
	}
	else
	{
...
static void
DrawBullet(HTMLW(html), XmHTMLObjectTableElement data)
{
	GC gc = HTML_ATTR(gc);
	int ys, xs;
	ToolkitAbstraction *tka = HTML_ATTR(tka);
	...
	switch(data->marker)
	{
		...
		default: 
			...
			tka->SetFont(tka->dpy, gc, HTML_ATTR(default_font)->xfont->fid);
			tka->DrawString(tka->dpy, tka->win, gc,
...
DrawTable(HTMLW(html), XmHTMLObjectTableElement start, 
	...
	for(i = 0; i < nrows; i++)
	{
		...
		for(j = 0; j < row->ncells; j++)
		{
			...
			*****/
			...
			if(cell->properties->bg != HTML_ATTR(body_bg) ||
...
*****/
static void
DrawCellContent(HTMLW(html), XmHTMLObjectTableElement start,
	XmHTMLObjectTableElement end)
{
...
*****/
static void
DrawCellFrame(HTMLW(html), TableCell *cell)
{
	...
	rw   = width  = data->width - xoff;
	yoff = data->font->m_ascent;
	rh   = height = data->height - cell->parent->parent->vmargin;
	...
	*****/
	...
	if(cell->owner->bg != HTML_ATTR(body_bg))
	{
	...
	{
		tka->DrawShadows(tka->dpy, tka->win, MGR_ATTR(top_shadow_GC),
			MGR_ATTR(bottom_shadow_GC), xs, ys, width, height,
			cell->properties->border, XmSHADOW_IN);
		return;
	}

	/*****
	* Code below currently unused, DrawShadows draws a full rectangle by
	* default, need to figure out something else here.
	*****/
	/* top border */
	if(rule & DRAW_TOP)
		tka->DrawShadows(tka->dpy, tka->win, MGR_ATTR(top_shadow_GC),
			MGR_ATTR(bottom_shadow_GC), xs, ys, width, 2,
			cell->properties->border, XmSHADOW_IN);
	...
	if(rule & DRAW_BOTTOM)
		tka->DrawShadows(tka->dpy, tka->win, MGR_ATTR(top_shadow_GC),
			MGR_ATTR(bottom_shadow_GC), xs, ys + height, width, 2,
			cell->properties->border, XmSHADOW_OUT);
...
*****/
static void
DrawTableBorder(HTMLW(html), XmHTMLTable *table)
{
	int xs, ys;
	...
	xs = data->x - HTML_ATTR(scroll_x) - xoff;

	/* vertical offset, taking scrolling into account */
	ys = data->y - HTML_ATTR(scroll_y) - yoff;
	...
	if(DRAW_BOX == (rule & DRAW_BOX))
	{
		tka->DrawShadows(tka->dpy, tka->win, MGR_ATTR(top_shadow_GC),
			MGR_ATTR(bottom_shadow_GC), xs, ys, width, height,
			table->properties->border, XmSHADOW_OUT);
		return;
	}
	...
	if(rule & DRAW_BOTTOM)
		tka->DrawShadows(tka->dpy, tka->win, MGR_ATTR(top_shadow_GC),
			MGR_ATTR(bottom_shadow_GC), xs, ys + height, width, 2,
			table->properties->border, XmSHADOW_IN);
...
*****/
void
_XmHTMLPaint(HTMLW(html), XmHTMLObjectTable *start, 
...
*****/
void
_XmHTMLRestartAnimations(HTMLW(html))
{
	XmHTMLImage *tmp;
...
*****/
void
_XmHTMLDrawImage(HTMLW(html), XmHTMLObjectTableElement data, int y_offset,
	Boolean from_timerCB)
{
	...
	*/
	...
	else if(image->pixmap != None)
	{
		...
		/* copy to screen */
		tka->CopyArea(tka->dpy, image->pixmap, tka->win, gc, 0, y_offset,
			image->width, image->height, xs, ys + y_offset);
	}
	/* reset gc */
	RESET_GC(gc);
	...
	*****/
		...
		!(data->text_data & TEXT_ANCHOR))
	{
		...
		xs += image->width + 4;
		ys += image->height/2 + 4;
		tka->DrawString(tka->dpy, tka->win, gc, xs, ys, data->words->word,
				data->words->len);
	}
...
*****/
void
_XmHTMLPaintAnchorUnSelected(HTMLW(html))
{
...
*****/
void
_XmHTMLPaintAnchorSelected(HTMLW(html), XmHTMLWord *anchor)
{
	XmHTMLObjectTable *start, *end;
...
*****/
void
_XmHTMLPaintAnchorLeave(HTMLW(html))
{
	XmHTMLObjectTable *start, *end;
...
*****/
void
_XmHTMLPaintAnchorEntry(HTMLW(html), XmHTMLObjectTable *anchor)
{
	XmHTMLObjectTable *start, *end;
