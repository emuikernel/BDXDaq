#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/callbacks.c,v 1.1.1.1 1998/08/25 17:48:01 rwm Exp $";
#endif
/*****
...
_XmHTMLFocusOutCallback(XmHTMLWidget html, XEvent *event)
{
	...
	cbs.event = event;
	XtCallCallbackList((Widget)html, HTML_ATTR(losing_focus_callback), &cbs);
}
...
_XmHTMLFocusInCallback(XmHTMLWidget html, XEvent *event)
{
	...
	cbs.event = event;
	XtCallCallbackList((Widget)html, HTML_ATTR(focus_callback), &cbs);
}
...
_XmHTMLMotionCallback(XmHTMLWidget html, XEvent *event)
{
	...
	cbs.event = event;
	XtCallCallbackList((Widget)html, HTML_ATTR(motion_track_callback), &cbs);
}
...
_XmHTMLInputCallback(XmHTMLWidget html, XEvent *event)
{
	...
	cbs.event = event;
	XtCallCallbackList((Widget)html, HTML_ATTR(input_callback), &cbs);
}
...
_XmHTMLArmCallback(XmHTMLWidget html, XEvent *event)
{
	...
	XtCallCallbackList((Widget)html, HTML_ATTR(arm_callback), &cbs);
}
	...
	XmHTMLImagemapCallbackStruct *cbs)
{
	...
	*****/
	...
	XtCallCallbackList((Widget)html, html->html.imagemap_callback, cbs);
}
