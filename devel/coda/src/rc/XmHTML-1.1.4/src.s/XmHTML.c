#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/XmHTML.c,v 1.1.1.1 1998/08/25 17:47:59 rwm Exp $";
#endif
/*****
...
* Last modification: 	$Date: 1998/08/25 17:47:59 $
* By:					$Author: rwm $
...
* ChangeLog 
* $Log: XmHTML.c,v $
* Revision 1.1.1.1  1998/08/25 17:47:59  rwm
...
* Private functions 
***/
static void CreateAnchorCursor(HTMLW(html));
static void CreateHTMLWidget(HTMLW(html));
static void CheckAnchorUnderlining(HTMLW(html), HTMLW(req));
static void CheckAlignment(HTMLW(html), HTMLW(req));
static void ScrollCB(WIDGET(w), XtPointer arg1, XtPointer arg2);
static void CheckPLCIntervals(HTMLW(html));
...
static void DrawRedisplay(WIDGET(w), HTMLW(html), XEvent *event);

/* VisibilityNotify event handler for the work area */
static void VisibilityHandler(WIDGET(w), HTMLW(html), XEvent *event);

/* MapNotify action routine for the work area */
static void Mapped(WIDGET(w), HTMLW(html), XEvent *event); 
...
{
	HTMLW(html) = (XmHTMLWidget)init;
	HTMLW(req)  = (XmHTMLWidget)request;
	...
	HTML_ATTR(tka)         = _XmHTMLCreateToolkitAbstraction();

	/* Store display */
	_XmHTMLToolkitAbstractionSetDisplay(HTML_ATTR(tka), (Widget)html);
	...
	if(HTML_ATTR(repeat_delay) < 1)
	{
		_XmHTMLWarning(__WFUNC__(init, "Initialize"), XMHTML_MSG_9,\
			HTML_ATTR(repeat_delay));
		HTML_ATTR(repeat_delay) = 25;
	}
	...
	*****/
	if(CORE_ATTR(width) <= 0)
	{
			...
			&value)))
		{
			...
			XTextExtents(HTML_ATTR(default_font)->xfont, "m", 1, &dir, &ascent,
				&descent, &ch);
...
CreateAnchorCursor(HTMLW(html))
{
	...
	if(HTML_ATTR(anchor_cursor) == None)
	{
		...
		(void)XParseColor(display, CORE_ATTR(colormap), "white", 
			&white_def);

		(void)XParseColor(display, CORE_ATTR(colormap), "black", 
			&black_def);
...
void
_XmHTMLRaiseFormWidgets(HTMLW(html))
{
	XmHTMLFormData *form;
	XmHTMLForm *entry;
...
*****/
void
_XmHTMLMoveToPos(WIDGET(w), HTMLW(html), int value)
{
			...
			if(inc < HTML_ATTR(work_height))
			{
				...
				*****/
				...
				{ 
					...
					{
						tka->SetClipMask(tka->dpy, gc, form->clip);
						tka->SetClipOrigin(tka->dpy, gc, xs, ys);
					}
				}

				/* copy visible part upward */
				tka->CopyArea(tka->dpy, tka->win, tka->win, gc, 0, inc,
		...
		else
		{
			...
			if(inc < HTML_ATTR(work_height))
			{
				...
				*****/
				...
				/* copy area down */
				tka->CopyArea(tka->dpy, tka->win, tka->win, gc, 0, 0,
					HTML_ATTR(work_width) + HTML_ATTR(margin_width), 
		...
		if (value > HTML_ATTR(scroll_x))
		{
			...
			if(inc < HTML_ATTR(work_width))
			{
				...
				*****/
				...
				tka->CopyArea(tka->dpy, tka->win, tka->win, gc, inc, 0,
					HTML_ATTR(work_width) - inc, HTML_ATTR(work_height), 0, 0);
...
_XmHTMLCheckXCC(HTMLW(html))
{
	...
	*/
	...
	{
		Visual *visual = NULL;
		Colormap cmap  = CORE_ATTR(colormap);
...
*****/
void
_XmHTMLGetScrollDim(HTMLW(html), int *hsb_height, int *vsb_width)
{
	Arg args[1];
		...
		*/
		if(height >= CORE_ATTR(height))
		{
			_XmHTMLWarning(__WFUNC__(HTML_ATTR(hsb), "_XmHTMLGetScrollDim"),\
				XMHTML_MSG_11, "Height", "horizontal", "height",\
				CORE_ATTR(height), 15);
			height = 15;
			XtSetArg(args[0], XmNheight, height);
	...
	if(HTML_ATTR(vsb))
	{
		...
		if(width >= CORE_ATTR(width))
		{
			_XmHTMLWarning(__WFUNC__(HTML_ATTR(vsb), "_XmHTMLGetScrollDim"), \
				XMHTML_MSG_11, "Width", "vertical", "width", \
				CORE_ATTR(width), 15);
			width  = 15;
			XtSetArg(args[0], XmNwidth, width);
...
*****/
void
_XmHTMLCheckScrollBars(HTMLW(html))
{
...
/*ARGSUSED*/
static void
Mapped(WIDGET(w), HTMLW(html), XEvent *event)
{
	...
	_XmHTMLToolkitAbstractionSetDrawable(HTML_ATTR(tka),
...
/*ARGSUSED*/
static void
VisibilityHandler(WIDGET(w), HTMLW(html), XEvent *event)
{
...
*****/
void
_XmHTMLAutoSizeWidget(HTMLW(html))
{
...
*****/
static void
DrawRedisplay(WIDGET(w), HTMLW(html), XEvent *event)
{
	/* 
...
SetValues(WIDGET(current), WIDGET(request), WIDGET(set),
	ArgList args, Cardinal *num_args)
{
	HTMLW(w_curr) = (XmHTMLWidget)current;
	HTMLW(w_req)  = (XmHTMLWidget)request;
	HTMLW(w_new)  = (XmHTMLWidget)set;
	...
	*/
			...
			ATTR_HTML(w_curr, anchor_activated_bg)))
	{
		...
		XtVaSetValues(ATTR_HTML(w_new, work_area), 
			XmNbackground, ATTR_HTML(w_new, body_bg), NULL);
			...
			ATTR_HTML(w_curr, def_body_image_url))
	{
		...
		*****/
		if(ATTR_HTML(w_new, body_image))
			...
			_XmHTMLRecomputeHighlightColor(w_new, ATTR_HTML(w_new, body_bg));
...
Destroy(WIDGET(w))
{
	...
	_XmHTMLDestroyToolkitAbstraction(HTML_ATTR(tka));
...
ExtendStart(WIDGET(w), XEvent *event, String *params, Cardinal *num_params)
{
	...
	HTMLW(html) = (XmHTMLWidget)XtParent(w);
...
*****/
static void
CheckAnchorUnderlining(HTMLW(html), XmHTMLWidget req)
{
...
*****/
static void
CheckAlignment(HTMLW(html), HTMLW(req))
{
...
XmHTMLRedisplay(WIDGET(w))
{
	...
	if(HTML_ATTR(gc) != NULL)
	{
		...
		XmUpdateDisplay((Widget)html);
		if(XtIsManaged(HTML_ATTR(vsb)))
			XmUpdateDisplay(HTML_ATTR(vsb));
		if(XtIsManaged(HTML_ATTR(hsb)))
