#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/Balloon.c,v 1.1.1.1 1998/08/25 17:48:12 rwm Exp $";
#endif
/*****
...
Realize(Widget w, XtValueMask *valueMask, XSetWindowAttributes *attrs)
{
	...
	_XmHTMLDebug(16, ("Balloon.c: Realize start\n"));
	if(ATTR(backing_store) != Always + WhenMapped + NotUseful)
	{
		_XmHTMLDebug(16, ("Balloon.c: Realize, backing store selected\n"));
	...
	if(ATTR(transparent))
	{
		_XmHTMLDebug(16, ("Balloon.c: Realize, transparency selected\n"));
	...
	XtCreateWindow(w, (unsigned int)InputOutput, (Visual*)CopyFromParent,
		*valueMask, attrs);
	_XmHTMLDebug(16, ("Balloon.c: Realize end\n"));
}
...
drawBalloonShaped(Widget w, Position x, Position y, int width)
{
	...
	{
		_XmHTMLDebug(16, ("Balloon.c: drawBalloonShaped, freeing pixmap\n"));
	...
	if(!(ATTR(shape_mask)))
	{
		_XmHTMLDebug(16, ("Balloon.c: drawBalloonShaped, creating pixmap\n"));
	...
	*****/
	...
	{
		_XmHTMLDebug(16, ("Balloon.c: drawBalloonShaped, drawing filled "
			"oval.\n"));
	...
	*****/
	if(ATTR(transparent))
	{
		...
		{
			_XmHTMLDebug(16, ("Balloon.c: drawBalloonShaped, drawing "
				"transparent border.\n"));
			XSetForeground(dpy, ATTR(shape_gc), 0);
			FillArc(dpy, shape_mask, ATTR(shape_gc), &(ATTR(maskt)),
...
popupBalloon(XtPointer client_data, XtIntervalId *id)
{
	...
	if(ATTR(pop_x) == 0 && ATTR(pop_y) == 0)
	{
		...
		_XmHTMLDebug(16, ("Balloon.c: popupBalloon, root_x = %i, root_y = %i, "
