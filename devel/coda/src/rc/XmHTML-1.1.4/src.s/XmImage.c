#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/XmImage.c,v 1.1.1.1 1998/08/25 17:48:08 rwm Exp $";
#endif
/*****
...
* Last modification: 	$Date: 1998/08/25 17:48:08 $
* By:					$Author: rwm $
...
* ChangeLog 
* $Log: XmImage.c,v $
* Revision 1.1.1.1  1998/08/25 17:48:08  rwm
...
copyInfo(XmImageInfo *info)
{
	...
	for(i = 0; src != NULL && i < frame_count; src = src->frame, i++)
	{
		...
		copy->data = memcpy(copy->data, src->data, size);
		if(ImageInfoClipmask(copy))
		{
			copy->clip = (Byte*)malloc(size);
			copy->clip = memcpy(copy->clip, src->clip, size);
		}
		else
			copy->clip = (Byte*)0;
		if(ImageInfoRGBSingle(copy))
		{
			...
			copy->reds   = memcpy(copy->reds, src->reds, size);
		...
		else
		{
			...
			copy->greens = memcpy(copy->greens, src->greens, size);
			copy->blues  = (Dimension*)malloc(size);
			copy->blues  = memcpy(copy->blues, src->blues, size);
		}
		/*
	...
	XmImageConfig *config)
{
	...
	if(_xmimage_cfg)
	{
		...
		_xmimage_cfg->tka = _XmHTMLCreateToolkitAbstraction();
		_XmHTMLToolkitAbstractionSetDisplay(_xmimage_cfg->tka, w);
		_XmHTMLToolkitAbstractionSetDrawable(_xmimage_cfg->tka, XtWindow(w));
	}
	...
	Dimension height, XmImageConfig *config)
{
	...
	*****/
	...
	tka = _xmimage_cfg->tka = _XmHTMLCreateToolkitAbstraction();
	_XmHTMLToolkitAbstractionSetDisplay(tka, w);
	_XmHTMLToolkitAbstractionSetDrawable(tka, XtWindow(w));
