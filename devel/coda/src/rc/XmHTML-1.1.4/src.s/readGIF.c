#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/readGIF.c,v 1.1.1.1 1998/08/25 17:48:04 rwm Exp $";
#endif
/*****
...
*****/
static int
ReadColorMap(ImageBuffer *ib, int number, Byte buffer[3][XmHTML_MAX_IMAGE_COLORS],
	int *gray)
{
	register int i;
	Byte rgb[3];
...
CopyColormap(XColor *colrs, int cmapSize, Byte cmap[3][XmHTML_MAX_IMAGE_COLORS])
{
	...
	{
		colrs[i].red   = (cmap[CM_RED][i]) << 8;
		colrs[i].green = (cmap[CM_GREEN][i]) << 8;
		colrs[i].blue  = (cmap[CM_BLUE][i]) << 8;
	}
}
