#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/images.c,v 1.1.1.1 1998/08/25 17:48:02 rwm Exp $";
#endif
/*****
...
getMaxColors(Widget w, int max_colors)
{
	...
	ncolors = GetVisualMapEntries(visual) > XmHTML_MAX_IMAGE_COLORS ?
			XmHTML_MAX_IMAGE_COLORS : GetVisualMapEntries(visual); 
...
*****/
static XXImage*
XImageBizarre(XmHTMLWidget html, int depth, XXImage *ximage)
{
	_XmHTMLWarning(__WFUNC__(html, "_XmHTMLCreateXImage"),
		XMHTML_MSG_62, depth, XXImageBitsPerPixel(ximage));
	...
	Dimension height, String url)
{
	...
	XVisual *vis   = xcc->visual;
	static XXImage *ximage = NULL;
		...
		case 1:
			{
				...
				data = (Byte*)malloc(XXImageBytesPerLine(ximage) * height);
				XXImageData(ximage) = (char *)data;
		...
		case 2:
			{
				...
				bpp = XXImageBitsPerPixel(ximage);
		...
		case 6:
			{
				...
				if(XXImageBitsPerPixel(ximage) != 8)
					return(XImageBizarre(html, depth, ximage));
		...
		case 16:
			{
				...
				if(depth == 12 && XXImageBitsPerPixel(ximage) != 16)
					return(XImageBizarre(html, depth, ximage));
...
*****/
void
_XmHTMLFillXImage(XmHTMLWidget html, XXImage *ximage, XCC xcc, Byte *data,
	...
	*/
	...
	switch(XCCGetDepth(xcc))
	{
		case 8:
		{
			...
			* processed.
			*/
			imagedata = (Byte*)XXImageData(ximage) + lo*imWIDE;
			pp = data;
		...
		case 4:
		{
			...
			bpl = XXImageBytesPerLine(ximage);
    
			pp = data;

			if(XXImageCheck4bpp(ximage))
			{
				lo /= wide;	/* starting scanline index (image data) */
				hi /= wide;	/* ending scanline index (image data) */

				/* compute offset into XImage data */
				imagedata = (Byte*)XXImageData(ximage) + lo*bpl;
				...
				for(i = lo, lip = imagedata; i < hi; i++, lip += bpl)
				{
					for(j = 0, ip = lip, half = 0; j < wide; j++, pp++, half++)
					{
						...
						if(XXImageBitmapBitOrder(ximage) == XLSBFirst)
						{
							if(half&1)
							{
								*ip = *ip + (xcol<<4);
			...
			{
				/* compute offset into XImage data */
				imagedata = (Byte*)XXImageData(ximage) + lo;
		...
		case 6:
		{
			...
			bpl = XXImageBytesPerLine(ximage);
			imagedata = (Byte*)XXImageData(ximage) + (lo/wide)*bpl;
		...
		case 16:
		{
			...
			/* 2 bytes per pixel */
			bpl = XXImageBytesPerLine(ximage);
			imagedata = (unsigned short*)XXImageData(ximage) + (lo/wide)*bpl;

			pp = data;

			if(XXImageByteOrder(ximage) == XMSBFirst)
			{
	...
	if(dither || (max_colors && gcolors > max_colors))
	{
		...
		if(!use_local_cmap)
		{
			...
			memcpy(img_data->cmap, master->cmap,
				master->cmapsize * sizeof(XXColor));
...
_XmHTMLInfoToPixmap(XmHTMLWidget html, XmHTMLImage *image, 
	...
	unsigned long *global_cmap, XPixmap *clip)
{
	XColormap cmap;
	XWindow win;
	unsigned long *color_map;
	XXImage *ximage = NULL;
	static XPixmap pixmap;
	XCC xcc;
	ToolkitAbstraction *tka = NULL;
	...
	*/
	if(tka->IsRealized(html))
		win = tka->win;
	else
		win = XXDefaultRootWindow(tka->dpy);

	/* get colormap: every widget is derived from core so this is easy */
	cmap = GetCoreColormap(html);
	...
	Dimension height)
{
	...
	*/
	...
	{
		ToolkitAbstraction *tka = html->html.tka;
		XWindow win = (html->html.gc == NULL ?
			XXDefaultRootWindow(tka->dpy) : tka->win);
...
initAlphaChannels(XmHTMLWidget html, Boolean for_body_image)
{
	...
	*****/
	...
	alpha->bg_cmap = (XXColor*)NULL;
	alpha->ncolors = 0;
	alpha->fb_maxsample = (1 << GetVisualDepth(html)) - 1;
	...
	if(html->html.body_image == NULL || for_body_image)
	{
		...
		/* get rgb components */
		tka->QueryColor(tka->dpy, GetCoreColormap(html), &bg_color);
	...
	else
	{
		...
		*****/
		...
		alpha->bg_cmap = (XXColor*)calloc(alpha->ncolors, sizeof(XXColor));
		...
		/* get rgb values */
		tka->QueryColors(tka->dpy, html->core.colormap, alpha->bg_cmap,
				alpha->ncolors);
...
{
	static String func = "ImageFreeImageInfo";
...
XmHTMLImageUpdate(Widget w, XmImageInfo *image)
{
	...
	static String func = "ImageUpdate";
	...
	else
	{
		...
		{
			_XmHTMLClearArea(html, 0, 0, GetCoreWidth(html),
				GetCoreHeight(html));
			tka->Sync(tka->dpy, True);
		}
	}
...
XmHTMLImageReplace(Widget w, XmImageInfo *image, XmImageInfo *new_image)
{
	...
	XmImageStatus status;
	Boolean is_body_image;
	static String func = "ImageReplace";
	ToolkitAbstraction *tka = NULL;
