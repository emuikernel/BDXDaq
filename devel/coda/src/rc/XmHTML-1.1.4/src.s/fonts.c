#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/fonts.c,v 1.1.1.1 1998/08/25 17:48:11 rwm Exp $";
#endif
/*****
...
static XmHTMLfont *loadQueryFont(HTMLW(html), String name, String family,
...
static void initializeFontSizeLists(HTMLW(html));
...
allocFont(XFontStruct *xfont, String name, String family, Byte style)
{
	...
	font->m_lbearing = xfont->max_bounds.lbearing;
	font->m_rbearing = xfont->max_bounds.rbearing;
	font->m_width    = xfont->max_bounds.width;
	font->m_ascent   = xfont->max_bounds.ascent;
	font->m_descent  = xfont->max_bounds.descent;
	...
	Byte style)
{
	...
	xfont = tka->LoadQueryFont(tka->dpy, name);
...
*****/
static XmHTMLfont*
loadQueryFont(HTMLW(html), String name, String family, int ptsz,
...
*****/
void
_XmHTMLaddFontMapping(HTMLW(html), String name, String family,
...
*****/
static void
initializeFontSizeLists(HTMLW(html))
{
	char *chPtr;
	char size_list[64];
	int i;
...
*****/
XmHTMLfont*
_XmHTMLSelectFontCache(HTMLW(html), Boolean reset)
{
	ToolkitAbstraction *tka = HTML_ATTR(tka);
	fontCache *cache;
...
*****/
XmHTMLfont*
_XmHTMLLoadFont(HTMLW(html), htmlEnum font_id, int size, XmHTMLfont *curr_font)
{
