#ifndef production
static char rcsId[]="$Header: /net/mizar/usr/local/source/coda_source/rc/XmHTML-1.1.4/XmHTML-1.1.4/src/format.c,v 1.1.1.1 1998/08/25 17:48:01 rwm Exp $";
#endif
/*****
...
* Last modification: 	$Date: 1998/08/25 17:48:01 $
* By:					$Author: rwm $
...
* ChangeLog 
* $Log: format.c,v $
* Revision 1.1.1.1  1998/08/25 17:48:01  rwm
...
static String CopyText(HTMLW(html), String text, Boolean formatted,
...
static XmHTMLWord *ImageToWord(HTMLW(html), String attributes, 
	int *num_words, Dimension *height, XmHTMLObjectTableElement owner,
	Boolean formatted, ToolkitAbstraction *tka, Boolean is_anchor,
	Byte text_data);

static XmHTMLWord *allocFormWord(HTMLW(html), XmHTMLForm *form,
	Dimension *width, Dimension *height, XmHTMLObjectTableElement owner,
	Boolean formatted);

static XmHTMLWord *InputToWord(HTMLW(html), String attributes, 
	int *num_words, Dimension *width, Dimension *height,
	XmHTMLObjectTableElement owner, Boolean formatted);

static XmHTMLWord *SelectToWord(HTMLW(html), XmHTMLObject *start,
	int *num_words, Dimension *width, Dimension *height,
	XmHTMLObjectTableElement owner, Boolean formatted);

static XmHTMLWord *TextAreaToWord(HTMLW(html), XmHTMLObject *start,
	int *num_words, Dimension *width, Dimension *height,
	XmHTMLObjectTableElement owner, Boolean formatted);
...
static void FillBullet(HTMLW(html), XmHTMLObjectTableElement owner,
	ToolkitAbstraction *tka);

/* get properties of a table element */
static TableProperties *tableCheckProperties(HTMLW(html),
	String attributes, TableProperties *parent, Alignment halign, Pixel bg);

/* open a new table */
static XmHTMLTable *tableOpen(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg);

/* close the current table */
static XmHTMLTable *tableClose(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end);

/* open a caption in the current table */
static void tableOpenCaption(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj,
	Pixel *bg);

/* close the current caption */
static void tableCloseCaption(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end);

/* open a row in the current table */
static void tableOpenRow(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg);

/* close the current row */
static void tableCloseRow(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end);

/* open a cell in the current row */
static void tableOpenCell(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg);

/* close the current cell in the current row */
static void tableCloseCell(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end);

/* Parse body tags and update the widget */
static void ParseBodyTags(HTMLW(html), XmHTMLObject *data);

/* Check whether a linefeed is required or not */
static int CheckLineFeed(int this, Boolean force, Byte *text_data);
	...
	*height = font->height;
	...
	for(i = 0, j = 0, len = 0; ; chPtr++, len++, j++)
	{
		...
		if(*chPtr == ' ' || *chPtr == '\0')
		{
			...
			words[i].width     = tka->TextWidth(font->xfont, words[i].word,
									len);
			words[i].owner     = owner;
			words[i].font      = font;
...
*****/
static XmHTMLWord*
ImageToWord(HTMLW(html), String attributes, int *num_words, 
	...
	ToolkitAbstraction *tka, Boolean is_anchor, Byte text_data)
{
	...
	*****/
	if(!HTML_ATTR(images_enabled))
		word->width += tka->TextWidth(word->font->xfont, word->word,
						word->len);
...
*****/
static XmHTMLWord*
allocFormWord(HTMLW(html), XmHTMLForm *form, Dimension *width,
	Dimension *height, XmHTMLObjectTableElement owner, Boolean formatted)
{
	static XmHTMLWord *word;
...
*****/
static XmHTMLWord*
InputToWord(HTMLW(html), String attributes, int *num_words, 
	Dimension *width, Dimension *height, XmHTMLObjectTableElement owner,
	Boolean formatted)
{
...
*****/
static XmHTMLWord*
SelectToWord(HTMLW(html), XmHTMLObject *start, int *num_words, 
	Dimension *width, Dimension *height, XmHTMLObjectTableElement owner,
	Boolean formatted)
{
	static XmHTMLForm *form_entry;
...
*****/
static XmHTMLWord*
TextAreaToWord(HTMLW(html), XmHTMLObject *start, int *num_words,
	Dimension *width, Dimension *height, XmHTMLObjectTableElement owner,
	Boolean formatted)
{
	static XmHTMLForm *form_entry;
...
*****/
static XmHTMLWord* 
indexToWord(HTMLW(html), listStack list_stack[], int current_list,
	XmHTMLObjectTableElement owner, Boolean formatted)
{
	static XmHTMLWord *word;
	int i;
	...
	XmHTMLObjectTableElement owner, ToolkitAbstraction *tka)
{
	...
	tab->width     = tka->TextWidth(font->xfont, raw, size);
	tab->owner     = owner;
...
*****/
static String 
CopyText(HTMLW(html), String text, Boolean formatted, Byte *text_data,
...
*****/
static void 
ParseBodyTags(HTMLW(html), XmHTMLObject *data)
{
	char *chPtr;
	...
	*****/
	if(HTML_ATTR(body_image))
		_XmHTMLRecomputeHighlightColor(html, HTML_ATTR(anchor_activated_fg));
	else
		_XmHTMLRecomputeHighlightColor(html, HTML_ATTR(body_bg));
}

static void
FillBullet(HTMLW(html), XmHTMLObjectTableElement owner, ToolkitAbstraction *tka)
{
	Dimension radius;
	...
	*****/
	...
	else
	{
		...
		*****/
		...
		owner->width = radius + tka->TextWidth(font->xfont, owner->text,
						owner->len);
...
*****/
static int
CheckLineFeed(int this, Boolean force, Byte *text_data)
{
...
tableCheckProperties(HTMLW(html), String attributes,
...
*****/
static XmHTMLTable*
tableOpen(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg)
{
	...
	*****/
	...
	/*****
	* If we've got a border, increase padding values. A zero padding
	* will cause everything to be flushed against the cell borders.
	*****/
	if(table->properties->border > 0)
	{
		table->hpadding++;
		table->vpadding++;
	}
...
*****/
static XmHTMLTable*
tableClose(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end)
{
	/* current table */
...
*****/
static void
tableOpenCaption(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj,
	Pixel *bg)
{
...
*****/
static void
tableCloseCaption(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end)
{
	XmHTMLTable *table = parent;
	TableRow *caption;
...
*****/
static void
tableOpenRow(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg)
{
	XmHTMLTable *table = parent;
	...
	row->properties = tableCheckProperties(html, obj->attributes,
						table->properties, HTML_ATTR(default_halign), *bg);
	/* set return alignment */
...
*****/
static void
tableCloseRow(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end)
{
	XmHTMLTable *table = parent;
	TableRow *row;
	...
	*****/
	...
	if(row->start == NULL)
	{
		_XmHTMLWarning(__WFUNC__(html, "tableCloseRow"), "Row %i lost"
			"it's start point. (near line %i in input)\n",
...
*****/
static void
tableOpenCell(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement start, XmHTMLObject *obj, Alignment *halign,
	Pixel *bg)
{
	XmHTMLTable *table = parent;
...
*****/
static void
tableCloseCell(HTMLW(html), XmHTMLTable *parent,
	XmHTMLObjectTableElement end)
{
	XmHTMLTable *table = parent;
	TableRow *row;
...
*****/
XmHTMLAnchor*
_XmHTMLNewAnchor(HTMLW(html), XmHTMLObject *object)
{
...
*****/
void
_XmHTMLformatObjects(HTMLW(old), HTMLW(html))
{
					...
					*****/
						...
						elePtr->next->id == temp->id)
					{
						...
						*/
						...
						{
							_XmHTMLDebug(2, ("format.c: _XmHTMLformatObjects; "
								"empty element %s found at line %i in input,ignoring.\n",
...
/* all url types we know about, 16 in total */
static String anchor_tokens[] = {"about", "exec", "file", "ftp", "gopher",
	"http", "https" ,"info", "mailto", "man", "news", "pipe", "telnet",
	"wais", "xexec", "zzz"};
