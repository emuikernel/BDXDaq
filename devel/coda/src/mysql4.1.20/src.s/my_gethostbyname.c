				   ...
				   int buflen, int *h_errnop)
{
  struct hostent *hp;
#ifdef DEBUG
printf("ser99: my_gethostbyname_r reached\n");
#endif
  pthread_mutex_lock(&LOCK_gethostbyname_r);
  hp= gethostbyname(name);
#ifdef VXWORKS
  *h_errnop= errno;
  if(hp == 0 && (inet_addr(name)) == -1)
  {
printf("ser13: unknown host >%s< 0x%08x\n",name,inet_addr(name));
	return(NULL);
  }
#else
  *h_errnop= h_errno;
#endif
#ifdef DEBUG
printf("ser14: err=0x%08x(%d) hp=0x%08x\n",*h_errnop,*h_errnop,hp);
#endif
  return(hp);
}
