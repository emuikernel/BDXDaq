  ...
  if (net_blocking)
    ...
    for (i=0 ; i < 2 ; i++)
    {
      while (remain > 0)
      {
		...
		/*
printf("ser: length=%d\n",length);
printf("ser: pos-> 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
      ...
      /* sergey: decode packet length */
      if (i == 0)
      {					/* First parts is packet length */
	    ulong helping;

		/*
printf("ser: %d %d %d %d\n",net->buff[net->where_b+0],
                                    net->buff[net->where_b+1],
                                    net->buff[net->where_b+2],
			                        net->buff[net->where_b+3]);
		*/
