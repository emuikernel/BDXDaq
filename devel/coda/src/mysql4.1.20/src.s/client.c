...
#else
#ifdef VXWORKS
#define CONNECT_TIMEOUT 5 /*sergey: was zero for all except windows*/
#else
...
net_safe_read(MYSQL *mysql)
{
  ...
  if (net->read_pos[0] == 255) /*sergey: special condition ?? */
  {
    if (len > 3)
    {
...
#ifdef DEBUG
printf("INFO(mysql_real_connect1): errno=%d\n",errno);
perror("INFO(mysql_real_connect1)");
#endif
  ...
  */
       ...
       mysql->options.protocol == MYSQL_PROTOCOL_TCP))
  {
...
#ifdef DEBUG
    printf("INFO(socket0): errno=%d\n",errno);
    perror ("INFO(socket0)");
#endif

    sock = (my_socket) socket(PF_INET/*AF_INET*/, SOCK_STREAM, 0);
    if(sock <= 0)
    {
      perror ("ERROR(socket1)");
      printf ("ser61: cannot open socket, sock=%d\n",sock);
	}
    if (sock == SOCKET_ERROR)
    {
      ...
      perror ("ERROR(socket2)");
      goto error;
    }

#ifdef DEBUG
    printf("INFO(socket3): errno=%d\n",errno);
    perror ("INFO(socket3)");
#endif

    net->vio = vio_new(sock, VIO_TYPE_TCPIP, FALSE); /* VXWORKS fix inside */
    ...
    */
    ...
    if ((int) (ip_addr = inet_addr(host)) != (int) INADDR_NONE)
    {
      ...
      /*sergey: same as 'sock_addr.sin_addr.s_addr = inet_addr(host);' ??? */
    }
    else
    {
      /* server is host name */
    ...
    if( (res=connect(sock,(const struct sockaddr *) &sock_addr, sizeof(sock_addr))) < 0)
    {
...
#ifdef DEBUG
printf("ser11: res=%d errno=0x%08x(%d)\n",res,errno,errno);
perror ("INFO");
#endif
  }
...
#ifdef DEBUG
printf("INFO(mysql_real_connect8): errno=%d\n",errno);
perror("INFO(mysql_real_connect8)");
#endif
...
printf("INFO(mysql_real_connect9): errno=%d\n",errno);
perror("INFO(mysql_real_connect9)");
printf("mysql_real_connect9: error message: %u/%s (%s)\n",
  net->last_errno,net->sqlstate,net->last_error);
...
void mysql_detach_stmt_list(MYLIST **stmt_list __attribute__((unused)))
{
#ifdef MYSQL_CLIENT
  /* Reset connection handle in all prepared statements. */
  MYLIST *element= *stmt_list;
  for (; element; element= element->next)
  {
    MYSQL_STMT *stmt= (MYSQL_STMT *) element->data;
    stmt->mysql= 0;
