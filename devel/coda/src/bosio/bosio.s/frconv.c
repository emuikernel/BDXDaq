...
frconv_(int *IREC, int *IFM, int *IER)
{
  ...
  *IER = 0;
  ...
  KFM = *IFM;
#ifdef DEBUG
  printf("frconv: KFM1=0x%08x\n",KFM);
  printf("frconv: LOCAL=0x%08x\n",LOCAL);
#endif
  if(KFM == 0) KFM = LOCAL;
#ifdef DEBUG
  printf("frconv: KFM2=0x%08x\n",KFM);
#endif
  JFM = IREC[3] / 16;
...
a10:
...
#ifdef DEBUG
  printf("I=%4d IREC[0]=%4d IREC[1]=%4d IREC[I]=0x%08x IREC[I+1]=0x%08x IREC[I+10]=%4d name >%4.4s%4.4s< ievt=%d\n",
        I,IREC[0],IREC[1],IREC[I],IREC[I+1],IREC[I+10],&IREC[I+irNAME1],&IREC[I+irNAME2],IREC[I+irNEVNT]);
#endif
  ...
  JFM = IREC[I+1] / 16;
#ifdef DEBUG
  printf("frconv: JFM=0x%08x\n",JFM);
#endif



  /*bos swap correction for corrupted g9a data*/ 
  if(JFM != 4)
  {
    /*printf("correcting JFM=%d\n",JFM);*/
    /*JFM = 4;*/
  }
