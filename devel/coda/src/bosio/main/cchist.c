...
main(int argc, char **argv)
{
  ...
  if(argc != 2)
  {
    printf("Usage: cchist <fpack_filename>\n");
    exit(1);
  }
  ...
  for(k=1; k<=6; k++)
  {
    for(i=1; i<=36; i++)
    {
      sprintf(histname,"TDC sec=%2d counter=%2d",k,i);
      idn = k*100+i;
      hbook1_(&idn,histname,&nbins,&x1,&x2,&ww,strlen(histname));

      sprintf(histname,"ADC sec=%2d counter=%2d",k,i);
      idn = 1000 + k*100+i;
      hbook1_(&idn,histname,&nbins,&x1,&x2,&ww,strlen(histname));
    }
  }
  ...
  for(iev=0; iev<10000000; iev++)
  {
    ...
    for(k=1; k<=6; k++)
    {
    if((ind1=bosNlink(bcs_.iw,"CC  ",k)) > 0)
    {
