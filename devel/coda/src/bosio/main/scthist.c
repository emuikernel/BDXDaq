...
main(int argc, char **argv)
{
  ...
  for(k=1; k<=6; k++)
  {
    for(j=1; j<=2; j++)
    {
      for(i=1; i<=57; i++)
      {
        if(j==1) sprintf(histname,"TDC: sec %1d slab %2d left",k,i);
        else sprintf(histname,"TDC: sec %1d slab %2d right",k,i);
  ...
  */
  ...
  for(iev=0; iev<1000000; iev++)
  {
    ...
    for(k=1; k<=6; k++)
    {
    if((ind1=bosNlink(bcs_.iw,"SCT ",k)) > 0)
    {
	  ...
	  */
      ...
      while(fb < fbend)
      {
        ...
        layer = (((fb16[0]>>8)&0xFF)&0x1)+1;
        wire = (fb16[0])&0xFF;
        tdc = fb16[1];
		/*
        printf("layer=%d, wire=%d tdc=%d (0x%08x)\n",layer,wire,tdc,(*fb));
		*/
		  
        if(layer>0&&layer<3&&wire>0&&wire<58)
        {
          tmpx = tdc;
          idn=k*1000+layer*100+wire;
          hf1_(&idn,&tmpx,&ww);
		}
        else printf("ERROR: sec=%d layer=%d wire=%d (0x%08x)\n",k,layer,wire,*fb);
