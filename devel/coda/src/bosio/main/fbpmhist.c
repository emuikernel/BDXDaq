...
main(int argc, char **argv)
{
  ...
  idn = 1;
  sprintf(histname,"rasterX",idn);
  hbook1_(&idn,histname,&nbins,&x1,&x2,&ww,strlen(histname));

  idn = 2;
  sprintf(histname,"rasterY",idn);
  hbook1_(&idn,histname,&nbins,&x1,&x2,&ww,strlen(histname));

  idn=3;
  sprintf(histname,"rasterXY",idn);
  ...
  for(iev=0; iev<100000; iev++)
  {
    ...
    if((ind1=bosNlink(bcs_.iw,"FBPM",0)) > 0)
    {
      unsigned int *fb, *fbend;
      unsigned short *fb16;
      int crate,slot,channel,edge,data,count,ncol1,nrow1,igood;
	  ...
	  */
      ...
      {
        wire = fb16[0];
        if(wire==1) tmpx = fb16[2];
        else if(wire==2) tmpy = fb16[2];
        else
        {
          igood = 0;
          printf("bad id=%d\n",wire);
	  ...
	  */
	  ...
	  {
        idn=1;
        if(tmpx>500.) hf1_(&idn,&tmpx,&ww);
        idn=2;
        if(tmpy>500.) hf1_(&idn,&tmpy,&ww);
        idn=3;
        if(tmpx>500. && tmpy>500.) hf2_(&idn,&tmpx,&tmpy,&ww);
	  }
