...
int iev_spect=0, iev_step=10, iev_START=100, iev_STOP=300000;
int ii_base=-1;
...
int nev[] = {1358,1573,2027,2166,2838,4242,4350,6143,6645,7212,
  ...
  */
  ...
  {
    hbook1_(&idn,"FADC Pedestal",&nbins,&x1,&x2,&ww,13);
  }
...
if(iev < 103) goto nextevent;
    ...
    if((ind1=bosNlink(bcs_.iw,"RC29",0)) > 0)
    {
	  ...
	  */
      ...
      if(iev%(iev_step*100) == 0)
      {
	ii_base++;
	/*
	printf("------------- ii_base=%d   iev_step=%d \n",ii_base,iev_step);
	*/
      }
	...
	*/
	  ...
	  */
	  ...
	  if(iev%iev_step == 0 && k == 0)
	  {
	    if(channel == 0 && (iev%(iev_step*1000) == 0))
	      printf("=== Event=%d  slot=%d chan=%2d  adc=0x%04x (%d)\n",iev,slot,channel,data,data);
