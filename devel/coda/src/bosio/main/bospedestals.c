...
int nev[] = {1358,1573,2027,2166,2838,4242,4350,6143,6645,7212,
  ...
  for(i1=16; i1<=25; i1++)
  {
	...
	{
      idn = i1*100+i2;
      hbook1_(&idn,"ADC PEDESTAL",&nbins,&x1,&x2,&ww,12);
	}
  }
...
printf("===== Event no. %d\n",iev);
	*/
    ...
    if((ind1=bosNlink(bcs_.iw,"RC13",0)) > 0)
    {
      unsigned int *raw1;
      int crate,slot,channel,edge,data,count,ncol1,nrow1,kk;
      int oldslot = 100;
	  ...
	  */
      ...
      baseline = sum = 0.;
      raw1 = (unsigned int *)&bcs_.iw[ind1]; /*skip first word (board header)*/
      ...
      while(k<nrow1)
      {
		...
		/*
        printf("RAW: slot=%d chan=%d data=%d count=%d\n",
          slot,channel,data,count);
		*/
