...
static int		TestupvarCmd _ANSI_ARGS_((ClientData dummy,
			    Tcl_Interp *interp, int argc, char **argv));
static int		TestExit _ANSI_ARGS_((ClientData dummy,
			    Tcl_Interp *interp, int argc, char **argv));

/*
 *----------------------------------------------------------------------
...
int
Tcl_TestInit(Tcl_Interp *interp)
{
    /*
    ...
    Tcl_CreateCommand(interp, "testasync", (Tcl_CmdProc *) TestasyncCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateCommand(interp, "testcmdinfo", (Tcl_CmdProc *) TestcmdinfoCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateCommand(interp, "testdcall", (Tcl_CmdProc *) TestdcallCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateCommand(interp, "testdstring", (Tcl_CmdProc *) TestdstringCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateCommand(interp, "testlink", (Tcl_CmdProc *) TestlinkCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateCommand(interp, "testupvar", (Tcl_CmdProc *) TestupvarCmd,
		      (ClientData) 0, (Tcl_CmdDeleteProc *) NULL);
    ...
    Tcl_CreateCommand(interp, "exit-test", TestExit, (ClientData) 0,
	    (Tcl_CmdDeleteProc *) NULL);
    Tcl_Eval(interp, "rename exit exit.old");
    Tcl_Eval(interp, "rename exit-test exit");
 ...
 * TestExit --
 *
 *	Don't do anything.  Allow procedure to run down gracefully
 *
 * Results:
 ...
 */
	...
	/* ARGSUSED */
static int
TestExit(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    forceExit = 1;
    return TCL_OK;
}
 ...
 */
	...
	/* ARGSUSED */
static int
TestasyncCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
...
static int
AsyncHandlerProc(ClientData clientData, Tcl_Interp *interp, int code)
{
...
static int
IntHandlerProc(ClientData clientData, Tcl_Interp *interp, int code)
{
    char *listArgv[4];
    char string[20], *cmd;
 ...
 */
	...
	/* ARGSUSED */
static int
TestdcallCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int  i, id;
    long l_id;
...
static void
DelCallbackProc(ClientData clientData, Tcl_Interp *interp)
{
    long l_id = (long) clientData;
    int  id   = (int) l_id;
    char buffer[10];
 ...
 */
	...
	/* ARGSUSED */
static int
TestcmdinfoCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    Tcl_CmdInfo info;
    ...
    if (strcmp(argv[1], "create") == 0) {
	Tcl_CreateCommand(interp, argv[2], (Tcl_CmdProc *) CmdProc1,
			  (ClientData) "original", CmdDelProc1);
    } else if (strcmp(argv[1], "delete") == 0) {
	Tcl_DStringInit(&delString);
	...
	/*ARGSUSED*/
static int
CmdProc1(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
	...
	/*ARGSUSED*/
static int
CmdProc2(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
...
static void
CmdDelProc1(ClientData clientData)
{
    Tcl_DStringInit(&delString);
...
static void
CmdDelProc2(ClientData clientData)
{
    Tcl_DStringInit(&delString);
 ...
 */
	...
	/* ARGSUSED */
static int
TestdstringCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int count;
 ...
 */
	...
	/* ARGSUSED */
static int
TestlinkCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
 ...
 */
	...
	/* ARGSUSED */
static int
TestMathFunc(ClientData clientData,Tcl_Interp * interp, Tcl_Value *args,
	     Tcl_Value *resultPtr)
{
    long l_id = (long) clientData;
    
    resultPtr->type = TCL_INT;
    resultPtr->intValue = (int) l_id;
    return TCL_OK;
}

/*
 ...
 */
	...
	/* ARGSUSED */
static int
TestupvarCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    if ((argc != 5) && (argc != 6)) {
