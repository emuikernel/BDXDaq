...
OpenFile **tclOpenFiles;	/* Pointer to malloc-ed array of pointers
				 ...
				 * setup yet. */
static char *strMsg = NULL;	/* Posix error return */
				 ...
				 * will be performed on this name. */
{
    int fileId, result;
    struct stat statBuf;
    char *cmdBuffer,*cmdBuffer2, *oldScriptFile;
    ...
    cmdBuffer2 = (char *) ckalloc((unsigned) statBuf.st_size+1);
...
#ifdef USE_CRUNCH
    cmdBuffer[statBuf.st_size] = EOF;
    tclCrunch(iPtr->scriptFile,cmdBuffer,cmdBuffer2,statBuf.st_size);
    result = Tcl_Eval(interp, cmdBuffer2);
#else
