...
/* VXWORKS 5.0 chdir insists on full pathnames, so we have to
 * get "cute" here
 */
	/* ARGSUSED */
int
    ...
    char **argv;			/* Argument strings. */
{
    ...
    /* force a getcwd, if necessary */
    if  (Tcl_PwdCmd(NULL, interp, 1, &pwdArg ) == TCL_ERROR)
	return TCL_ERROR;
    
    /* now, look at the directory name we've been passed.
     *    1) pathname starting with '/' are passed unchanged to chdir
     *	  2) pathname starting with ".." needs to remove trailing
     * 		directories from currentDir
     *	  3) "." is a nop
     */
    if (dirName[0] == '/')
    {
	...
	{
	    if (((p = rindex(currentDir, '/')) == NULL) || (p == currentDir))
	    {

		Tcl_AppendResult(interp, "bad arg -- attempt to cd too high ", 
			(char *) NULL);
		return TCL_ERROR;
	    }
	    *p = 0;	/* take one component off end of current dir */
	    dirName += 3;	/* skip past "../" */
	}
	sprintf(buffer,"%s/%s", currentDir, dirName);
	newDir = buffer;
	...
	/* just the ordinary case of moving "down" in a hierarchy */
	sprintf(buffer,"%s/%s", currentDir, dirName);
	newDir = buffer;
    }
...
#else
    Tcl_ResetResult(interp);
    Tcl_AppendResult(interp, "exec not supported under vxWorks",
		     (char *) NULL);
    return TCL_ERROR;
#endif /* !TCL_VW */
}
...
#else
/*
 * For vxWorks, "access" is absent so we'll need info from the
 * "stat" command to find out if files exist, are executable, etc.
 */
	...
	if (((statBuf.st_mode & S_IRUSR) == S_IRUSR) &&
	    ((statBuf.st_mode & S_IRGRP) == S_IRGRP) &&
	    ((statBuf.st_mode & S_IROTH) == S_IROTH))  {
	    interp->result = "1"; /* is readable by everyone */
	} else {
	    interp->result = "0";
	}
	goto done;
	...
	if (((statBuf.st_mode & S_IWUSR) == S_IWUSR) &&
	    ...
	    interp->result = "1"; /* is writable by everyone */
	} else {
	    interp->result = "0";
	}
	goto done;
	...
	if (((statBuf.st_mode & S_IXUSR) == S_IXUSR) &&
	    ((statBuf.st_mode & S_IXGRP) == S_IXGRP) &&
	    ((statBuf.st_mode & S_IXOTH) == S_IXOTH))  {
	    interp->result = "1"; /* is executable by everyone */
	} else {
	    interp->result = "0";
	}
	goto done;
	...
	/* if file can be statted it exists, otherwise not */
	if (stat(fileName, &statBuf) == -1) {
	    interp->result = "0";
	} else {
	    interp->result = "1";
	...
	goto done;
#else
	Tcl_AppendResult(interp,"lstat unsupported in vxWorks",(char *)NULL);
	result = TCL_ERROR;
	goto done;
#endif /* !TCL_VW */
	...
	statOp = 0;
#else
	Tcl_AppendResult(interp, "file owned not supported in vxWorks",
			(char *)NULL);
	result = TCL_ERROR;
	goto done;
#endif /* !TCL_VW */
	...
	goto done;
#else
	Tcl_AppendResult(interp, "readlink not supported in vxWorks",
			(char *)NULL);
	result = TCL_ERROR;
	goto done;
#endif /* !TCL_VW */
    ...
    char **argv;			/* Argument strings. */
{
	...
	f = fdopen(fd, access);
/*	setbuf (fd, (char *) NULL);  CODA - turn off buffering for files */
	...
	if (permissions & TCL_FILE_READABLE) {
	    ...
	    f = fdopen(outPipe, "r");
	    setbuf (f, (char *) NULL); /* CODA - turn off buffering for files */
	}
	if (permissions & TCL_FILE_WRITABLE) {
	    ...
	    if (f != NULL) {
		f2 = fdopen(inPipe, "w");
		setbuf (f2, (char *) NULL); /* CODA - turn off buffering for files */
	    } else {
		f = fdopen(inPipe, "w");
		setbuf (f, (char *) NULL); /* CODA - turn off buffering for files */
	    }
...
#else
    /* TODO - error cleanup ??? */
    Tcl_AppendResult(interp, "| not supported for open command in vxWorks",
				 (char *) NULL);
	return TCL_ERROR;
#endif
...
#ifdef TCL_VW
	sprintf(interp->result, "%d", taskIdSelf());
#else
    if (argc == 1) {
 ...
 *
 * Side effects:
 *	This forces a graceful exit from TCL as opposed to non-graceful
 *	exit.  Principally for debugging memory problems.
 *
 *----------------------------------------------------------------------
 */
	...
	/* ARGSUSED */
int
Tcl_QuitCmd(dummy, interp, argc, argv)
    ClientData dummy;			/* Not used. */
    Tcl_Interp *interp;			/* Current interpreter. */
    int argc;				/* Number of arguments. */
    char **argv;			/* Argument strings. */
