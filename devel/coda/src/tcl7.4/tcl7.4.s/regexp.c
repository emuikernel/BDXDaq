...
#ifdef TCL_VW
#   define FAIL(m) { TclRegError(m); r = NULL; goto done; }
#   define RETURN(val) {r = val; goto done; }
#else
...
char *exp;
{
...
#ifdef TCL_VW
	if (semTake(regexpSem, WAIT_FOREVER) != OK)
	{
	    /* Don't need common exit since we didn't get semaphore */
	    panic("error taking regexpSem\n");
	    return(NULL);
	}
#endif /* TCL_VW */
	...
	/* All RETURN() and FAIL() "calls" exit through */
	/* this label so as not to lose the semaphore.  */
done:
#ifdef TCL_VW
	if (semGive(regexpSem) != OK)
	{
	    panic("error giving regexpSem\n");
	}
#endif /* TCL_VW */
...
#ifdef TCL_VW
    /* Now we can use the original version of FAIL() */
#   undef  FAIL
#   define FAIL(m)	{ TclRegError(m); return(NULL); }
#endif /* TCL_VW */
...
/*
 * Force all TclRegExec() returns to go through the common exit point.
 * This is so we don't lose our semaphore.
 */
...
char *start;
{
	...
	int r;
        ...
        if (semTake(regexpSem, WAIT_FOREVER) != OK)
        {
            /* Error taking semaphore, don't bother with common exit */
            panic("error taking regexpSem\n");
            return(0);
        }
#endif /* TCL_VW */
	...
	r = 0;
	/* All RETURN() "calls" exit through this label */
done:
#ifdef TCL_VW
	if (semGive(regexpSem) != OK)
	{
