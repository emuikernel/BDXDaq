 ...
 *     RCS:  $Id: tclCmdMZ.c,v 1.1.1.1 1996/08/21 19:30:12 heyes Exp $
 * ========================================================================
...
typedef struct {
    ...
    char *errMsg;		/* Error message returned from Tcl command,
				 * or NULL.  Malloc'ed. */
    Itcl_Namespace namesp;      /* namespace where trace was created */
    ...
    char **argv;			/* Argument strings. */
{
    ...
    if (argc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"",
            argv[0], " oldName newName\"", (char*)NULL);
        return TCL_ERROR;
    }
    ...
    if (*argv[2] == '\0') {
        ...
        if (ItclFollowNamespPath(interp, iPtr->activeNs, argv[2],
            ITCL_FIND_AUTO_CREATE, &newNs,&newName,&specificRef) != TCL_OK) {
            return TCL_ERROR;
        }
        if (newNs == NULL) {
            Tcl_AppendResult(interp, "can't access \"",
                argv[2], "\"", (char*)NULL);
            return TCL_ERROR;
        }
        if (newName == NULL) {
            Tcl_AppendResult(interp, "missing command name for \"",
                argv[2], "\"",
                (char*)NULL);
            return TCL_ERROR;
        }
        if (Tcl_FindHashEntry(&newNs->commands, newName)) {
         ...
         */
        oldNs   = cmdPtr->namesp;
        oldName = Tcl_GetHashKey(&cmdPtr->namesp->commands, cmdPtr->hPtr);

        Itcl_NsCacheCmdChanged((Itcl_Namespace)oldNs, oldName);
        Itcl_NsCacheCmdChanged((Itcl_Namespace)newNs, newName);
        ...
        hPtr = Tcl_CreateHashEntry(&newNs->commands, newName, &new);
    ...
    char **argv;			/* Argument strings. */
{
	...
	switch (*p) {
		...
		if (doCmds) {
		    ...
		    if (((Interp *) interp)->results.typeAttr &
			TCL_TYPE_RETOVR) Tcl_OvrLoadResult(interp);
    ...
    char **argv;			/* Argument strings. */
{
	...
	tvarPtr->errMsg = NULL;
        ...
        tvarPtr->namesp = (Itcl_Namespace)iPtr->activeNs;
        Itcl_PreserveData((ClientData)tvarPtr->namesp);
	...
	if (Tcl_TraceVar(interp, argv[2], flags, TraceVarProc,
		(ClientData) tvarPtr) != TCL_OK) {
            Itcl_ReleaseData((ClientData)tvarPtr->namesp);
	    ckfree((char *) tvarPtr);
	    return TCL_ERROR;
	}
				 ...
				 * information. */
{
    ...
    if ((tvarPtr->flags & flags) && !(flags & TCL_INTERP_DESTROYED)) {
         ...
         */
        nsPtr = (Namespace*)tvarPtr->namesp;
        if ((nsPtr->flags&ITCL_NS_ALIVE) == 0) {
	    Tcl_DStringInit(&err);
            Tcl_DStringAppend(&err, "context namespace \"", -1);
            Tcl_DStringAppend(&err, Itcl_GetNamespPath(tvarPtr->namesp), -1);
            Tcl_DStringAppend(&err, "\" no longer exists", -1);
            errMsg = Tcl_DStringValue(&err);

	    tvarPtr->errMsg = ckalloc((unsigned)(strlen(errMsg)+1));
	    strcpy(tvarPtr->errMsg, errMsg);
	    Tcl_DStringFree(&err);

	    result = tvarPtr->errMsg;
            goto traceVarCleanup;
        }
	...
	Tcl_tVarInitMACRO(&dummy.results);
	if (interp->freeProc == 0) {
        ...
        nsToken = Itcl_ActivateNamesp(interp, tvarPtr->namesp);
        if (nsToken == NULL) {
            code = TCL_ERROR;
        }
        else {
