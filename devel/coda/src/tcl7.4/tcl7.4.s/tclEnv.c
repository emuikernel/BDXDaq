...
#include <taskLib.h>
char **environ = NULL;
#define _STDLIB	/* to not get compat version */
#endif
    ...
    CONST char *value;		/* New value for variable. */
{
    ...
    *p = '=';
    strcpy(p+1, value);
#ifdef TCL_VW	/* update WRS copy of the environment */
    putenv(newVal);
#endif
...
#ifdef TCL_VW
#ifdef DEBUG_SEM
    logMsg("%s(%d): semGive misc", __FILE__, __LINE__);
#endif
    if (semGive(miscSem) != OK)
    {
	panic("error taking miscSem\n");
...
void Tcl_EnvFinish()
{
    ...
    /* Funky way of counting environment variables ... */
