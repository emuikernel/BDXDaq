 ...
 *     RCS:  $Id: tclUtil.c,v 1.1.1.1 1996/08/21 19:30:14 heyes Exp $
 * ========================================================================
				 ...
				 * of a Tcl_FreeProc such as free. */
{
    ...
    Tcl_ResetValueMACRO(&iPtr->results.allValue, &iPtr->results.typeAttr);
...
Tcl_AppendResult(va_alist)
    va_dcl
#else
Tcl_AppendResult(Tcl_Interp *first, ...)
#endif
{
    va_list argList;
...
Tcl_SetErrorCode(va_alist)
    va_dcl
#else
Tcl_SetErrorCode(Tcl_Interp *first, ...)
#endif
{
    va_list argList;
    char *string;
...
Tcl_PrintDouble(interp, value, dst)
    ...
    char *dst;				/* Where to store converted value;
					 ...
					 * characters. */
{
    Tcl_ConvertDoubleToBuffer(((Interp *) interp)->pdFormat, value, dst);
}

/*
 *----------------------------------------------------------------------
    ...
    int flags;			/* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    Tcl_TraceVar2(interp, name1, name2,
		    ITCL_GLOBAL_VAR|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
    ...
    value = Tcl_GetVar2(interp, name1, name2, flags & ITCL_GLOBAL_VAR);
    if (value == NULL) {
	value = "";
    }
	...
	Tcl_SetVar2(interp, name1, name2, oldValue, flags & ITCL_GLOBAL_VAR);
