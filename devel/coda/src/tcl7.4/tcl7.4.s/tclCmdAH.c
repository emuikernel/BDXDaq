 ...
 *     RCS:  $Id: tclCmdAH.c,v 1.2 1996/08/29 14:59:04 timmer Exp $
 * ========================================================================
    ...
    char **argv;			/* Argument strings. */
{
    ...
    /* Validate the result if the result is in the overloaded
       result structure ... */
    
    if (argc == 3) {
	if (((Interp *) interp)->results.typeAttr & TCL_TYPE_RETOVR)
 ...
 *----------------------------------------------------------------------
 *
 * Tcl_CommandCompleteCmd --
 *
 *	This procedure is invoked to process the "command complete" Tcl command.
 *	Implemented by Carl Timmer to make use of Tcl's ability to check
 *	if a string represents a complete or only a partially complete
 *	Tcl statement.
 *
 * Results:
 *	A standard Tcl result.
 *
 * Side effects:
 *	Returns 1 if argument string is a complete Tcl statement, 0 otherwise.
 *
 *----------------------------------------------------------------------
 */
	...
	/* ARGSUSED */
int
Tcl_CommandCompleteCmd(dummy, interp, argc, argv)
    ClientData dummy;			/* Not used. */
    Tcl_Interp *interp;			/* Current interpreter. */
    int argc;				/* Number of arguments. */
    char **argv;			/* Argument strings. */
{
    ...
    if (argc != 3) {
	sprintf(interp->result, "Usage: command complete <Tcl command string>");
	return TCL_ERROR;
    }
    
    result = Tcl_CommandComplete(argv[2]);
    if (result == 1) {
	interp->result = "1";
    } else {
	interp->result = "0";
 ...
 * Tcl_ExprBoolCmd --
 *
 *	This procedure is invoked to process the exprBool" Tcl command.
 *	See the user documentation for details on what it does.
 *
 * Results:
 *	A standard Tcl result.
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_ExprBoolCmd(dummy, interp, argc, argv)
    ClientData dummy;			/* Not used. */
    Tcl_Interp *interp;			/* Current interpreter. */
    int argc;				/* Number of arguments. */
    char **argv;			/* Argument strings. */
{
    ...
    result = Tcl_ExprBoolean(interp, str, &value);
 
    /* Last Chance:  Try directly as a boolean value.  Not same as regular TCL
       expr command in that regard. ... */
    
    if (result != TCL_OK) {
	Tcl_SetResult(interp, "", TCL_STATIC);
	result = Tcl_GetBoolean(interp, str, &value);
    }
    if (argc > 2) Tcl_DStringFree(&buffer);
    
    /* Write to the result buffer ... */
    
    if (result == TCL_OK) {
	sprintf(intRep, "%d", value);
	Tcl_SetResult(interp, intRep, TCL_VOLATILE);
    }
    return result;
}

