...
#define RORC_MAX_REVISION 4 /* revision 1: pRORC, 2: D-RORC with pluggable DIU
                            ...
                            */
#define EXP_PCI_COM_STAT_REV23 0x04300107
#define EXP_PCI_COM_STAT_REV4  0x04100107
...
typedef struct
{
  unsigned long tmcs;     /* # of time ticks                              */
  unsigned long ddlfe;    /* DDL in  FIFO empty: FE   not   sending data  */
  unsigned long ddlff;    /* DDL out FIFO full : FE   not accepting data  */
  unsigned long pcife;    /* PCI out FIFO empty: RORC not   sending data  */
  unsigned long diuff;    /* DIU in  FIFO full : RORC not accepting data  */ 
                          /* the following counters count in 32 bit words */
  unsigned long ddlfr;    /* DDL in  FIFO read : # of bytes FE sent       */
  unsigned long ddlfw;    /* DDL out FIFO write: # of bytes FE received   */ 
  unsigned long pcifr;    /* PCI out FIFO read : # of bytes read  from PC */
  unsigned long pcifw;    /* PCI in  FIFO write: # of bytes written to PC */
} rorcFifoCounter_t;
...
volatile __u32 prorc_cmd_rdy;
volatile int interrupt_arrived;
